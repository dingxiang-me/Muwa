...
Muwa encrypts everything sensitive on disk under `~/.muwa/`:
...
1. Muwa generates a fresh 32-byte key with `SecRandomCopyBytes` and persists it to your Keychain (you won't see a Touch ID prompt — see [Key Management](#key-management) below).
2. Each database is created already SQLCipher-encrypted on first open via [`EncryptedSQLiteOpener`](../Packages/MuwaCore/Storage/EncryptedSQLiteOpener.swift).
...
|---|---|---|
| Chat history | SQLCipher | `~/.muwa/chat-history/history.sqlite` |
| Memory (identity, pinned facts, episodes, transcript, FTS5 mirrors) | SQLCipher | `~/.muwa/memory/memory.sqlite` |
| Methods catalog | SQLCipher | `~/.muwa/methods/methods.sqlite` |
| Tool index | SQLCipher | `~/.muwa/tool-index/tool_index.sqlite` |
| Per-plugin databases | SQLCipher | `~/.muwa/Tools/<plugin-id>/data/data.db` |
| Per-agent database (opt-in) | SQLCipher | `~/.muwa/agents/<uuid>/db.sqlite` |
| Self-scheduling slots | SQLCipher | `~/.muwa/scheduler.sqlite` |
| Large chat attachments | AES-GCM (`.osec`) | `~/.muwa/chat-history/blobs/<sha256>.osec` |

**Attachment spillover.** Every `Attachment.image` or `Attachment.document` payload greater than or equal to **16 KB** is hashed, encrypted, and written to its own `.osec` file via [`AttachmentBlobStore`](../Packages/MuwaCore/Storage/AttachmentBlobStore.swift). The chat row stores only `{ "ref": "<sha256>", ... }`, so resaving a session no longer rewrites every attachment byte. Smaller payloads (icons, short text snippets) stay inline in the row to avoid filesystem chatter.

**Plaintext, by design.** A few artifacts deliberately stay plaintext:

- JSON config under `~/.muwa/config/`, `agents/`, `themes/`, `providers/`, `schedules/`, `watchers/`, `skills/`. These are read as raw JSON by various consumers and stay plaintext by design.
- Plugin manifests under `~/.muwa/sandbox-plugins/`.
- Vector index files under `~/.muwa/memory/vectura/<agentId>/`. These are rebuilt from the encrypted SQLite source on demand; see [Limitations](#limitations-and-trade-offs).

---

## Key Management

The DEK is managed by [`StorageKeyManager`](../Packages/MuwaCore/Identity/StorageKeyManager.swift).
...
| Attribute | Value |
|---|---|
| `kSecAttrService` | `com.muwa.storage` |
| `kSecAttrAccount` | `data-encryption-key` |
| Accessibility | `kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly` |

### Why not biometric?

Unlike the Identity master key, the DEK is **not** wrapped behind Face/Touch ID. Every Muwa launch — including background relaunches by `launchd`, Sparkle auto-updates, and watcher-driven wakeups — needs to open the encrypted databases without a user-facing prompt. `kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly` means the key is available any time the user has unlocked the Mac at least once since boot, and is never copied off the device.

### Optional: derive from the master key

For users who want their DEK to be reproducible across devices via the iCloud-synced Identity master key, `StorageKeyManager.deriveFromMasterKey(context:)` replaces the Keychain entry with `HKDF-SHA256(masterKeyBytes, salt, "muwa-storage-v1")`. The salt is persisted in two places:

- The Keychain (`com.muwa.storage` / `data-encryption-salt`), bound to the device.
- A sidecar file at `~/.muwa/.storage-key.salt` so the salt travels with the rest of the encrypted artifacts during a manual restore.
...
| `resetForWipe()` | Delete the Keychain key + salt + sidecar file and clear the cache. **Irreversible without the original key or a plaintext backup.** Used when the user explicitly wipes Muwa state. |
...
2. [`StorageMutationGate`](../Packages/MuwaCore/Storage/StorageMutationGate.swift) flips `isMutating = true`, parking new `blockingAwaitNotMutating()` callers so no `*Database.open()` can race a half-rekeyed file.
3. Every registered handle is closed via `withAllHandlesQuiesced`.
4. SQLCipher's `PRAGMA rekey` rewrites each database (enumerated from [`StorageDatabaseCatalog`](../Packages/MuwaCore/Storage/StorageDatabaseCatalog.swift)) with the new key.
...
[`StorageMaintenance`](../Packages/MuwaCore/Storage/StorageMaintenance.swift) is a background actor that runs three SQLite housekeeping operations on every registered database:
...
State is persisted in `~/.muwa/.storage-maintenance.json` so cadence survives restarts. The first load stamps the "last run" times to now, so the first tick after install never triggers a 30-day-old VACUUM.

The ticker is started from [`AppDelegate`](../Packages/MuwaCore/AppDelegate.swift) via `Task.detached` during launch.
...
| Path | Description |
|---|---|
| `~/.muwa/.storage-maintenance.json` | Last `optimize` / `checkpoint` / `vacuum` timestamps |
| `~/.muwa/.storage-key.salt` | HKDF salt sidecar (only present when DEK is master-derived) |
| `~/.muwa/chat-history/history.sqlite` | SQLCipher chat database |
| `~/.muwa/chat-history/blobs/<sha256>.osec` | AES-GCM-encrypted spilled attachments |
| `~/.muwa/memory/memory.sqlite` | SQLCipher memory database |
| `~/.muwa/memory/vectura/<agentId>/` | Per-agent VecturaKit vector index (plaintext, see Limitations) |
| `~/.muwa/methods/methods.sqlite` | SQLCipher methods catalog |
| `~/.muwa/tool-index/tool_index.sqlite` | SQLCipher tool index |
| `~/.muwa/Tools/<plugin-id>/data/data.db` | Per-plugin SQLCipher database |
| `~/.muwa/agents/<uuid>/db.sqlite` | Per-agent SQLCipher database (see [Agent DB & Self-Scheduling](AGENT_DB.md)) |
| `~/.muwa/scheduler.sqlite` | SQLCipher cross-agent next-run + pause slots |

The DEK lives in macOS Keychain, **not** in `~/.muwa/`.
