# Step 3.7 / LFM Muwa E2E Evidence - 2026-05-30

This document records the no-sign Muwa app proof for the Step/LFM lane and
later bounded MiniMax/ZAYA additions. It deliberately separates proven rows
from partial rows.

## Code State

- Final local Muwa PR head before the final vMLX repin commit:
...
- Final Step verdict: both JANG_2L and JANG_K pass the strict no-sign Muwa
...
- Current Muwa PR head after the latest Step proof-boundary refresh:
...
- Previous final vMLX pin in Muwa before the current refresh:
...
- Muwa fix: local JANGTQ sidecar preflight accepts bundles that declare
  `format: "jangtq"` with `jangtq_runtime.safetensors` even when
  `weight_format` is absent.
- Muwa fix: Step JANGTQ_K sidecar preflight uses the sidecar sentinel
...
- Muwa fix: SwiftTransformers local-tokenizer loading routes Step sentinel
...
- Muwa cache policy: engine-selected TurboQuant KV remains topology-gated.
  Full simple-KV models can use TurboQuant KV. Step 3.7 is now conservative
  native/fp16 by default in Muwa because current no-sign app rows prove
...
- Live app launches used `MUWA_DISABLE_KEYCHAIN_FOR_TESTS=1`, isolated
  `MUWA_TEST_ROOT`, and explicit `OSU_MODELS_DIR`.

## 2026-05-31 Current-Head Proof: LFM2.5 JANG_2L

- Muwa worktree head at proof time:
  `ff7b5ff9b70cb8ff23fe9b4c0a63c9f4071b0489` plus local repin/docs to vMLX
  `60b888659e1196995fa57f7af91d982e5948a680`.
- Built app:
  `/tmp/muwa-1310-60b888-nosign-dd/Build/Products/Release/osaurus.app`.
- Launch root:
  `/tmp/muwa-1310-60b888-live-root-20260531-031451`.
- Model root: `/tmp/muwa-step37-localmeta-modelroot`.
...
- `/tmp/muwa-1310-60b888-final-lfm-jang2l-warm-20260531-031528`
...
## 2026-06-01 Current-Head Repair: Step 3.7 JANG_2L Muwa App Path
...
- Guard:
  `assert-muwa-no-forced-behavior-pr.sh` passed after the patch.
- No-sign app:
...
- Keychain/signing boundary:
  ...
  ad-hoc seal. The live run used `MUWA_DISABLE_KEYCHAIN_FOR_TESTS=1`, a
  fresh `MUWA_TEST_ROOT`, and a model root containing only
  `Step-3.7-Flash-JANG_2L`.
- Smoke artifact:
  `/tmp/muwa-post1314-step37-compiled-off-20260601-012207`. Cold two-token
...
- Source proof:
  ...
  passed in Muwa against the pinned checkout. `RuntimePolicySourceTests`
...
- No-sign/keychain boundary:
  ...
  `MUWA_DISABLE_KEYCHAIN_FOR_TESTS=1`, fresh `MUWA_TEST_ROOT`, and
...
- Current Muwa PR head after the stale Step cache-mode test fix:
  `eda7ac94cd78b54846db850a44fa7f1f2dcacb4d`.
- Current Muwa PR head after Step proof-harness hardening:
  `31911e2319b324250bca3f3660a75d2d182e55a9`.
...
- Current Muwa PR head after the latest Step proof-boundary refresh:
  `9804bb474ad73cd107a493ccaba2e9b3f5c964c1`.
  Source guards `assert-muwa-vmlx-pr-readiness.sh`,
  ...
  `MUWA_DISABLE_KEYCHAIN_FOR_TESTS=1`, a fresh `MUWA_TEST_ROOT`, and a
  one-model root containing `Step-3.7-Flash-JANG_2L`; `/v1/models` served
  `step-3.7-flash-jang_2l`. Artifact
  `/tmp/muwa-post1314-9804bb47-step-jang2l-full-20260601-140957` was
  ...
  Isolation artifact
  `/tmp/muwa-post1314-920fa406-step-probe-20260601-141746` then sent a tiny
  ...
  `/tmp/muwa-post1314-920fa406-step-plain-sample.txt` showed
  `TokenIterator.next()` waiting in
  `mlx::core::scheduler::Scheduler::wait_for_one()`, so the current blocked
  row is below Muwa chat/autodetect/tool-parser wiring.
- GitHub CI on `eda7ac94` passed:
  `shellcheck`, `swiftlint`, `test-cli`, `test-core`, and
  `update_release_draft`.
- Current Step JANG_2L strict partial artifact:
  `/tmp/muwa-post1314-eda7ac94-step-jang2l-20260601-132614`.
  This no-sign app row used `MUWA_DISABLE_KEYCHAIN_FOR_TESTS=1`, a fresh
  `MUWA_TEST_ROOT`, and explicit `OSU_MODELS_DIR`; `/v1/models` served
...
- Current Step JANG_2L topology/cache after the direct row:
  `/tmp/muwa-post1314-step-turn3-cache-after.json` reported 45 layers,
...
- Model root: `/tmp/muwa-e2e-lfm-one`.
- Final warm artifact:
  `/tmp/muwa-lfm-finalapp-warm-2048-20260530-145426`.
- Harness:
...
- Model root: `/tmp/muwa-step37-modelroot-jang-and-tqk`.
- Cold artifact:
  `/tmp/muwa-step37-discoveryfix-430481c-step-jangtqk-tool-20260530-221008`.
- Cold summary:
  `/tmp/muwa-step37-discoveryfix-430481c-step-jangtqk-tool-20260530-221008/step-3.7-flash-jangtq_k_summary.json`.
- Warm artifact:
  `/tmp/muwa-step37-discoveryfix-430481c-step-jangtqk-warm-20260530-221128`.
- Warm summary:
...
- This row proves Step JANGTQ_K required-tool parsing, reasoning separation,
  ...
  and rotating/topology detection through the real no-sign Muwa app.
...
- Launch path: LaunchServices with `launchctl setenv
  MUWA_DISABLE_KEYCHAIN_FOR_TESTS 1`, isolated `MUWA_TEST_ROOT`, and
...
- The earlier vMLX pin `60b888659e1196995fa57f7af91d982e5948a680` included the
  ...
  `KVCacheSimple` even when Muwa also supplies `defaultMaxKVSize`. Without
...
- Muwa `ModelRuntime.shouldUseTurboQuantByDefault` now keeps Step 3.7
  ...
  default.
- Focused Muwa source tests pin the current Step native/fp16 policy text.
...
- Muwa PR head: `497a52dad9c7c98df407ee3b855216401d9d2d71`.
- No-sign app:
...
- Muwa PR head: `6aa8aa17ee83d82f6c04ca2ff69bdb2af14b59c9`.
- No-sign app:
...
- The earlier red row
  `/tmp/muwa-step37-jangtqk-open-proof-20260530-151428` failed because the
