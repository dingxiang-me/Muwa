...
A distributable plugin is a single zip named `<plugin_id>-<version>.zip` containing the compiled dylib and any companion files Muwa surfaces in the UI:
...
```bash
muwa-cli tools package <plugin_id> <version> [dylib_path]
```

`muwa-cli tools package` auto-detects `.dylib` files in the current directory, and includes `web/`, `SKILL.md`, `README.md`, `CHANGELOG.md` if present. Output: `<plugin_id>-<version>.zip` in the current directory.

## Installed layout (created by Muwa)

When a user runs `muwa-cli tools install <zip>` (or installs from the marketplace), Muwa unpacks the zip into the user's tools directory and writes a `receipt.json` recording the install metadata, hashes, and signatures. The author never writes `receipt.json` — it's host-generated.

```
~/Library/Application Support/Muwa/Tools/my-plugin/
├── 0.1.0/                       # version directory (created on install)
│   ├── MyPlugin.dylib           # from the zip
│   ├── receipt.json             # generated by Muwa, not the author
│   ├── web/                     # from the zip
...
├── current → 0.1.0              # symlink Muwa maintains
```
...
- `muwa_plugin_entry_v2(host: *const osr_host_api) -> *const osr_plugin_api`

The legacy `muwa_plugin_entry` symbol still loads but is not recommended for new plugins (no host API access).
...
Required for release builds. Without a signature, Muwa refuses to load the plugin.
...
For Apple notarization (required when distributing outside the Muwa marketplace), submit a zip:
...
After install, Muwa writes a `receipt.json` next to the dylib recording install metadata:
...
`muwa-cli tools verify` reads each receipt and re-hashes the dylib to confirm the bundle hasn't been tampered with after install. As an author you don't write or ship `receipt.json` — Muwa generates it from the artifact's metadata when the user installs your zip.

## User consent

On first install in release builds, Muwa shows the user a permission summary and creates `.user_consent` once they grant. Until that file exists the plugin fails to load with `consent_required:` prefixed errors. DEBUG builds (running `muwa-cli tools dev`) skip consent.
...
After running `muwa-cli tools package`, install the zip you produced:

```bash
muwa-cli tools install ./my-plugin-0.1.0.zip
```

Muwa unpacks the zip into the tools directory, generates a `receipt.json`, updates the `current` symlink, and triggers a reload. To trigger a reload manually after files change in place:

```bash
muwa-cli tools reload
```

For an even tighter dev loop, use `muwa-cli tools dev` — it builds, installs, and watches your sources for changes.
