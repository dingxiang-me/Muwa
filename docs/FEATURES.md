...
| Gemma 4 Diffusion `diffusion_gemma` | Blocked with an explicit block-diffusion diagnostic; current vMLX scaffold decodes config and fails closed before autoregressive generation. | Wire only after vmlx exposes native canvas denoising generation and Osaurus has live multi-turn/cache/RAM proof. | vmlx DiffusionGemma engine plus Osaurus diagnostics. |
