...
/// Process entry point.
///
/// `MUWA_SPAWN_CHECK=1` makes the binary print a sentinel and exit before any
...
private extension MuwaApp {
    ...
    var helpMenuCommands: some Commands {
        CommandGroup(replacing: .help) {
            Button {
                openURL(ProductBrand.docsURL)
            } label: {
                Text(verbatim: L("Muwa Help"))
            }
            .keyboardShortcut("?", modifiers: .command)
            ...
            Button {
                openURL(ProductBrand.communityURL)
            } label: {
                Text(verbatim: L("Discord Community"))
            }

            Button {
                openURL(ProductBrand.issueURL)
            } label: {
            ...
            Button {
                openURL("\(ProductBrand.docsURL)keyboard-shortcuts")
            } label: {
