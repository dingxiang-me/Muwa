...
```bash
swift build --package-path Packages/MuwaEvals --product muwa-evals
codesign --force --sign - \
  --entitlements Packages/MuwaEvals/muwa-evals.entitlements \
  Packages/MuwaEvals/.build/debug/muwa-evals

ANTHROPIC_API_KEY=... JUDGE_MODEL=xai/grok-4.3 XAI_API_KEY=... \
Packages/MuwaEvals/.build/debug/muwa-evals run \
  --suite Packages/MuwaEvals/Suites/SandboxFrontier \
...
- The freshly signed binary triggers a one-time macOS Keychain prompt
  ("muwa-evals wants to use your confidential information") for the
  Muwa storage key — approve with **Always Allow**. Every re-sign
  re-prompts.
- Don't run sandbox evals while another process (the Muwa app, a second
  ...
  Recovery: `rm -rf ~/.muwa/container/containers/muwa-sandbox` to
...
- `expect.agentLoop.sandboxFiles` asserts on files in the agent's VM home,
  ...
  (`~/.muwa/container/workspace/agents/<agent>/`). All other assertions
