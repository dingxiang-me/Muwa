...
Catalog-driven behaviour / integration tests for Muwa that hit a real model (Foundation, MLX, remote provider).
...
Packages/MuwaEvals/
  ...
  Sources/
    MuwaEvalsKit/    — library (case schema, runner, scorers, model override)
    MuwaEvalsCLI/    — `muwa-evals` executable
  Suites/
...
make evals EVALS_SUITE=Packages/MuwaEvals/Suites/AgentLoop  # other suite
```

Or call the CLI directly if you need flags the Makefile doesn't expose:

```bash
cd Packages/MuwaEvals
swift run muwa-evals run --suite Suites/CapabilitySearch --model foundation
swift run muwa-evals run --suite Suites/CapabilitySearch --filter browser --out report.json
swift run muwa-evals run --suite Suites/CapabilitySearch --bootstrap-plugins
```
...
a deterministic in-process storage key; release builds still use MuwaCore's
...
```json
{
  ...
  "fixtures": {
    "requirePlugins": ["muwa.browser"],
    "enableSkills": ["Muwa Browser"],
...
```bash
make evals EVALS_SUITE=Packages/MuwaEvals/Suites/AgentLoop MODEL=foundation
make evals EVALS_SUITE=Packages/MuwaEvals/Suites/AgentLoop MODEL=mlx-community/Qwen3-4B-MLX-4bit
make evals EVALS_SUITE=Packages/MuwaEvals/Suites/AgentLoop MODEL=openai/gpt-4o-mini JUDGE_MODEL=openai/gpt-4o
```
...
2. `swift run muwa-evals run --suite Suites/<Domain> --filter my-case` to iterate.
...
2. In `Sources/MuwaEvalsKit/EvalRunner.swift`, add a `case "<newdomain>":` arm to `runOne(...)`. Keep domain runners as separate top-level functions; merging them into one branch gets messy fast.
3. If the domain needs a new `expect.<sub>` block, add it to `EvalCase.Expectations` in `Sources/MuwaEvalsKit/EvalCase.swift` (all sub-blocks are optional so existing cases keep decoding).
4. If the domain drives an LLM agent loop or a judge, add a public facade in MuwaCore (mirror `CapabilityClaimsEvaluator`) rather than reaching into internal chat types from the evals package.

## CI isolation

This package is a **separate Swift package**. CI / Xcode builds run `swift build` and `swift test` from `Packages/MuwaCore`, never from here. Even if someone does `swift test` from inside `Packages/MuwaEvals`, no test target exists yet — runner unit tests should be added with a `MUWA_EVALS_ENABLED=1` env-var gate so they never burn tokens unintentionally.

## Future hooks (deliberately stubbed)

- `muwa-evals diff baseline.json current.json` — regression check against a stored baseline.
