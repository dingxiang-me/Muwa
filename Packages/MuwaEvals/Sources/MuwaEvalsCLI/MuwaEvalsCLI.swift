...
//  Muwa-evals
//
//  Tiny CLI over `MuwaEvalsKit`. Deliberately no
...
//
//  Usage:
//    muwa-evals run --suite Suites/CapabilitySearch [--model foundation] [--filter browser] [--out report.json]
//
//  Exit codes:
...
struct MuwaEvalsCLI {
    ...
    static func defaultFloorsURL() -> URL {
        URL(fileURLWithPath: "Packages/MuwaEvals/Config/recall_floors.json")
    }
    ...
    static func printUsage() {
        let usage = """
            muwa-evals — run behaviour evals against a chosen model

            USAGE:
                muwa-evals run --suite <dir> [--model <id>] [--filter <substr>] [--out <path>]
            ...
            FLAGS:
                ...
                --floors <path>       Path to recall_floors.json. Defaults to
                                      `Packages/MuwaEvals/Config/recall_floors.json`
                ...
                --startup-timeout <s> Wall-clock guard for startup bootstrap
                                      ...
                                      MUWA_EVALS_STARTUP_TIMEOUT_SECONDS.
            ...
            EXAMPLES:
                muwa-evals run --suite Suites/CapabilitySearch --model foundation
                muwa-evals run --suite Suites/CapabilitySearch --filter browser --out report.json
                muwa-evals run --suite Suites/CapabilitySearch --threshold 0.25 --report-forensics
                muwa-evals run --suite Suites/CapabilitySearch --fail-on-floor
            """
        print(usage)
    }
}
