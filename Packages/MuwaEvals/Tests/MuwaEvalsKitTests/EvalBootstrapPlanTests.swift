...
struct EvalBootstrapPlanTests {
    ...
    @MainActor
    @Test func isolatedSearchStorageOverridesMuwaRoot() {
        ...
        #expect(root != nil)
        #expect(MuwaPaths.overrideRoot == root)
        #expect(root?.lastPathComponent.hasPrefix("muwa-evals-") == true)
    ...
    @Test func nonIsolatedBootstrapDoesNotReplaceExistingRootOverride() {
        ...
        let existingRoot = FileManager.default.temporaryDirectory
            .appendingPathComponent("muwa-evals-existing-\(UUID().uuidString)", isDirectory: true)
        MuwaPaths.overrideRoot = existingRoot
        defer {
            MuwaPaths.overrideRoot = previousRoot
        }
        ...
        #expect(root == nil)
        #expect(MuwaPaths.overrideRoot == existingRoot)
    }
