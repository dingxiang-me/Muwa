...
Test harness for Muwa native-plugin authors. Lets you unit-test your
plugin's `muwa_plugin_entry_v2`, `init`, `invoke`, `handle_route`,
...
Muwa app or depending on `MuwaCore`.
...
struct MyPluginConfigTests {
    ...
    @Test func setupWebhookOnTunnelChange() {
        ...
        host.withInstalled { hostAPI in
            ...
            let ctx = MyPlugin.muwa_init(hostAPI: hostAPI)

            // 2. Drive the lifecycle event you care about.
            "https://0xagentX.agent.muwa.ai".withCString { val in
...
- Spawn a real Muwa app or background tasks.
...
For end-to-end tests against a live host, run the Muwa app and use
`muwa-cli tools dev` for hot-reload iteration.
