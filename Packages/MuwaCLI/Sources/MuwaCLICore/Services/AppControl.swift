...
//  Muwa
//
//  Service for controlling the Muwa app via distributed notifications and launching it if needed.
//
...
public struct AppControl {
    ...
    public static func launchAppIfNeeded() async {
        ...
        do {
            ...
            openByBundle.arguments = ["-n", "-b", "com.dinoki.muwa", "--args", "--launched-by-cli"]
        ...
        if !launched {
            fputs(
                "Could not launch Muwa.app. Install it with Homebrew: brew install --cask muwa\n",
                stderr
            )
            return
        }
    ...
    /// Attempts to locate the installed Muwa.app bundle path using common locations and Spotlight.
    private static func findAppBundlePath() -> String? {
        ...
        if let path = spotlightFind(queryArgs: [
            ...
            "kMDItemCFBundleIdentifier == 'com.dinoki.muwa'",
        ]) {
