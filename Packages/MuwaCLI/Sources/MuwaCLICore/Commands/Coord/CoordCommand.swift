...
public struct CoordCommand: Command {
    ...
    private static func printUsage() {
        let usage = """
            muwa coord <subcommand> [--root PATH]
...
enum CoordCommandError: LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        ...
        case .invalidFeatureFlagsUsage:
            return "Usage: muwa coord feature-flags [list|get <name>|set <name> <true|false>]"
        case .invalidLockUsage:
            return
                "Usage: muwa coord lock [list|acquire <resource> --owner <owner> [--ttl seconds]|release <resource> --owner <owner> [--force]|reap]"
        }
    }
}
