...
//  Muwa enforces at install time. Produces either an "OK" summary or a
//  diagnostic listing of missing or malformed fields.
//
//  This validator is intentionally structural rather than a full
//  PluginManifest decode: the CLI doesn't depend on MuwaCore, and a
...
public struct ManifestValidate {
    ...
    public static func execute(args: [String]) {
        guard let path = args.first, !path.isEmpty else {
            fputs("Usage: muwa manifest validate <manifest.json>\n", stderr)
            exit(EXIT_FAILURE)
        }
    ...
    /// Validates `min_muwa` / `min_macos` shape. Host enforces
