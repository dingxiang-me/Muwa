...
public struct ToolsDoctor {
    ...
    private static func runChecks(for pluginDir: URL, fm: FileManager) -> [Check] {
        ...
        // 8. Sandbox status — Muwa is sandboxed in production. We can't
        ...
        checks.append(
            .init(
                ...
                result: .ok(
                    "doctor cannot introspect; check Console.app for [Muwa] dlopen errors if the plugin fails to load"
                )
            )
        )
    ...
    private static func entrySymbolChecks(dylibURL: URL) -> [Check] {
        ...
        let v2 = dlsym(handle, "muwa_plugin_entry_v2") != nil
        let v1 = dlsym(handle, "muwa_plugin_entry") != nil

        switch (v2, v1) {
        case (true, _):
            checks.append(.init(label: "entry symbol", result: .ok("muwa_plugin_entry_v2 (v3 surface)")))
        case (false, true):
            checks.append(
                .init(
                    label: "entry symbol",
                    result: .warn(
                        "only legacy muwa_plugin_entry exported; plugin loads but cannot call host APIs. "
                            + "Rebuild with muwa_plugin_entry_v2 to access the v3 surface."
                    )
                )
            )
        case (false, false):
            checks.append(
                .init(
                    label: "entry symbol",
                    result: .fail("neither muwa_plugin_entry_v2 nor muwa_plugin_entry is exported")
                )
            )
        }
