...
public struct ToolsInstall {
    public static func execute(args: [String]) async {
        guard let src = args.first, !src.isEmpty else {
            fputs(
                "Usage: muwa tools install <plugin_id|url-or-path> [--version <semver>] [--no-consent]\n",
                stderr
            )
            exit(EXIT_FAILURE)
        }
    ...
    private static func installManual(src: String, grantConsent: Bool) async {
        ...
        // `muwa-plugin.json` is caught BEFORE we touch the install
        ...
        for candidate in ["muwa-plugin.json", "plugin.json", "manifest.json"] {
        ...
        do {
            ...
            if !grantConsent {
                print(
                    "Consent NOT granted (--no-consent). Approve in Muwa settings before the plugin will load."
                )
            }
