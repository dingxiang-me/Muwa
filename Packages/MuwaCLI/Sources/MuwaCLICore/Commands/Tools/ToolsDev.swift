...
//
//  1. Project-root mode (no args): reads muwa-plugin.json from cwd,
...
public struct ToolsDev {
    ...
    private static func runProjectMode(webProxyURL: String?) async {
        ...
        let configURL = cwd.appendingPathComponent("muwa-plugin.json")
        guard fm.fileExists(atPath: configURL.path) else {
            fputs("No muwa-plugin.json found in current directory.\n", stderr)
            fputs("Run this command from a plugin project root, or pass a plugin_id:\n", stderr)
            fputs("  muwa tools dev <plugin_id>\n\n", stderr)
            fputs("To create a new plugin project:\n", stderr)
            fputs("  muwa tools create MyPlugin\n", stderr)
            exit(EXIT_FAILURE)
        }
        ...
        guard let configData = try? Data(contentsOf: configURL),
            ...
            fputs("Failed to parse muwa-plugin.json\n", stderr)
            exit(EXIT_FAILURE)
        }
    ...
    private static func sendReload() {
        AppControl.postDistributedNotification(
            name: "com.dinoki.muwa.control.toolsReload",
            userInfo: [:]
        )
    }
