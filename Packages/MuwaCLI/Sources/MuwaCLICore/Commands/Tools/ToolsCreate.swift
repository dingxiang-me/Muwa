...
public struct ToolsCreate {
    public static func execute(args: [String]) {
        guard let name = args.first, !name.isEmpty else {
            fputs("Usage: muwa tools create <name> [--language swift|rust]\n", stderr)
            exit(EXIT_FAILURE)
        }
        var language = "swift"
    ...
    private static func createPluginConfig(dir: URL, pluginId: String, version: String) {
        let config = """
            ...
            """
        try? config.write(
            to: dir.appendingPathComponent("muwa-plugin.json"),
            atomically: true,
            encoding: .utf8
        )
    }
    ...
    private static func createSwiftPlugin(name: String, rootDirectory: URL) {
        ...
        let pluginSwift = """
            ...
            // in `muwa_plugin.h`. Field order, count, and types are
            ...
            @_cdecl("muwa_plugin_entry_v2")
            public func muwa_plugin_entry_v2(_ host: UnsafePointer<osr_host_api>?) -> UnsafeRawPointer? {
                hostAPI = host
                return UnsafeRawPointer(&api)
            }

            @_cdecl("muwa_plugin_entry")
            public func muwa_plugin_entry() -> UnsafeRawPointer? {
                return UnsafeRawPointer(&api)
            }
            """
        ...
        let readme = """
            ...
            An Muwa plugin (v2 ABI).

            ## Development

            Start dev mode from the project root. This builds the plugin, installs it
            into Muwa, and watches for source changes with automatic rebuild + reload:
            ...
            muwa tools dev --web-proxy http://localhost:5173
            ```
            ...
            muwa tools package dev.example.\(name) 0.1.0
            muwa tools install ./dev.example.\(name)-0.1.0.zip
            ```
            ...
            The `muwa-plugin.json` file at the project root stores the plugin_id
            and version used by `muwa tools dev`.
            """
    ...
    private static func createRustPlugin(name: String, rootDirectory: URL) {
        ...
        let libRs = """
            ...
            // ── Host API (provided by Muwa at init) ──
            //
            // This Rust mirror MUST stay byte-compatible with `osr_host_api`
            // in `muwa_plugin.h`. Field order, count, and types are FROZEN
            ...
            #[no_mangle]
            pub unsafe extern "C" fn muwa_plugin_entry_v2(
            ...
            #[no_mangle]
            pub unsafe extern "C" fn muwa_plugin_entry() -> *const OsrPluginApi {
                &raw const PLUGIN_API
            }
            """
        ...
        let readme = """
            ...
            An Muwa plugin (v2 ABI) written in Rust.
    ...
    private static func createClaudeMd(name: String, displayName: String, dir: URL, language: String) {
        ...
        let claudeMd = """
            # \(displayName) - Muwa Plugin

            This is an Muwa plugin project. Use this guide to develop, test, and submit the plugin.
            ...
            Muwa plugins use a C ABI interface. New plugins target the **v3 documented surface** by
            exporting `muwa_plugin_entry_v2(host)` and reading host callbacks from the
            ...
            **Reserved (do not call):** `dispatch_clarify`, `dispatch_add_issue`. The slots remain
            ...
            50 MB cap and refuses paths outside `~/.muwa/artifacts/`.

            \(isSwift ? """
            ```swift
            if let fileRead = hostAPI?.pointee.file_read {
                let request = #"{"path":"/Users/me/.muwa/artifacts/abc/file.png"}"#
            ...
            unsafe {
                if let Some(file_read) = (*HOST_API).file_read {
                    let req = make_c_string(r#"{"path":"/Users/me/.muwa/artifacts/abc/file.png"}"#);
            ...
            private struct MyAPITool {
                ...
                struct Args: Decodable {
                    let query: String
                    let _secrets: [String: String]?  // Secrets injected by Muwa
                }
            ...
            When a user has a working folder selected in chat, Muwa automatically injects the folder context into tool payloads. This allows your plugin to resolve relative file paths.
            ...
            private struct MyFileTool {
                ...
                struct Args: Decodable {
                    let path: String
                    let _context: FolderContext?  // Folder context injected by Muwa
                }
            ...
            MCP tools map directly to Muwa tools:

            | MCP Concept | Muwa Equivalent |
            |-------------|-------------------|
            ...
            Becomes this Muwa manifest entry:
            ```json
            {
              "id": "get_weather",
              "description": "Get weather for a location",
            ...
            This builds the plugin, installs it into Muwa, and watches for source changes.
            ...
            Edit source files and save. `muwa tools dev` automatically rebuilds and reloads.
            ...
            Before submitting to the Muwa plugin registry:

            - [ ] Plugin builds without warnings
            - [ ] `muwa manifest extract` returns valid JSON
            ...
            - Check `muwa manifest extract` for errors
            ...
            - Test with `echo '{"param":"value"}' | muwa manifest extract ...`
            """
