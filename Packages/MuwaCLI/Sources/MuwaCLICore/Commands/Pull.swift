...
public struct PullCommand: Command {
    ...
    public static func execute(args: [String]) async {
        guard let modelId = args.first, !modelId.isEmpty else {
            ...
            fputs("Usage: muwa pull <model_id>\n", stderr)
            fputs("Example: muwa pull mlx-community/Llama-3.2-1B-4bit\n", stderr)
            exit(EXIT_FAILURE)
        }

        print("Pulling \(modelId) ...")

        // Resolve the local destination: ~/.muwa/models/<org>/<name>
    ...
    /// Attempts to read the Muwa shared defaults for the user-configured models path,
    /// falling back to ~/.muwa/models.
    private static func resolveLocalDirectory(for modelId: String) -> URL {
        let baseDir: URL
        if let shared = UserDefaults(suiteName: "group.com.muwa.shared"),
            ...
            !storedPath.isEmpty
        {
            ...
            baseDir = FileManager.default.homeDirectoryForCurrentUser
                .appendingPathComponent(".muwa/models", isDirectory: true)
        }
...
// MARK: - Minimal glob matcher (standalone, no MuwaCore dependency)
