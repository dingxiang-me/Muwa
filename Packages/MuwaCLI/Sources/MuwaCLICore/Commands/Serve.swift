...
//  Muwa
//
//  Command to start the Muwa server with optional port configuration and network exposure settings.
//
...
public struct ServeCommand: Command {
    ...
    public static func execute(args: [String]) async {
        ...
        if expose && !assumeYes {
            ...
            let warning = """
                WARNING: Exposing Muwa to the local network will allow other devices on your LAN
        ...
        AppControl.postDistributedNotification(
            name: "com.dinoki.muwa.control.serve",
            userInfo: buildInfo()
        )
        ...
        fputs(
            ...
            Hints:
              - The port may be busy. Try: muwa serve --port \(altPort)
              - Ensure Muwa.app is installed: brew install --cask muwa
              - Try launching the app first, then serve:
                open -a /Applications/Muwa.app && muwa serve
              - You can also open the UI: muwa ui
            """.appending("\n"),
            stderr
        )
        exit(EXIT_FAILURE)
