...
//  Muwa
//
//  Tests for `muwa manifest validate`. The validator is structural
...
/// was hardened: secrets array, docs object, min_muwa / min_macos
...
final class ManifestValidateExtendedCoverageTests: XCTestCase {
    ...
    // MARK: - min_muwa / min_macos

    func testValidMinMuwaPasses() {
        let report = validate(
            #"{ "plugin_id": "com.test", "capabilities": {}, "min_muwa": "0.18.0" }"#
        )
        XCTAssertTrue(report.errors.isEmpty)
        XCTAssertTrue(report.warnings.isEmpty)
    }

    func testUnparseableMinMuwaIsAWarning() {
        let report = validate(
            #"{ "plugin_id": "com.test", "capabilities": {}, "min_muwa": "soon" }"#
        )
        XCTAssertTrue(report.errors.isEmpty, "unparseable should warn, not error: \(report.errors)")
        XCTAssertTrue(report.warnings.contains(where: { $0.contains("min_muwa") && $0.contains("semver") }))
    }
