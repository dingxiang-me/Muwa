...
public final class PluginInstallManager: @unchecked Sendable {
    ...
    public func install(pluginId: String, preferredVersion: SemanticVersion? = nil) async throws -> InstallResult {
        ...
        let keyRotatedFromVersion: SemanticVersion? = {
            ...
            NSLog(
                "[Muwa] Signing key changed for %@ — will remove old version %@ after new install succeeds",
                spec.plugin_id,
                latest.description
            )
            return latest
        ...
        do {
            ...
            NSLog("[Muwa] Minisign verification failed for \(pluginId): \(error)")
            throw PluginInstallError.signatureInvalid
        }
        NSLog("[Muwa] Minisign signature verified for \(pluginId)")
        ...
        if !skillFileURLs.isEmpty {
            ...
            for skillURL in skillFileURLs {
                ...
                NSLog("[Muwa] Installed SKILL.md for plugin \(pluginId): \(destName)")
            }
        }
        ...
        for docName in ["README.md", "CHANGELOG.md"] {
            if let docURL = findDocFile(named: docName, in: tmpDir) {
                ...
                NSLog("[Muwa] Installed \(docName) for plugin \(pluginId)")
            }
        }
        ...
        if let old = keyRotatedFromVersion, old != resolution.version.version {
            ...
            NSLog(
                "[Muwa] Key rotated for %@ — removed prior version %@",
                spec.plugin_id,
                old.description
            )
        }
        ...
        for pluginDir in entries where pluginDir.hasDirectoryPath {
            ...
            if let fallback = InstalledPluginsStore.shared.installedVersions(pluginId: pluginId).first {
                do {
                    ...
                    NSLog(
                        "[Muwa] Repaired dangling 'current' for %@ → %@",
                        pluginId,
                        fallback.description
                    )
                } catch {
                    NSLog(
                        "[Muwa] Failed to repair dangling 'current' for %@: %@",
                        pluginId,
                ...
                NSLog(
                    "[Muwa] Removed dangling 'current' for %@ (no installed versions found)",
                    pluginId
                )
            }
        }
    ...
    private func makeTempDirectory() throws -> URL {
        ...
        let dir = base.appendingPathComponent("muwa-plugin-\(UUID().uuidString)", isDirectory: true)
        try fm.createDirectory(at: dir, withIntermediateDirectories: true)
        return dir
    }
