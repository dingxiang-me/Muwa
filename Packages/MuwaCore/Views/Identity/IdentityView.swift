...
//  Muwa
//
//  Muwa Identity management UI: master address, agent addresses,
...
struct IdentityView: View {
    ...
    private var phaseContent: some View {
        ...
        case .ready(let muwaId, let deviceId):
            readyContent(muwaId: muwaId, deviceId: deviceId)
        }
    }

    @ViewBuilder
    private func readyContent(muwaId: MuwaID, deviceId: String) -> some View {
    ...
    private var subtitleText: String {
        ...
        case .noIdentity:
            return "Set up your Muwa Identity"
        case .ready:
        ...
        Task { @MainActor in
            ...
            let (newPhase, newDrift): (IdentityPhase, IdentityDrift?) = await Task.detached(
                ...
                do {
                    ...
                    let muwaId = try deriveMuwaId(from: masterKeyData)
                    ...
                    return (.ready(muwaId: muwaId, deviceId: deviceId), diagnosed)
                } catch {
    ...
    private func handleIdentityCreated(_ info: IdentityInfo) {
        ...
        // `MuwaIdentity.setup()` itself, so there's nothing to gate the
        ...
        phase = .ready(muwaId: info.muwaId, deviceId: info.deviceId)
        runRefresh()
    }
...
private enum IdentityPhase {
    case checking
    case noIdentity
    case ready(muwaId: MuwaID, deviceId: String)
}
...
private struct IdentitySetupCard: View {
    ...
    var body: some View {
        VStack(spacing: 24) {
            ...
            VStack(spacing: 8) {
                Text("Create Your Muwa Identity", bundle: .module)
    ...
    private func createIdentity() {
        ...
        Task {
            do {
                let info = try await MuwaIdentity.setup()
                await MainActor.run {
                    isCreating = false
                    onCreated(info)
                }
...
private struct MasterAddressSection: View {
    ...
    private func copyId() {
        ...
        NSPasteboard.general.setString(muwaId, forType: .string)
...
private struct AgentAddressesSection: View {
    ...
    let masterAddress: MuwaID
    let mismatchedAgentIds: Set<UUID>
    let onChange: () -> Void

    @State private var copiedAddress: MuwaID?
    ...
    private func copyAddress(_ address: MuwaID) {
        NSPasteboard.general.clearContents()
