...
private extension SandboxView {
    ...
    /// which is a host bind mount at `~/.muwa/container/workspace/`.
    ...
    var workspaceCard: some View {
        sectionCard(title: "Workspace", icon: "folder") {
            VStack(alignment: .leading, spacing: 12) {
                Text(
                    "The container's /workspace directory is bind-mounted from ~/.muwa/container/workspace/. Open it in Finder to browse or edit files directly on the host.",
                    bundle: .module
                )
                .font(.system(size: 11))
                .foregroundColor(theme.tertiaryText)
                ...
                HStack {
                    ...
                    accentButton("Open in Finder", icon: "folder") {
                        MuwaPaths.revealInFinder(MuwaPaths.containerWorkspace())
                    }
                }
            }
        }
    ...
    var resourceConfigCard: some View {
        sectionCard(title: "Resources", icon: "cpu") {
            VStack(alignment: .leading, spacing: 12) {
                ...
                toggleRow(
                    title: L("Auto-Start"),
                    description: L("Start container when Muwa launches"),
