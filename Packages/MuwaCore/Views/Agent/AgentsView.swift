...
struct AgentDetailView: View {
    ...
    private var bodyWithAlerts: some View {
        bodyWithSheets
            ...
            .themedAlert(
                ...
                message:
                    L(
                        "This will create a public URL for this agent via agent.muwa.ai. Anyone with the URL can send requests to your local server. Your access keys still protect the API endpoints."
                    ),
    ...
    private var bundleImportReviewSheet: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            Text(
                "Activate copies the agent into ~/.muwa/agents/<id>/, rekeys its database to your local key, and registers the agent for use. Discard wipes the unpacked scratch directory and changes nothing on disk.",
                bundle: .module
            )
            .font(.system(size: 11))
            .foregroundColor(theme.tertiaryText)
    ...
    private func beginBundleExport() {
        ...
        panel.message = String(
            localized: "Pick a folder for the .muwa-agent bundle.",
            bundle: .module
        )
        Task { @MainActor in
            guard await panel.beginModal() == .OK, let url = panel.url else { return }
    ...
    /// immediately. `MuwaPaths.revealInFinder` lazily creates the
    ...
    private var workspaceFolderRow: some View {
        ...
        let workspaceURL = MuwaPaths.containerAgentDir(linuxName)
        ...
        return HStack(spacing: 12) {
            ...
            Spacer()
            Button {
                MuwaPaths.revealInFinder(workspaceURL)
            } label: {
                HStack(spacing: 4) {
                    Image(systemName: "folder")
                        .font(.system(size: 10, weight: .semibold))
    ...
    private func saveAgent() {
        ...
        // The built-in Muwa agent is in-memory only — `AgentManager.update`
