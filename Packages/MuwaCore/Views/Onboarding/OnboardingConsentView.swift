...
struct ConsentBody: View {
    ...
    var body: some View {
        OnboardingTwoColumnBody(
            illustrationAsset: "muwa-data",
            leftHeadline: "Help us squash bugs",
            leftBody:
                "If Muwa crashes, an anonymous report shows us what went wrong so we can fix it."
        ) {
...
struct ConsentCTA: View {
    ...
    var body: some View {
        OnboardingBrandButton(title: "Start using Muwa", action: onFinish)
