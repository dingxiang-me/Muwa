...
//  Muwa
//
//  Onboarding step 2 — a delightful, low-friction "Meet your frog" step.
...
//      transitions).
//    - `CreateAgentBody`: the single-column body slot (hero frog + editable
//      name badge + role description + avatar picker + archetype picker).
//    - `CreateAgentCTA`: the primary "Create Frog" footer button.
//
...
final class CreateAgentState: ObservableObject {
    ...
    /// to move on can tap "Create Frog" immediately.
    ...
    static let funNames = [
        ...
        "Froggy", "Bruno", "Sunny", "Fern", "Bramble", "Ziggy",
    ]
...
struct CreateAgentBody: View {
    ...
    /// The selected frog's signature color — themes the hero glow, avatar
    ...
    /// fun rather than form-like, and the glow adopts the frog's color.
    private var heroFrog: some View {
        ...
        return ZStack {
            ...
            // makes the frog feel like it's reacting to its new identity.
    ...
    /// rolls a fun frog name; the text stays editable and a faint pencil hints
    /// at that.
    ...
    /// always picks a colorful mascot so the row of cute frogs reads as the
