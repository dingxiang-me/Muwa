...
struct ModelDownloadView: View {
    ...
    nonisolated static func makeGridLists(_ input: GridListInput) -> GridLists {
        ...
        let muwaSuggested = input.suggestedModels.filter { isOsaurusAI($0) }
        ...
        let suggSearched = SearchService.filterModels(muwaSuggested, with: searchText)
    ...
    private var explainer: some View {
        HStack(alignment: .top, spacing: 8) {
            Image(systemName: "info.circle")
                ...
                .padding(.top, 1)
            Text(
                "Muwa only runs MLX models. Try a repo from `OsaurusAI` or `mlx-community`.",
                bundle: .module
            )
            .font(.system(size: 12))
            .foregroundColor(theme.secondaryText)
