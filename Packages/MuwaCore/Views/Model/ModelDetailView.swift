...
struct ModelDetailView: View, Identifiable {
    ...
    /// Muwa actually downloads. Falls back to a loading state while the
    ...
    private var completedFooter: some View {
        HStack(spacing: 12) {
            if isExternalModel {
                ...
                // aren't owned by Muwa, so in-app delete can't remove their
    ...
    /// True for models discovered outside Muwa (Hugging Face cache, LM
    /// Studio). Their files live in another app's directory, so Muwa
...
/// subtle marker when the file is part of what Muwa downloads.
