...
//  Muwa
//
//  Sheet that accepts a theme hash, an `muwa://themes-install?hash=…`
    ...
    private var inputView: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            if !input.isEmpty && !isInputValid {
                HStack(spacing: 6) {
                    Image(systemName: "exclamationmark.triangle.fill")
                        .font(.system(size: 11))
                    Text(
                        "Doesn't look like a 64-character theme ID or muwa://themes-install link.",
                        bundle: .module
                    )
                    .font(.system(size: 11))
                    .lineLimit(2)
            ...
            HStack(spacing: 6) {
                ...
                Text(
                    verbatim:
                        "Accepts a 64-character ID, muwa://themes-install?hash=… or https://themes.muwa.ai/themes/…"
                )
                .font(.system(size: 11))
                .lineLimit(3)
