...
struct ThemeEditorView: View {
    ...
    private func saveTheme() {
        ...
        print("[Muwa] ThemeEditor: Saving theme '\(themeToSave.metadata.name)' (id: \(themeToSave.metadata.id))")
        themeManager.saveTheme(themeToSave)
        print("[Muwa] ThemeEditor: Theme saved successfully")
    ...
    private func handleImageImport(_ result: Result<[URL], Error>) {
        ...
        case .success(let urls):
            ...
            Task {
                ...
                guard let encoded else {
                    print("[Muwa] Failed to import image")
                    return
                }
        ...
        case .failure(let error):
            print("[Muwa] Image import failed: \(error)")
        }
    }
...
struct ThemeChatPreview: View {
    ...
    private func previewAvatar(size: CGFloat, name: String, tint: Color) -> some View {
        if let mascot = Bundle.module.image(forResource: "muwa-avatar-green") {
