...
struct ThemesView: View {
    ...
    var body: some View {
        ...
        .fileImporter(
            ...
            allowedContentTypes: [.json, UTType(filenameExtension: "muwa-theme") ?? .json],
    ...
    private func performDelete(_ theme: CustomTheme) {
        ...
        if success {
            print("[Muwa] Successfully deleted theme: \(themeName)")
            showToast(L("Deleted \"\(themeName)\""))
        } else {
            print("[Muwa] Failed to delete theme: \(themeName)")
        }
        themeToDelete = nil
    }
    ...
    private var communityThemesBanner: some View {
        Button(action: openCommunityThemes) {
            HStack(spacing: 14) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    Text("Discover and install more themes shared by the Muwa community", bundle: .module)
                        .font(.system(size: 12))
                        .foregroundColor(theme.secondaryText)
                        .lineLimit(2)
        ...
        .buttonStyle(PlainButtonStyle())
        .help(Text("Open muwa.ai/themes", bundle: .module))
    }

    private func openCommunityThemes() {
        if let url = URL(string: "https://muwa.ai/themes") {
    ...
    /// Honor a pending `muwa://themes-install?hash=…` deeplink request.
    ...
    private func confirmDelete(_ theme: CustomTheme) {
        guard !theme.isBuiltIn else {
            print("[Muwa] Cannot delete built-in theme: \(theme.metadata.name)")
            return
        }
        themeToDelete = theme
    ...
    private func handleImport(_ result: Result<[URL], Error>) {
        ...
        case .success(let urls):
            ...
            do {
                ...
                print("[Muwa] Failed to import theme: \(error)")
                showToast(L("Import failed: \(error.localizedDescription)"), type: .error)
            }
        case .failure(let error):
            print("[Muwa] Import failed: \(error)")
            showToast(L("Import failed: \(error.localizedDescription)"), type: .error)
        }
    }
    ...
    private func handleExport(_ result: Result<URL, Error>) {
        ...
        case .success:
            ...
            themeToExport = nil
        case .failure(let error):
            print("[Muwa] Export failed: \(error)")
        }
    }
}
