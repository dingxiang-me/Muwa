...
struct TranscriptionModeSettingsTab: View {
    ...
    @State private var transcriptionProvider: SpeechTranscriptionProvider = .local
    @State private var volcengineAPIKey: String = ""
    @State private var volcengineModelService: VolcengineASRModelService =
        VolcengineASRSettings.defaultModelService
    @State private var volcengineResourceType: VolcengineASRResourceType =
        VolcengineASRSettings.defaultResourceType
    ...
    private func loadVoiceSettings() {
        ...
        let resourceId = VolcengineASRSettings.normalizedResourceId(config.volcengineResourceId)
        volcengineModelService =
            VolcengineASRSettings.modelService(for: resourceId) ?? VolcengineASRSettings.defaultModelService
        volcengineResourceType =
            VolcengineASRSettings.resourceType(for: resourceId) ?? VolcengineASRSettings.defaultResourceType
        volcengineAPIKey = VolcengineASRKeychain.apiKey() ?? ""
    }

    private func saveVoiceSettings(saveAPIKey: Bool = false) {
        ...
        config.transcriptionProvider = transcriptionProvider
        config.volcengineResourceId = selectedVolcengineResourceId
        SpeechConfigurationStore.save(config)

        if saveAPIKey {
            VolcengineASRKeychain.setAPIKey(volcengineAPIKey)
        }
    ...
    private var hasSpeechBackend: Bool {
        if transcriptionProvider == .volcengine {
            return isVolcengineConfigured
        }
        return modelManager.downloadedModelsCount > 0 && modelManager.selectedModel != nil
    }

    private var isVolcengineConfigured: Bool {
        !volcengineAPIKey.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
            && !selectedVolcengineResourceId.isEmpty
    }

    private var selectedVolcengineResourceId: String {
        VolcengineASRSettings.resourceId(
            modelService: volcengineModelService,
            resourceType: volcengineResourceType
        )
    }
    ...
    private var requirementsCard: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(spacing: 12) {
                ...
                RequirementRowView(
                    title: transcriptionProvider == .volcengine ? L("火山引擎 API Key") : L("语音模型已下载"),
                    description: transcriptionProvider == .volcengine
                        ? L("控制台 APP Key，用于 X-Api-Key")
                        : L("本地转写必需"),
                    isComplete: transcriptionProvider == .volcengine
                        ? !volcengineAPIKey.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
                ...
                RequirementRowView(
                    title: transcriptionProvider == .volcengine ? L("火山引擎资源 ID") : L("已选择模型"),
                    description: transcriptionProvider == .volcengine
                        ? selectedVolcengineResourceId
                        : L("在模型页选择默认模型"),
                    isComplete: transcriptionProvider == .volcengine
                        ? !selectedVolcengineResourceId.isEmpty
    ...
    // MARK: - Speech Provider Card

    private var speechProviderCard: some View {
        VStack(alignment: .leading, spacing: 16) {
            HStack(spacing: 12) {
                ZStack {
                    RoundedRectangle(cornerRadius: 12)
                        .fill(theme.accentColor.opacity(0.15))
                    Image(systemName: transcriptionProvider == .volcengine ? "cloud.fill" : "waveform")
                        .font(.system(size: 20, weight: .medium))
                        .foregroundColor(theme.accentColor)
                }
                .frame(width: 48, height: 48)
            ...
            ThemedTabPicker(
                selection: $transcriptionProvider,
                tabs: SpeechTranscriptionProvider.allCases.map { ($0, $0.displayName) }
            )
            ...
            if transcriptionProvider == .volcengine {
                ...
                VStack(alignment: .leading, spacing: 12) {
                    credentialTextField(
                        title: L("API Key"),
                        placeholder: L("火山引擎控制台 APP Key"),
                        text: $volcengineAPIKey,
                        secure: true
                    )
                    .onSubmit {
                        saveVoiceSettings(saveAPIKey: true)
                    }

                    volcengineResourceSelector

                    HStack(spacing: 10) {
                        Button(action: { saveVoiceSettings(saveAPIKey: true) }) {
                            HStack(spacing: 6) {
                                Image(systemName: "key.fill")
                                    .font(.system(size: 12, weight: .medium))
                                Text("保存 API Key", bundle: .module)
                                    .font(.system(size: 12, weight: .medium))
                            }
                        ...
                        HStack(spacing: 6) {
                            Image(systemName: isVolcengineConfigured ? "checkmark.circle.fill" : "circle")
                                .font(.system(size: 12, weight: .medium))
                                .foregroundColor(isVolcengineConfigured ? theme.successColor : theme.tertiaryText)
                            Text(isVolcengineConfigured ? "已配置" : "缺少凭据", bundle: .module)
                                .font(.system(size: 11))
                                .foregroundColor(isVolcengineConfigured ? theme.successColor : theme.tertiaryText)
                        }
    ...
    private var volcengineResourceSelector: some View {
        VStack(alignment: .leading, spacing: 10) {
            VStack(alignment: .leading, spacing: 6) {
                Text("模型服务", bundle: .module)
                    .font(.system(size: 11, weight: .medium))
                    .foregroundColor(theme.secondaryText)

                ThemedTabPicker(
                    selection: $volcengineModelService,
                    tabs: VolcengineASRModelService.allCases.map { ($0, $0.displayName) }
                )
                .onChange(of: volcengineModelService) { _, _ in
                    saveVoiceSettings()
                }
            }

            VStack(alignment: .leading, spacing: 6) {
                Text("资源类型", bundle: .module)
                    .font(.system(size: 11, weight: .medium))
                    .foregroundColor(theme.secondaryText)

                ThemedTabPicker(
                    selection: $volcengineResourceType,
                    tabs: VolcengineASRResourceType.allCases.map { ($0, $0.displayName) }
                )
                .onChange(of: volcengineResourceType) { _, _ in
                    saveVoiceSettings()
                }
            }

            VStack(alignment: .leading, spacing: 6) {
                Text("资源 ID", bundle: .module)
                    .font(.system(size: 11, weight: .medium))
                    .foregroundColor(theme.secondaryText)

                Text(selectedVolcengineResourceId)
