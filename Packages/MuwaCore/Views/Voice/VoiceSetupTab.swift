...
struct VoiceSetupTab: View {
    ...
    private var usesCloudProvider: Bool {
        transcriptionProvider == .volcengine
    }

    private var hasSpeechBackend: Bool {
        usesCloudProvider ? speechService.cloudTranscriptionReady : hasModel
    }
    ...
    private var modelRequirementItem: some View {
        HStack(spacing: 8) {
            ...
            // Label
            Text(usesCloudProvider ? "火山引擎语音识别" : "语音模型", bundle: .module)
                .font(.system(size: 13, weight: .medium))
                .foregroundColor(hasSpeechBackend ? theme.primaryText : theme.secondaryText)
    ...
    private var modelStatusIcon: some View {
        if hasSpeechBackend {
            Image(systemName: "checkmark.circle.fill")
                .font(.system(size: 16, weight: .medium))
                .foregroundColor(theme.successColor)
        } else if !usesCloudProvider, let progress = downloadProgress {
            ZStack {
                Circle()
    ...
    private var modelActionView: some View {
        if usesCloudProvider {
            if speechService.cloudTranscriptionReady {
                Text("已配置", bundle: .module)
                    .font(.system(size: 11))
