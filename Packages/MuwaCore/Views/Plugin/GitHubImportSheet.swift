...
struct GitHubImportSheet: View {
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                if !report.allStdioProvidersNeedingConfiguration.isEmpty {
                    InstallReportPendingProvidersNotice(
                        ...
                        message: L(
                            "These stdio MCP servers were imported into the Muwa sandbox but ship with `${VAR}` placeholders for sensitive env vars. Tap one to open its editor:"
                        ),
                ...
                if !report.allStdioProvidersBlockedNoSandbox.isEmpty {
                    InstallReportNotice(
                        ...
                        message: L(
                            "Imported stdio MCP servers run inside the Muwa sandbox, which isn't available on this machine. These weren't installed:"
                        ),
    ...
    private func openMCPProvider(id: UUID?) {
        ...
        // ToolsTab is internal to MuwaCore; we know the Remote sub-tab is
