...
struct ClaudeMarketplaceDetailView: View {
    ...
    /// Muwa supports. These are normally hidden from the grid, so this is a
    /// safety net rather than a common state.
    private var notImportablePanel: some View {
        infoPanel(
            title: "Nothing to import into Muwa",
            message:
                "Muwa imports skills, agents, commands, and MCP servers. This plugin ships none of those."
        )
    }
