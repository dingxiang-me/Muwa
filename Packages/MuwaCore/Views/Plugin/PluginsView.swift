...
struct PluginsView: View {
    ...
    /// Honor a pending `muwa://plugins-install?tool=<id>` deeplink request.
...
private struct PluginDetailView: View {
            ...
            VStack(spacing: 8) {
                    ...
                    let webConfig = loadedPlugin?.webConfig
                {
                    Button {
                        ...
                        // Browsers cannot set the X-Muwa-Agent-Id header
                        // on top-level navigation; pass the agent id via the
                        // `muwa_agent` query param so the server accepts the
                        // initial GET. The injected `window.__muwa.fetch`
                        // helper then carries it forward to subsequent calls.
                        let agentId = Agent.defaultId.uuidString
                        let url = URL(
                            string:
                                "http://127.0.0.1:\(port)/plugins/\(plugin.pluginId)\(webConfig.mount)?muwa_agent=\(agentId)"
                        )!
    ...
    private func loadServerPort() -> Int {
        let url = MuwaPaths.serverConfigFile()
