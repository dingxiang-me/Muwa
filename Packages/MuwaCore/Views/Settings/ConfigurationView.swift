...
struct ConfigurationView: View {
                    ...
                    VStack(alignment: .leading, spacing: 24) {
                            ...
                            SettingsSection(title: "General", icon: "gear") {
                                VStack(alignment: .leading, spacing: 20) {
                                    ...
                                    SettingsToggle(
                                        ...
                                        description: "Launch Muwa when you sign in",
                                    ...
                                    SettingsSubsection(label: "Command Line Tool") {
                                        VStack(alignment: .leading, spacing: 12) {
                                            Text(
                                                "Install the `muwa` CLI into your PATH for terminal access.",
                                                bundle: .module
                                            )
                                            .font(.system(size: 12))
                                            .foregroundColor(theme.tertiaryText)
                        ...
                        if matchesSearch(
                            ...
                            SettingsSection(title: "Privacy", icon: "hand.raised") {
                                VStack(alignment: .leading, spacing: 20) {
                                    Text(
                                        "Control what anonymous data Muwa collects.",
                                        bundle: .module
                                    )
                                    .font(.system(size: 12))
                                    .foregroundColor(theme.secondaryText)
                                    ...
                                    SettingsToggle(
                                        ...
                                        description:
                                            "Send anonymous, aggregated usage analytics to help improve Muwa. Never includes your chats, prompts, files, or keys. Turn off any time.",
                            ...
                            SettingsSection(title: "Chat", icon: "message") {
                                VStack(alignment: .leading, spacing: 20) {
                                    ...
                                    StyledSettingsTextArea(
                                        ...
                                        placeholder: "Enter the default Muwa agent's instructions...",
                                        hint: "Optional. Persona for the built-in Muwa agent."
                                    )
                                    ...
                                    SettingsSubsection(label: "Clipboard") {
                                        VStack(alignment: .leading, spacing: 8) {
                                            ...
                                            Text(
                                                "Automatically detect and offer text from any app as context. Includes 'grab selection' feature when summoning Muwa.",
                                                bundle: .module
                                            )
                                            .font(.system(size: 11))
                                            .foregroundColor(theme.tertiaryText)
            ...
            if isResetting {
                ZStack {
                    ...
                    VStack(spacing: 24) {
                        ...
                        VStack(spacing: 8) {
                            Text("Resetting Muwa", bundle: .module)
    ...
    private var headerView: some View {
        ManagerHeaderWithActions(
            title: L("Settings"),
            subtitle: L("Configure your Muwa settings")
        ) {
    ...
    private func showFactoryResetConfirmation() {
        let alert = NSAlert()
        alert.messageText = L("Factory Reset Muwa?")
...
extension ConfigurationView {
    private func installCLI() {
        ...
        if tryInstall(cliURL: cliURL, into: brewBin) {
            ...
            cliInstallMessage = "Installed to \(brewBin.appendingPathComponent(ProductBrand.lowercaseName).path)"
            return
        }

        if tryInstall(cliURL: cliURL, into: usrLocalBin) {
            cliInstallSuccess = true
            cliInstallMessage = "Installed to \(usrLocalBin.appendingPathComponent(ProductBrand.lowercaseName).path)"
            return
        }
        ...
        if tryInstall(cliURL: cliURL, into: userLocalBin) {
            let linkPath = userLocalBin.appendingPathComponent(ProductBrand.lowercaseName).path
    ...
    private func resolveCLIExecutableURL() -> URL? {
        ...
        let helpers = appURL.appendingPathComponent("Contents/Helpers/muwa", isDirectory: false)
        if fm.fileExists(atPath: helpers.path), fm.isExecutableFile(atPath: helpers.path) {
            return helpers
        }

        // 2. Try MacOS folder (alternative embedding)
        let macOS = appURL.appendingPathComponent("Contents/MacOS/muwa", isDirectory: false)
        ...
        let debugMuwaCLI = productsDir.appendingPathComponent("muwa-cli", isDirectory: false)
        if fm.fileExists(atPath: debugMuwaCLI.path), fm.isExecutableFile(atPath: debugMuwaCLI.path) {
            return debugMuwaCLI
        }

        // Check for muwa binary in Products (might be named this in some builds).
        let debugMuwa = productsDir.appendingPathComponent("muwa", isDirectory: false)
        if fm.fileExists(atPath: debugMuwa.path), fm.isExecutableFile(atPath: debugMuwa.path) {
            return debugMuwa
        }

        // Check Release folder
        let releaseDir = productsDir.deletingLastPathComponent().appendingPathComponent("Release")
        let releaseMuwaCLI = releaseDir.appendingPathComponent("muwa-cli", isDirectory: false)
        if fm.fileExists(atPath: releaseMuwaCLI.path), fm.isExecutableFile(atPath: releaseMuwaCLI.path) {
            return releaseMuwaCLI
        }

        let releaseMuwa = releaseDir.appendingPathComponent("muwa", isDirectory: false)
        if fm.fileExists(atPath: releaseMuwa.path), fm.isExecutableFile(atPath: releaseMuwa.path) {
            return releaseMuwa
        }
        ...
        let releaseAppHelpers =
            releaseDir
            .appendingPathComponent("Muwa.app/Contents/Helpers/muwa", isDirectory: false)
        ...
        let releaseAppMacOS =
            releaseDir
            .appendingPathComponent("Muwa.app/Contents/MacOS/muwa", isDirectory: false)
    ...
    private func tryInstall(cliURL: URL, into dir: URL) -> Bool {
        ...
        let linkURL = dir.appendingPathComponent(ProductBrand.lowercaseName)
...
// `Packages/MuwaCore/Views/Settings/Shared/SettingsPrimitives.swift`
