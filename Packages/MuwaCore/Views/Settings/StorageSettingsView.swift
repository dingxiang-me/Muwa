...
public struct StorageSettingsView: View {
    ...
    private var rotateAlertMessage: String {
        if hasExportedBackupThisSession {
            return
                "A new 256-bit key will be generated and every encrypted database + file under ~/.muwa will be re-encrypted against it. The old key is destroyed — backups made under the old key will no longer be readable on this Mac."
        }
        return
            "A new 256-bit key will be generated and every encrypted database + file under ~/.muwa will be re-encrypted against it. The old key is destroyed — backups made under the old key will no longer be readable on this Mac. We strongly recommend exporting a plaintext backup first."
    }
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            DisclosureGroup(isExpanded: $showTechnicalDetails) {
                VStack(alignment: .leading, spacing: 4) {
                    Text("Service: com.muwa.storage", bundle: .module)
        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 12) {
                actionRow(
                    ...
                    buttonLabel: "Export…",
                    subtitle:
                        "Decrypts every artifact under ~/.muwa and writes a plaintext copy to the destination of your choice. Recommended before reinstalling macOS or moving Macs.",
                    isPrimary: true,
    ...
    private func runExport(reason: ExportReason) {
        ...
        Task { @MainActor in
            ...
            let backupDir = dest.appendingPathComponent("muwa-plaintext-backup", isDirectory: true)
            isWorking = true
            errorMessage = nil
            do {
