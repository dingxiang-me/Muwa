    ...
    var body: some View {
        ...
        mainContent
            ...
            .themedAlert(
                ...
                message:
                    "Muwa needs microphone access to transcribe speech. Enable it in System Settings → Privacy & Security → Microphone, then try again.",
...
extension FloatingInputCard {
    ...
    private func scheduleLiveVoicePreencodeIfNeeded(
        ...
        let task = Task.detached(priority: .utility) {
            ...
            print(
                "[Muwa][LiveVoice] preencode_status=\(result.status.rawValue) samples=\(result.sampleCount) sample_rate=\(result.sampleRate) encode_ms=\(result.encodeMs) message=\(result.message ?? "")"
            )
        }
    ...
    private func sendVoiceMessage(_ message: String) {
        ...
        if mediaCapabilities.supportsAudio {
            ...
            print(
                "[Muwa][LiveVoice] snapshot_ms=\(snapshotMs) wav_encode_ms=\(wavEncodeMs) wav_bytes=\(wavBytes) sample_rate=\(voiceSnapshot?.sampleRate ?? 0) duration_ms=\(durationMs)"
            )
        }
