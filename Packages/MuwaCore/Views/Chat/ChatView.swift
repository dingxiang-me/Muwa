...
private struct GenerativeGreetingTrigger: ViewModifier {
    ...
    /// env `MUWA_MOCK_STREAM=1` to enable without editing code) when done.
    enum MockToolStream {
        static let forceEnabled = false
        static var enabled: Bool {
            forceEnabled || ProcessInfo.processInfo.environment["MUWA_MOCK_STREAM"] == "1"
        }
    }
#endif
...
final class ChatSession: ObservableObject {
    ...
    private func rebuildVisibleBlocksImpl() {
        ...
        let displayName = agent?.isBuiltIn == true ? L("Muwa") : (agent?.name ?? L("Muwa"))
    ...
    private func processStreamDeltas(
        ...
        print(
            "[Muwa][UI] Stream consumption completed: \(uiDeltaCount) deltas in \(String(format: "%.2f", totalTime))s, final contentLen=\(currentTurn.contentLength)"
        )
        ...
        currentTask = Task { @MainActor [weak self] in
            ...
            await ChatExecutionContext.$currentAgentId.withValue(turnAgentId) { [self] in
                ...
                rebuildVisibleBlocks()
                    ...
                    // Toggle via `MockToolStream.forceEnabled` (or env MUWA_MOCK_STREAM=1).
                ...
                do {
                    ...
                    func postProcessToolResult(
                        ...
                        print(
                            "[Muwa][Tool] Success: \(inv.toolName) returned \(resultText.count) chars: \(truncatedResult)\(resultText.count > 500 ? "..." : "")"
                        )
                    ...
                    func executeSingleToolCall(
                        ...
                        do {
                            ...
                            print(
                                "[Muwa][Tool] Executing: \(inv.toolName) with args: \(truncatedArgs)\(inv.jsonArguments.count > 200 ? "..." : "")"
                            )
                    ...
                    func executeToolBatch(
                        ...
                        if !approved.isEmpty {
                            print(
                                "[Muwa][Tool] Executing batch of \(approved.count) in parallel: \(approved.map { $0.invocation.toolName }.joined(separator: ", "))"
                            )
                            let sessionIdForTools =
                                self.sessionId?.uuidString ?? "chatwindow-\(ObjectIdentifier(self).hashValue)"
                        ...
                        modelStep: { msgs, attempt in
                            ...
                            do {
                                ...
                                if transientRetries < maxTransientRetries {
                                    transientRetries += 1
                                    print(
                                        "[Muwa] Transient stream error (retry \(transientRetries)/\(maxTransientRetries)): \(error.localizedDescription)"
                                    )
...
struct ChatView: View {
    ...
    var body: some View {
        ...
        chatModeContent
            .themedAlert(
                L("Do you want Muwa to auto speak every reply in this chat?"),
    ...
    private func connectToDiscoveredAgent(_ agent: DiscoveredAgent, token: String, isEphemeral: Bool = true) {
        ...
        let providerId: UUID
        // Reuse an existing Muwa provider that already targets the same agent
        if let existing = manager.configuration.providers.first(where: {
            $0.providerType == .muwa && $0.remoteAgentId == agent.id
        }) {
    ...
    private func connectToRelayAgent(_ relay: PairedRelayAgent) {
        let relayHost = "\(relay.remoteAgentAddress).agent.muwa.ai"
        let manager = RemoteProviderManager.shared
    ...
    /// default Muwa agent, chat-oriented for everything else).
    ...
    private static func verifyServerIdentity(
        ...
        guard let sigBytes = Data(hexEncoded: hex),
            let recovered = try? recoverAddress(
                ...
                signature: sigBytes,
                domainPrefix: "Muwa Signed Pairing Server"
            ),
