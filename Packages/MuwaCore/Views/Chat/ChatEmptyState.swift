...
/// agents share the same `AgentAvatarView` path so the default Muwa
...
/// backing, leaving the frog floating against the chat background.
    ...
    private func encryptionBadge(_ status: RemoteEncryptionStatus) -> some View {
        ...
        return HStack(spacing: 5) {
            ...
            if encrypted {
                ...
                Text("Peer needs a Muwa upgrade for encrypted chat", bundle: .module)
            }
        }
        ...
        .help(
            encrypted
                ? L(
                    "Agent traffic is protected by the Muwa Secure Channel: forward-secret, mutually authenticated end-to-end encryption."
                )
                : L(
                    "This peer runs an older Muwa without the Secure Channel. Agent chat is refused until it upgrades — no plaintext fallback."
                )
        )
    }
    ...
    private var noModelsState: some View {
        VStack(spacing: 14) {
            ...
            VStack(spacing: 8) {
                ...
                Text("Muwa needs an AI to work — either a cloud provider or a local model.", bundle: .module)
                    .font(theme.font(size: CGFloat(theme.bodySize) + 2))
                    .foregroundColor(theme.secondaryText)
                    .multilineTextAlignment(.center)
                    .opacity(hasAppeared ? 1 : 0)
