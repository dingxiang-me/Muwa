...
/// `ai.muwa` subsystem used elsewhere in MuwaCore so the
/// whole app can be filtered with one `log stream --subsystem ai.muwa`.
private let log = Logger(subsystem: "ai.muwa", category: "AppDelegate")
...
public final class AppDelegate: NSObject, NSApplicationDelegate, NSPopoverDelegate {
    ...
    private var keychainDisabledUIPresentationMode: Bool {
        ...
        return env["MUWA_KEYCHAIN_FREE_SHOW_UI"] == "1"
            || env["MUWA_KEYCHAIN_FREE_SHOW_UI"] == "1"
    }
    ...
    public func applicationWillFinishLaunching(_ notification: Notification) {
        ...
        ProcessInfo.processInfo.disableAutomaticTermination(
            "\(ProductBrand.name) local LLM HTTP server (long-running)"
        )
    ...
    public func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        // wires `muwa_provider_add`, `muwa_model_download`, etc. into
        ...
        // App has launched
        NSLog("Muwa server app launched")
        ...
        NSLog(
            "[Muwa] AgentDB adoption: %d/%d agents have dbEnabled=true",
            dbEnabledCount,
            allAgents.count
        )
        ...
        if keychainDisabledTestMode {
            log.warning(
                "Keychain disabled by MUWA_DISABLE_KEYCHAIN_FOR_TESTS=1; stored secrets will not be readable in this process"
            )
        ...
        Task { @MainActor in
            guard StorageKeyManager.shared.hasCachedKey else {
                NSLog("[Muwa] Scheduler disabled: storage key is not already unlocked")
                return
            }
        ...
        Task { @MainActor in
            ...
            if keychainDisabledTestMode && !keychainDisabledUIPresentationMode {
                ProcessInfo.processInfo.disableAutomaticTermination(
                    "\(ProductBrand.name) keychain-free headless live proof server"
                )
            }
    ...
    /// so it behaves like an app modal and recedes when Muwa is deactivated,
    ...
    private func maybePromptForTelemetryConsent() {
        ...
        Task { @MainActor in
            ...
            // recedes when Muwa is deactivated — rather than the toast
            ...
            ThemedAlertCenter.shared.present(
                ThemedAlertRequest(
                    id: requestId,
                    title: "Help improve \(ProductBrand.name)",
                    message: L(
                        "Muwa can send anonymous usage data to help us understand how it's used and improve it."
                    ),
        ...
        Task { @MainActor in
            ...
            await runWithDeadline(seconds: 3) {
                do {
                    ...
                    NSLog("[Muwa] Sandbox stop failed: \(error)")
                }
            }
    ...
    public func applicationWillTerminate(_ notification: Notification) {
        NSLog("Muwa server app terminating")
    ...
    private func installStatusItem() {
        ...
        if let button = item.button {
            if let image = NSImage(named: "muwa") {
                image.size = NSSize(width: 18, height: 18)
                image.isTemplate = true
                button.image = image
            } else {
                button.title = ProductBrand.name
            }
            button.toolTip = L("Muwa Server")
            button.target = self
    ...
    private func updateStatusItemAndMenu() {
        ...
        if let button = statusItem.button {
            ...
            case .stopped:
                tooltip =
                    serverController.isRestarting
                        ? "\(ProductBrand.name) — Restarting…"
                        : "\(ProductBrand.name) — Ready to start"
            case .starting:
                tooltip = "\(ProductBrand.name) — Starting…"
            case .restarting:
                tooltip = "\(ProductBrand.name) — Restarting…"
            case .running:
                tooltip = "\(ProductBrand.name) — Running on port \(serverController.port)"
            case .stopping:
                tooltip = "\(ProductBrand.name) — Stopping…"
            case .error(let message):
                tooltip = "\(ProductBrand.name) — Error: \(message)"
            }
...
extension AppDelegate {
    func applyChatHotkey() {
        ...
        HotKeyManager.shared.register(hotkey: cfg.hotkey) { [weak self] in
            Task { @MainActor in
                // if opening (about to be shown), and clipboard monitoring is enabled, trigger a selection grab before showing Muwa
    ...
    /// `muwa://<addr>?pair=<base64url(invite)>` — incoming agent share link.
    /// `muwa://plugins-install?tool=<plugin_id>` — open Plugins tab on a plugin's detail page.
    /// `muwa://themes-install?hash=<sha256>` — open Themes tab and install a shared theme.
    fileprivate func handleMuwaDeepLink(_ url: URL) {
        Task { @MainActor in
            NSApp.activate(ignoringOtherApps: true)
        ...
        Task { @MainActor in
            if await ModelManager.shared.resolveModelIfMLXCompatible(byRepoId: modelId) == nil {
                ...
                alert.informativeText = L(
                    "Muwa supports MLX-compatible Hugging Face repositories, including MLX, MXFP, JANG, JANGTQ, and TurboQuant artifacts when required files are present."
                )
