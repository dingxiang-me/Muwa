...
//  Muwa
//
//  Manages a single WebSocket tunnel to the muwa-relay service.
...
/// can actually proxy back to the local Muwa server.
struct RelayPublicURLProbe: Sendable {
    ...
    static func makeHealthRequest(baseURL: String) -> URLRequest? {
        ...
        request.setValue("MuwaRelayHealthCheck/1", forHTTPHeaderField: "User-Agent")
        return request
    }
...
public final class RelayTunnelManager: ObservableObject {
    ...
    private static let relayURL = URL(string: "wss://agent.muwa.ai/tunnel/connect")!
    ...
    /// Proxy a relay request frame to the local Muwa server and send result frames
    ...
    private static func buildLocalRequest(
        ...
        if let agentUUID {
            request.setValue(agentUUID, forHTTPHeaderField: "X-Muwa-Agent-Id")
        }
    ...
    private static func sendFrame<T: Encodable>(_ frame: T, via webSocket: URLSessionWebSocketTask?) {
        guard let data = try? JSONEncoder.muwaCanonical().encode(frame),
    ...
    private func obtainMasterKey() async -> Data? {
        await Task.detached(priority: .userInitiated) {
            guard MuwaIdentity.exists() else { return nil }
    ...
    private static func signAgentAuth(
        ...
        let message = "muwa-tunnel:\(address):\(nonce):\(timestamp)"
    ...
    private func sendJSON(_ object: [String: Any]) {
        guard let data = try? JSONSerialization.data(withJSONObject: object, options: .muwaCanonical),
            let str = String(data: data, encoding: .utf8)
        else { return }
