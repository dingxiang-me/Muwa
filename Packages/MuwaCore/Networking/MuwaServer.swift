...
public actor MuwaServer: Sendable {
    ...
    public func start(
        ...
        self.group = group
        self.channel = ch
        print("[Muwa] MuwaServer started on http://\(config.host):\(config.port)")
    }
    ...
    public func stop(gracefully: Bool = true) async -> Bool {
        ...
        if let g = self.group {
            ...
            if completed {
                self.group = nil
                print("[Muwa] MuwaServer stopped")
                return true
            } else {
                print(
                    "[Muwa] MuwaServer graceful shutdown exceeded \(budget)s budget; proceeding (group left to finish)"
                )
                return false
            }
        } else {
    ...
    private static func buildValidator(agentIndex: UInt32?) -> APIKeyValidator {
        ...
        do {
            ...
            let masterAddress = try deriveMuwaId(from: masterKeyData)
            ...
            print("[Muwa] Failed to build validator: \(error). Falling back to empty validator.")
            return .empty
        }
    }
}
...
final class ConnectionLimitHandler: ChannelInboundHandler {
    ...
    func channelActive(context: ChannelHandlerContext) {
        ...
        if admitted {
            ...
            NSLog(
                "[Muwa] Refusing connection — at max concurrent connections (%d)",
