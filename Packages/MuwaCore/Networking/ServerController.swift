...
final class ServerController: ObservableObject {
    ...
    private var serverActor: MuwaServer?
    ...
    func startServer() async {
        ...
        do {
            ...
            print("[Muwa] Starting NIO server on \(bindHost):\(configuration.port)")

            // Ensure any previous instance is shut down
            try await stopServerIfNeeded()

            let server = MuwaServer()
            ...
            print("[Muwa] NIO server started successfully on port \(configuration.port)")
    ...
    func stopServer() async {
        ...
        print("[Muwa] Stopping NIO server...")
        ...
        print("[Muwa] Server stopped successfully")
    }
    ...
    func ensureShutdown() async {
        ...
        print("[Muwa] Ensuring NIO server shutdown before app termination")
        RelayTunnelManager.shared.disconnectAll()
        ...
        if let server = serverActor {
            ...
            if completed {
                ...
                print(
                    "[Muwa] NIO group still draining at quit; keeping serverActor rooted to avoid mid-shutdown dealloc"
                )
            }
        }
        ...
        print("[Muwa] Server shutdown completed")
    }
    ...
    init() {
        ...
        // `~/.muwa/config/`) is intentionally deferred to
    ...
    /// `config/server-runtime.json` in `~/.muwa/` before the app
    /// is fully up.
    ...
    func checkServerHealth() async -> Bool {
        ...
        do {
            ...
            print("[Muwa] Health check failed: \(error)")
            return false
        }
    }
    ...
    private func handleServerError(_ error: Error) {
        print("[Muwa] Failed to start server: \(error)")
        isRunning = false
        ...
        if let group = eventLoopGroup {
            await withCheckedContinuation { (continuation: CheckedContinuation<Void, Never>) in
                group.shutdownGracefully { error in
                    if let error {
                        print("[Muwa] Error shutting down EventLoopGroup: \(error)")
                    }
                    continuation.resume()
                }
            }
