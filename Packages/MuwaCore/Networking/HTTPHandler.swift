...
final class HTTPHandler: ChannelInboundHandler, Sendable {
    ...
    static let relayOriginHeaderName = "X-Muwa-Relay-Origin"
    ...
    func userInboundEventTriggered(context: ChannelHandlerContext, event: Any) {
        ...
        if let idle = event as? IdleStateHandler.IdleStateEvent {
            NSLog("[Muwa] Closing idle connection (state=%@)", String(describing: idle))
            _isChannelActive.value = false
            requestTasks.cancelAll()
        ...
        case .end:
            ...
            if !publicPaths.contains(path) && !isPluginRoute && !isLoopback {
                ...
                if !apiKeyValidator.hasKeys {
                    message = "No access keys configured. Create one in Muwa settings."
                } else {
            ...
            // production HTTP path is fully owned by this handler.)
                ...
                var headers = [("Content-Type", "text/plain; charset=utf-8")]
                headers.append(contentsOf: stateRef.value.corsHeaders)
                let rootBody = "\(ProductBrand.serverName) is running! 🐸"
                sendResponse(
                    context: context,
                    version: head.version,
                    status: .ok,
    ...
    private func handleCacheStatsEndpoint(
        ...
        runRequestTask(priority: .userInitiated) {
            ...
            let memorySafetyPlan = runtimeSettings.resolvedMemorySafetyPlan(
                baseLoadConfiguration: .muwaProduction,
                host: memoryStatus
            )
            ...
            let data = try? JSONSerialization.data(withJSONObject: obj, options: .muwaCanonical)
    ...
    private func handleGenerationSettingsEndpoint(
        ...
        runRequestTask(priority: .userInitiated) {
            ...
            let obj: [String: Any] = [
                ...
                "timestamp": Date().ISO8601Format(),
                "source":
                    "last effective settings resolved by Muwa; stage indicates whether they are pending preload or submitted to vmlx BatchEngine",
    ...
    private func handlePluginRoute(
        ...
        // Accept either the `X-Muwa-Agent-Id` header (preferred for SDK
        // and tunnel callers) or the `muwa_agent` query parameter (for
        // browser-launched web UIs that cannot set custom headers on the
        // top-level navigation). The injected `window.__muwa` script
        ...
        let queryAgent: String? = {
            ...
            for pair in q.split(separator: "&") {
                ...
                if kv.count == 2, kv[0] == "muwa_agent" {
                    return kv[1].removingPercentEncoding ?? kv[1]
                }
            }
            return nil
        }()
        let agentIdStr = headersDict["x-muwa-agent-id"] ?? queryAgent
        guard let agentIdStr, let agentUUID = UUID(uuidString: agentIdStr) else {
            sendPluginError(
                ...
                status: .unauthorized,
                message:
                    "Plugin routes require an agent context (X-Muwa-Agent-Id header or muwa_agent query parameter)",
                corsHeaders: corsHeaders,
                startTime: startTime,
                method: method,
                path: path,
        ...
        let task = Task {
            ...
            let queryParams = MuwaHTTPRequest.parseQueryParams(from: uri)
            ...
            let encoder = JSONEncoder.muwaCanonical()
            ...
            do {
                ...
                guard let responseData = responseJSON.data(using: .utf8),
                    let response = try? JSONDecoder().decode(MuwaHTTPResponse.self, from: responseData)
                else {
                    return self.sendPluginErrorFromTask(
                        loop: loop,
                        ctxBound: ctxBound,
    ...
    private func serveStaticFile(
        ...
        if ext == "html" || ext == "htm", var html = String(data: fileData, encoding: .utf8) {
            Self.injectMuwaContext(into: &html, pluginId: pluginId, apiMount: apiMount, agentId: agentId)
            sendPluginResponse(
                loop: loop,
                ctxBound: ctxBound,
                version: version,
    ...
    /// Injects the `window.__muwa` context object into an HTML string before `</head>`.
    ...
    /// can attach it as the `X-Muwa-Agent-Id` header without re-entering
    /// the URL bar.
    private static func injectMuwaContext(
        ...
        let script = """
            ...
            window.__muwa = {
              ...
              \(agentField)
              // Helper that wraps fetch with the X-Muwa-Agent-Id header
              // so the page never accidentally drops the agent context.
              fetch: function(input, init) {
                init = init || {};
                init.headers = new Headers(init.headers || {});
                if (window.__muwa.agentId && !init.headers.has("X-Muwa-Agent-Id")) {
                  init.headers.set("X-Muwa-Agent-Id", window.__muwa.agentId);
                }
    ...
    private static func loadDevProxyURL(for pluginId: String) -> String? {
        let configFile = MuwaPaths.config().appendingPathComponent("dev-proxy.json")
    ...
    func errorCaught(context: ChannelHandlerContext, error: Error) {
        ...
        print("[Muwa][NIO] errorCaught: \(error)")
        requestTasks.cancelAll()
        context.close(promise: nil)
    }
    ...
    private func handleSecureSessionEndpoint(
        ...
        runRequestTask(priority: .userInitiated) {
            ...
            let json =
                (try? JSONEncoder.muwaCanonical().encode(result.serverHello)).map {
    ...
    private func sendSecureChannelUpgradeRequiredIfNeeded(
        ...
        headers.append(contentsOf: stateRef.value.corsHeaders)
        let body =
            #"{"error":{"code":"secure_channel_required","message":"This peer requires end-to-end encryption for agent requests. Upgrade Muwa to a version that supports the secure channel.","type":"upgrade_required"}}"#
        sendResponse(
            context: context,
            version: head.version,
        ...
        runRequestTask(priority: .userInitiated) {
            ...
            guard let sigBytes = Data(hexEncoded: hexSig),
                let recovered = try? recoverAddress(
                    ...
                    signature: sigBytes,
                    domainPrefix: "Muwa Signed Pairing"
                ),
            ...
            let json =
                (try? JSONEncoder.muwaCanonical().encode(response)).map { String(decoding: $0, as: UTF8.self) }
                ?? #"{"error":"Encoding failed"}"#
            ...
            let redactedJson =
                (try? JSONEncoder.muwaCanonical().encode(redactedResponse)).map {
                    String(decoding: $0, as: UTF8.self)
                }
        ...
        runRequestTask(priority: .userInitiated) {
            ...
            do {
                ...
                func responseBody(apiKey: String, sealed: PairingKeyEnvelope.Sealed?) -> String {
                    ...
                    return (try? JSONEncoder.muwaCanonical().encode(body))
    ...
    private func handleGetAgentEndpoint(
        ...
        runRequestTask(priority: .userInitiated) {
            ...
            let json =
                (try? JSONEncoder.muwaCanonical().encode(item)).map { String(decoding: $0, as: UTF8.self) } ?? "{}"
    ...
    private func handleAgentRunEndpoint(
        ...
        // to reach the built-in agent so the App Intents "Ask Muwa" surface
        ...
        runRequestTask(priority: .userInitiated) {
            ...
            // API is stateless (no Muwa session id), so we cannot reuse
    ...
    private func handleDispatchEndpoint(
        ...
        // the built-in agent so App Intents "Run Muwa Agent" / "Ask Muwa"
        ...
        runRequestTask(priority: .userInitiated) {
            ...
            if let handle {
                ...
                responseBody =
                    (try? JSONSerialization.data(withJSONObject: resp, options: .muwaCanonical))
        ...
        runRequestTask(priority: .userInitiated) {
            do {
                ...
                if ollamaFormat {
                    ...
                    json =
                        (try? JSONEncoder.muwaCanonical().encode(response)).map {
                            String(decoding: $0, as: UTF8.self)
                        } ?? "{}"
                } else {
    ...
    private func handleChatCompletions(
        ...
        let rawMemoryAgentId = head.headers.first(name: "X-Muwa-Agent-Id")
    ...
    private static func ollamaGenerateJSON(model: String, response: String, done: Bool) -> String {
        ...
        guard let data = try? JSONSerialization.data(withJSONObject: object, options: .muwaCanonical) else {
            return #"{"done":true}"#
        }
    ...
    private func handleTagsEndpoint(
        ...
        runRequestTask(priority: .userInitiated) {
            ...
            let json =
                (try? JSONEncoder.muwaCanonical().encode(payload)).map { String(decoding: $0, as: UTF8.self) }
                ?? "{}"
        ...
        runRequestTask(priority: .userInitiated) {
            ...
            if modelName.lowercased() == "foundation" || modelName.lowercased() == "default" {
                if FoundationModelService.isDefaultModelAvailable() {
                    ...
                    let jsonData =
                        (try? JSONSerialization.data(withJSONObject: response, options: .muwaCanonical))
                        ?? Data("{}".utf8)
            ...
            let jsonData = (try? JSONEncoder.muwaCanonical().encode(response)) ?? Data("{}".utf8)
            let json = String(decoding: jsonData, as: UTF8.self)
    ...
    private func handleMCPListTools(
        ...
        runRequestTask(priority: .userInitiated) {
            ...
            let data =
                (try? JSONSerialization.data(withJSONObject: payload, options: .muwaCanonical))
                ?? Data("{}".utf8)
    ...
    private func handleMCPCallTool(
        ...
        if let header = head.headers.first(name: "X-Muwa-Agent-Id") {
            let agentUUID = UUID(uuidString: header)
            let rejection: (code: String, message: String)? = {
                if agentUUID == nil {
                    return (
                        "invalid_agent",
                        "X-Muwa-Agent-Id must be a valid agent UUID."
                    )
                }
        ...
        struct AnyCodable: Codable {
            ...
            func encode(to encoder: Encoder) throws {
                ...
                case let arr as [Any]:
                    let enc = try JSONSerialization.data(withJSONObject: arr, options: .muwaCanonical)
                    try container.encode(String(decoding: enc, as: UTF8.self))
                case let dict as [String: Any]:
                    let enc = try JSONSerialization.data(withJSONObject: dict, options: .muwaCanonical)
                    try container.encode(String(decoding: enc, as: UTF8.self))
                default:
        ...
        let argsJSON: String = {
            if let a = req.arguments?.value,
                let d = try? JSONSerialization.data(withJSONObject: a, options: .muwaCanonical)
            {
        ...
        if ToolRegistry.externallyDeniedToolNames.contains(req.name) {
            let message =
                "'\(req.name)' is not available to external callers. Folder write and shell tools can only run from the Muwa app."
    ...
    private func handleAnthropicMessages(
        ...
        guard let anthropicReq = try? JSONDecoder().decode(AnthropicMessagesRequest.self, from: data) else {
            ...
            let errorJson =
                (try? JSONEncoder.muwaCanonical().encode(error)).map { String(decoding: $0, as: UTF8.self) }
    ...
    private func handleAnthropicMessagesNonStreaming(
        ...
        runRequestTask(priority: .userInitiated) {
            ...
            do {
                ...
                let json = try JSONEncoder.muwaCanonical().encode(anthropicResp)
                var headers: [(String, String)] = [("Content-Type", "application/json")]
                headers.append(contentsOf: cors)
                let headersCopy = headers
                ...
                let errorJson =
                    (try? JSONEncoder.muwaCanonical().encode(errorResp))
                    .map { String(decoding: $0, as: UTF8.self) }
    ...
    private func handleAudioTranscriptions(
        ...
        runRequestTask(priority: .userInitiated) {
            do {
                ...
                let audioURL = tempDir.appendingPathComponent("muwa_transcription_\(UUID().uuidString).wav")
                ...
                if responseFormat == "text" {
                    ...
                    let jsonData = try JSONSerialization.data(withJSONObject: response, options: .muwaCanonical)
                    responseBody = String(decoding: jsonData, as: UTF8.self)
                } else {
                    // Default JSON format
                    let response = ["text": result.text]
                    let jsonData = try JSONEncoder.muwaCanonical().encode(response)
                    responseBody = String(decoding: jsonData, as: UTF8.self)
                }
    ...
    private static func openResponsesNonStreamingBody(
        ...
        return (try? JSONEncoder.muwaCanonical().encode(resp))
            .map { String(decoding: $0, as: UTF8.self) } ?? "{}"
    }
    ...
    private func handleOpenResponsesNonStreaming(
        ...
        runRequestTask(priority: .userInitiated) {
            ...
            do {
                ...
                let json = try JSONEncoder.muwaCanonical().encode(openResponsesResp)
                var headers: [(String, String)] = [("Content-Type", "application/json")]
                headers.append(contentsOf: cors)
                let headersCopy = headers
                let body = String(decoding: json, as: UTF8.self)
    ...
    private static func chatCompletionResponseBody(_ response: ChatCompletionResponse) throws -> String {
        let encoded = try JSONEncoder.muwaCanonical().encode(response)
    ...
    private static func canonicalToolArgumentsJSON(_ json: String, schema: JSONValue? = nil) -> String {
        ...
        guard JSONSerialization.isValidJSONObject(coerced),
            let encoded = try? JSONSerialization.data(withJSONObject: coerced, options: .muwaCanonical),
