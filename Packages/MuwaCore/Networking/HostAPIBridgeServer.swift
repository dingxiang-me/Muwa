...
//  relayed through vsock. The muwa-host CLI inside the container talks to
...
private final class HostAPIBridgeHandler: ChannelInboundHandler, RemovableChannelHandler, @unchecked Sendable {
    ...
    private func handleRequest(context: ChannelHandlerContext, head: HTTPRequestHead, body: String) {
        ...
        // ignore X-Muwa-User even if present — trusting it would let any
        // sandboxed code claim any agent.
        let bearerToken = Self.extractBearerToken(headers: head.headers)
        let pluginName = head.headers["X-Muwa-Plugin"].first
    ...
    private func handleSecrets(
        ...
        guard let pluginName = pluginName else {
            return .error(400, "X-Muwa-Plugin header required")
        }
    ...
    private func handleConfig(
        ...
        let configDir = MuwaPaths.pluginDataDirectory(for: pluginName)
        ...
        if method == .GET {
            ...
            if let parsed = parseJSON(body), let value = parsed["value"] as? String {
                MuwaPaths.ensureExistsSilent(configDir)
    ...
    private func handleEvents(
        ...
        if let payloadDict = payload {
            if let data = try? JSONSerialization.data(withJSONObject: payloadDict, options: .muwaCanonical),
    ...
    private func pluginCreateResponseBody(
        ...
        guard let data = try? JSONSerialization.data(withJSONObject: payload, options: .muwaCanonical),
            let str = String(data: data, encoding: .utf8)
        else {
