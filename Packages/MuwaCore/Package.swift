...
let package = Package(
    ...
    platforms: [.macOS(.v15)],
    products: [
        .library(name: "MuwaCore", targets: ["MuwaCore"])
    ],
    dependencies: [
        ...
        // MTP, and media-runtime surfaces Muwa previously pulled from
        ...
        // calls that Muwa's `TTSService` doesn't pass. Pinning to the
        // last working version until Muwa catches up. Bumping requires
        // a paired Muwa-side TTSService update.
    ...
    targets: [
        ...
        // contract. MuwaCore links this *instead of* Apple's
        ...
        //          `#ifndef MUWA_OMIT_FTS5_HEADERS` (search for
        //          MUWA LOCAL MODIFICATION inside that file).
        //       2. `include/MuwaSQLCipher.h` defines
        //          `MUWA_OMIT_FTS5_HEADERS` before including
        //          sqlite3.h so Swift's Clang module import sees the
        //          hidden extension API.
        //       3. The `cSettings` `.define("MUWA_OMIT_FTS5_HEADERS")`
        ...
        //     we only hide the C-extension API, which Muwa
        //     doesn't use.
        ...
        //     adopts that SQLite version. Muwa does not compile
