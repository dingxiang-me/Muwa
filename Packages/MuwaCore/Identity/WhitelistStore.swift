...
public final class WhitelistStore: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "com.muwa.whitelist", attributes: .concurrent)
    private var masterAddresses: Set<String> = []
    private var agentAddresses: [String: Set<String>] = [:]

    private static let keychainService = "com.muwa.whitelist"
    ...
    public func addMaster(address: MuwaID) {
    ...
    public func removeMaster(address: MuwaID) {
        queue.sync(flags: .barrier) {
    ...
    public func masterWhitelist() -> Set<MuwaID> {
        queue.sync { masterAddresses }
    }

    // MARK: - Per-Agent Overrides

    public func addAgent(address: MuwaID, forAgent agentAddress: MuwaID) {
        queue.sync(flags: .barrier) {
    ...
    public func removeAgent(address: MuwaID, forAgent agentAddress: MuwaID) {
        queue.sync(flags: .barrier) {
            let key = agentAddress.lowercased()
    ...
    public func agentWhitelist(forAgent agentAddress: MuwaID) -> Set<MuwaID> {
