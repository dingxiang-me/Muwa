...
//  Muwa's SQLite databases (via SQLCipher), VecturaKit indexes, JSON
...
//  - The HKDF salt is stored alongside in plaintext (`~/.muwa/.storage-key.salt`);
...
public final class StorageKeyManager: @unchecked Sendable {
    ...
    static let service = "com.muwa.storage"
    ...
    static let hkdfInfo = Data("muwa-storage-v1".utf8)

    /// Filename for the persisted salt (lives next to the encrypted
    /// artifacts so it travels with `~/.muwa/`). Without the master
    ...
    private let log = Logger(subsystem: "ai.muwa", category: "storage.key")

    private var lock = os_unfair_lock_s()
    private var cachedKey: SymmetricKey?
    private var cachedReadFailureStatus: OSStatus?
    private let keychainQueue = DispatchQueue(label: "ai.muwa.storage-key.keychain")
    ...
    private func provisionedMarkerFile() -> URL {
        MuwaPaths.root().appendingPathComponent(Self.provisionedMarkerFilename)
    }
    ...
    private func markProvisioned() {
        ...
        MuwaPaths.ensureExistsSilent(MuwaPaths.root())
    ...
    private func encryptedStorageExists() -> Bool {
        ...
        let encryptedArtifacts = [
            MuwaPaths.chatHistoryDatabaseFile(),
            MuwaPaths.memoryDatabaseFile(),
            MuwaPaths.methodsDatabaseFile(),
            MuwaPaths.toolIndexDatabaseFile(),
            MuwaPaths.workDatabaseFile(),
        ]
    ...
    private func generateInMemoryKey() throws -> SymmetricKey {
        ...
        let keyBytes = Data(raw)
        for i in raw.indices { raw[i] = 0 }
        log.info("Storage key generated in-memory for MUWA_DISABLE_KEYCHAIN_FOR_TESTS")
        return SymmetricKey(data: keyBytes)
    }
    ...
    private func saltFile() -> URL {
        MuwaPaths.root().appendingPathComponent(Self.saltFilename)
    }
    ...
    private func persistKeychain(data: Data) throws {
        ...
        try persistItem(account: Self.keyAccount, data: data, label: "Muwa Storage Encryption Key")
    }
    ...
    private func persistKeychainSalt(_ data: Data) throws {
        if Self.disablesKeychainForProcess { return }
        try persistItem(account: Self.saltAccount, data: data, label: "Muwa Storage Key Derivation Salt")
    }
