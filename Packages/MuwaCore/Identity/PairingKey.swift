...
//  Derived from the Master Key via HMAC-SHA512 with domain "muwa-pair-v1".
...
public struct PairingKey: Sendable {
    private static let domain = Data("muwa-pair-v1".utf8)
    ...
    public static func deriveAddress(masterKey: Data) throws -> MuwaID {
        let childKey = derive(masterKey: masterKey)
        return try deriveMuwaId(from: childKey)
    }
