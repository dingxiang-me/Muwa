...
//  Muwa
//
//  Data types for the Muwa Identity system.
//

import Foundation

/// Checksummed hex address: "0x742d35Cc6634C0532925a3b844Bc9e7595f2bD18"
public typealias MuwaID = String
...
public enum IdentityDefaultsKey {
    ...
    /// `MuwaIdentity.wipe()` cleanup keeps clearing it on reset for
...
public struct AccessKeyPayload: Codable, Sendable {
    public let aud: MuwaID
    public let cnt: UInt64
    public let exp: Int?
    public let iat: Int
    public let iss: MuwaID
...
public struct AgentInfo: Codable, Identifiable, Sendable {
    ...
    public let address: MuwaID
    public let label: String
    public let createdAt: Date

    public init(index: UInt32, address: MuwaID, label: String, createdAt: Date = Date()) {
        self.index = index
...
public struct RevocationSnapshot: Sendable {
    ...
    public let counterThresholds: [MuwaID: UInt64]

    public init(revokedKeys: Set<String>, counterThresholds: [MuwaID: UInt64]) {
        self.revokedKeys = revokedKeys
        self.counterThresholds = counterThresholds
    }

    /// Composite key for individual revocation lookups.
    public static func revocationKey(address: MuwaID, nonce: String) -> String {
        "\(address.lowercased()):\(nonce)"
    }

    public func isRevoked(address: MuwaID, nonce: String, cnt: UInt64) -> Bool {
...
public enum AccessKeyValidationResult: Sendable {
    ...
    case valid(issuer: MuwaID, audience: MuwaID)
