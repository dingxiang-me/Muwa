...
public final class APIKeyManager: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "com.muwa.api-keys", attributes: .concurrent)
    private var keys: [AccessKeyInfo] = []
    private var didLoadFromKeychain = false

    private static let keychainService = "com.muwa.access-keys"
    ...
    public func generate(
        ...
        let signerAddress: MuwaID
        let audienceAddress: MuwaID
    ...
    public func revokeAll(forAddress address: MuwaID) {
        ensureLoadedFromKeychain()
    ...
    public func listKeys(forAudience audience: MuwaID) -> [AccessKeyInfo] {
