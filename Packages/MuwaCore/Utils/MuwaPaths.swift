...
//  Muwa
//
//  Centralized path management for all Muwa app data.
...
public enum MuwaPaths {
    ...
    private static let defaultRoot: URL = {
        ...
        return fm.homeDirectoryForCurrentUser.appendingPathComponent(ProductBrand.rootDirectoryName, isDirectory: true)
    }()
    ...
    public static func root() -> URL {
        ...
        if let envRoot = env["MUWA_TEST_ROOT"],
    ...
    /// Per-agent feature directory: `~/.muwa/agents/<uuid>/`.
    ...
    /// Per-agent SQLite database file: `~/.muwa/agents/<uuid>/db.sqlite`.
    ...
    /// Per-agent migrations directory: `~/.muwa/agents/<uuid>/migrations/`.
    ...
    /// Per-agent saved-views directory: `~/.muwa/agents/<uuid>/views/`.
    ...
    /// Per-agent run-trace directory: `~/.muwa/agents/<uuid>/runs/`.
    ...
    /// an agent: `~/.muwa/agents/<uuid>/installed-packages.json`. Seeded
    ...
    /// Cross-agent scheduler database: `~/.muwa/scheduler.sqlite`.
    ...
    /// Path to the init filesystem image: `~/.muwa/container/initfs.ext4`
    ...
    /// Root directory for all shared artifacts: `~/.muwa/artifacts/`
    public static func artifactsDir() -> URL {
        root().appendingPathComponent("artifacts", isDirectory: true)
    }

    /// Per-context artifacts directory: `~/.muwa/artifacts/{contextId}/`
    ...
    /// `~/.muwa/claude-plugins/userconfig/`.
    ...
    /// `~/.muwa/claude-plugins/cache/`. Currently a placeholder so the
    ...
    /// `~/.muwa/claude-plugins/manifests/<safeId>.json`.
    ...
    /// `~/.muwa/claude-plugins/userconfig/<safeId>.json`.
    ...
    /// `~/.muwa/claude-plugins/data/<safeId>/`. Created lazily on first
    ...
    /// `~/.muwa/claude-plugins/cache/<safeId>/`.
