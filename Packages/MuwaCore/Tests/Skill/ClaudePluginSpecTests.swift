...
//     `MUWA_TEST_ROOT` so writes never touch `~/.muwa`.
//
...
/// mutate the process-wide `MUWA_TEST_ROOT` env var via `setenv`
/// (see `isolatedRoot(label:)`). Swift Testing runs `@Test` cases in
/// parallel by default, which lets concurrent tests trample each
/// other's value. Tests that also mutate `MuwaPaths.overrideRoot`
...
struct ClaudePluginSpecTests {
    ...
    /// inside `/tmp/muwa-claude-plugin-tests/*` so the suite never
    /// mutates `~/.muwa`.
    ///
    /// Drives isolation through `MuwaPaths.overrideRoot` (the typed
    /// test hook) rather than `setenv("MUWA_TEST_ROOT", …)`. Under
    ...
    /// to `MuwaPaths.root()` and two serialized tests wound up
    ...
    private static func isolatedRoot(label: String) -> (URL, () -> Void) {
        ...
        let root = URL(fileURLWithPath: "/tmp/muwa-claude-plugin-tests")
            .appendingPathComponent(unique, isDirectory: true)
        try? FileManager.default.createDirectory(
            at: root,
            withIntermediateDirectories: true
        )
        let previousOverride = MuwaPaths.overrideRoot
        let previousEnv = ProcessInfo.processInfo.environment["MUWA_TEST_ROOT"]
        MuwaPaths.overrideRoot = root
        setenv("MUWA_TEST_ROOT", root.path, 1)
        let teardown = {
            try? FileManager.default.removeItem(at: root)
            MuwaPaths.overrideRoot = previousOverride
            if let previousEnv {
                setenv("MUWA_TEST_ROOT", previousEnv, 1)
            } else {
                unsetenv("MUWA_TEST_ROOT")
            }
        }
    ...
    /// The catalog ships in the MuwaCore bundle and must parse. A missing
    ...
    /// agents, commands, or MCP) ships nothing Muwa can install, so the
    ...
    /// to the per-plugin paths owned by `MuwaPaths` so MCP servers can
    ...
    @Test func expandsPluginRootAndDataPaths() async {
        await Self.withIsolatedRoot(label: "expander-paths") { _ in
            ...
            #expect(expanded.contains(MuwaPaths.claudePluginCacheDir(for: pluginId).path))
            #expect(expanded.contains(MuwaPaths.claudePluginDataDir(for: pluginId).path))
    ...
    @Test func manifestStoreLoadsLegacySnapshotWithoutInstallOutcome() async {
        await Self.withIsolatedRoot(label: "manifest-store-legacy-outcome") { _ in
            ...
            let json = #"""
                ...
                """#
            let file = MuwaPaths.claudePluginManifestFile(for: pluginId)
            MuwaPaths.ensureExistsSilent(file.deletingLastPathComponent())
    ...
    @Test func manifestStoreAllSkipsCorruptFiles() async {
        await Self.withIsolatedRoot(label: "manifest-store-all") { _ in
            ...
            let dir = MuwaPaths.claudePluginsManifestsDir()
        ...
        await Self.withIsolatedRoot(label: "manifest-store-del") { _ in
            ...
            #expect(
                FileManager.default.fileExists(
                    atPath: MuwaPaths.claudePluginManifestFile(for: pluginId).path
                )
            )
            #expect(
                FileManager.default.fileExists(
                    atPath: MuwaPaths.claudePluginUserConfigFile(for: pluginId).path
                )
            )
            #expect(
                FileManager.default.fileExists(
                    atPath: MuwaPaths.claudePluginDataDir(for: pluginId).path
                )
            )
    ...
    @Test func safePluginIdReplacesUnsafeCharacters() {
        let id = MuwaPaths.claudePluginSafeId("github:owner/repo/plugin-name")
