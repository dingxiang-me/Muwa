...
//  enforcement. Manifest declares `min_muwa` and `min_macos`; this
...
struct PluginCompatibilityEnforcementTests {
    ...
    @Test func absentConstraintsPassUnconditionally() {
        ...
        #expect(result == nil, "no min_muwa / min_macos -> never gate")
    }
    ...
    @Test func emptyStringConstraintsArePassesNotErrors() {
        ...
        let manifest = makeManifest(minMuwa: "", minMacos: "")
        let result = PluginManager.compatibilityFailure(
            manifest: manifest,
            hostVersion: "1.0.0",
            osVersion: os(14, 0, 0)
    ...
    @Test func hostOlderThanMinMuwaFails() throws {
        let manifest = makeManifest(minMuwa: "0.20.0")
        let result = PluginManager.compatibilityFailure(
            manifest: manifest,
            hostVersion: "0.18.13",
            osVersion: os(14, 0, 0)
        ...
        #expect(err.message.contains("requires Muwa"))
        #expect(err.message.contains("0.20.0"))
        #expect(err.message.contains("0.18.13"))
    }

    @Test func hostExactlyMinMuwaPasses() {
    ...
    @Test func hostNewerThanMinMuwaPasses() {
        let manifest = makeManifest(minMuwa: "0.18.0")
        let result = PluginManager.compatibilityFailure(
            manifest: manifest,
            hostVersion: "0.20.5",
            osVersion: os(14, 0, 0)
    ...
    @Test func unparseableMinMuwaIsTreatedAsNoConstraint() {
        // We don't want to brick a plugin over a typo. Logged but
        // doesn't block the load.
        let manifest = makeManifest(minMuwa: "not-a-version")
        let result = PluginManager.compatibilityFailure(
            manifest: manifest,
            hostVersion: "0.1.0",
            osVersion: os(14, 0, 0)
    ...
    @Test func unknownHostVersionFailsOpenWithWarning() {
        ...
        // `min_muwa`. The current policy is fail-open with a
        // one-shot warning so dev iteration isn't blocked.
        let manifest = makeManifest(minMuwa: "0.5.0")
    ...
    @Test func twoComponentHostVersionParsesAsZeroPatched() {
        ...
        let manifest = makeManifest(minMuwa: "1.0.0")
        let result = PluginManager.compatibilityFailure(
            manifest: manifest,
            hostVersion: "1.0",
            osVersion: os(14, 0, 0)
    ...
    @Test func singleComponentHostVersionParsesAsZeroPaddedTwice() {
        ...
        let manifest = makeManifest(minMuwa: "1.5.0")
        let result = PluginManager.compatibilityFailure(
            manifest: manifest,
            hostVersion: "2",
            osVersion: os(14, 0, 0)
    ...
    @Test func bothConstraintsMustPass() throws {
        // min_muwa passes, min_macos fails → error reports macOS.
        let manifest = makeManifest(minMuwa: "0.1.0", minMacos: "20.0")
        let result = PluginManager.compatibilityFailure(
            manifest: manifest,
            hostVersion: "0.20.0",
            osVersion: os(14, 0, 0)
    ...
    @Test func minMuwaReportedFirstWhenBothFail() throws {
        // Implementation checks `min_muwa` before `min_macos`, so
        // the host-version mismatch wins the error message. Pin that
        // ordering so a future refactor doesn't silently swap them.
        let manifest = makeManifest(minMuwa: "9.0.0", minMacos: "99.0")
        let result = PluginManager.compatibilityFailure(
            manifest: manifest,
            hostVersion: "0.18.0",
            osVersion: os(14, 0, 0)
        ...
        let err = try #require(result)
        #expect(err.message.contains("Muwa"))
        #expect(!err.message.contains("macOS"))
    }
}
