...
struct PluginCrashLoopGuardTests {
    ...
    /// `MuwaRepository`, NOT `MuwaPaths`) so the override has to
    /// hit `ToolsPaths.overrideRoot` for marker / quarantine files to
    /// land inside the temp dir. `MuwaPaths.overrideRoot` is set
    /// too so any sibling helper that reads via `MuwaPaths.tools()`
    /// stays in lock-step.
    private func withTempToolsRoot<T: Sendable>(
        ...
        try await StoragePathsTestLock.shared.run {
            let tmp = FileManager.default.temporaryDirectory
                .appendingPathComponent("muwa-crashloop-\(UUID().uuidString)")
    ...
    func staleMarkerPromotesToQuarantineOnNextScan() async throws {
        try await withTempToolsRoot { toolsRoot in
            try writeMarker(pluginId: "ai.muwa.guarded-test", in: toolsRoot)
            #expect(markerExists(in: toolsRoot))

            _ = PluginManager.toolsDirectoryURLsWithFailures()

            #expect(!markerExists(in: toolsRoot))
            #expect(PluginManager.quarantinedPluginIds().contains("ai.muwa.guarded-test"))
        }
    }
    ...
    func successfulScanDoesNotReQuarantine() async throws {
        try await withTempToolsRoot { toolsRoot in
            try writeMarker(pluginId: "ai.muwa.first", in: toolsRoot)
            _ = PluginManager.toolsDirectoryURLsWithFailures()
            #expect(PluginManager.quarantinedPluginIds() == ["ai.muwa.first"])
    ...
    func removeFromQuarantineDropsOnlyTheNamedId() async throws {
        try await withTempToolsRoot { toolsRoot in
            try writeQuarantine(
                ["ai.muwa.alpha", "ai.muwa.beta", "ai.muwa.gamma"],
                in: toolsRoot
            )
            #expect(PluginManager.quarantinedPluginIds().count == 3)

            PluginManager.removeFromQuarantine("ai.muwa.beta")

            let remaining = PluginManager.quarantinedPluginIds()
            #expect(remaining == ["ai.muwa.alpha", "ai.muwa.gamma"])
        }
    }
    ...
    /// surprising during manual inspection of `~/.muwa/Tools/`.
    @Test
    func removeFromQuarantineDeletesFileWhenEmptied() async throws {
        try await withTempToolsRoot { toolsRoot in
            try writeQuarantine(["ai.muwa.only"], in: toolsRoot)

            PluginManager.removeFromQuarantine("ai.muwa.only")
    ...
    func removeFromQuarantineIgnoresUnknownId() async throws {
        try await withTempToolsRoot { toolsRoot in
            try writeQuarantine(["ai.muwa.alpha"], in: toolsRoot)

            PluginManager.removeFromQuarantine("ai.muwa.unknown")

            #expect(PluginManager.quarantinedPluginIds() == ["ai.muwa.alpha"])
        }
    }
    ...
    func removeFromQuarantineWipesStaleLoadingMarker() async throws {
        try await withTempToolsRoot { toolsRoot in
            try writeQuarantine(["ai.muwa.target"], in: toolsRoot)
            try writeMarker(pluginId: "ai.muwa.target", in: toolsRoot)

            PluginManager.removeFromQuarantine("ai.muwa.target")
    ...
    func failedPluginCarriesLastKnownManifestWhenProvided() {
        let manifest = PluginManifest(
            plugin_id: "ai.muwa.test.manifest",
            description: "test",
        ...
        let failed = PluginManager.FailedPlugin(
            pluginId: "ai.muwa.test.manifest",
            error: "boom",
            lastKnownManifest: manifest
        )

        #expect(failed.lastKnownManifest?.plugin_id == "ai.muwa.test.manifest")
