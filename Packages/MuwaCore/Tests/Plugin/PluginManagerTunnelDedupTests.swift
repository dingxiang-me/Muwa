...
struct PluginManagerTunnelDedupTests {
    ...
    @Test func samePushedUrl_skips() {
        let agent = UUID()
        let url = "https://0xabc.agent.muwa.ai"
    ...
    @Test func differentUrl_pushes() {
        let agent = UUID()
        let cache: [String: [UUID: String?]] = [
            pluginId: [agent: "https://old.agent.muwa.ai"]
        ]
        #expect(
            PluginManager.shouldPushTunnelURL(
                url: "https://new.agent.muwa.ai",
                pluginId: pluginId,
                agentId: agent,
                cache: cache
            )
    ...
    @Test func tunnelGoesDown_pushes() {
        let agent = UUID()
        let cache: [String: [UUID: String?]] = [
            pluginId: [agent: "https://0xabc.agent.muwa.ai"]
        ]
    ...
    @Test func staysDown_skipsAfterFirstDownPush() {
        ...
        var cache: [String: [UUID: String?]] = [
            pluginId: [agent: "https://x.agent.muwa.ai"]
        ]
