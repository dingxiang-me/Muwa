...
struct PluginQueryParamsTests {
    ...
    @Test func noQueryString() {
        let params = MuwaHTTPRequest.parseQueryParams(from: "/path")
    ...
    @Test func singleParam() {
        let params = MuwaHTTPRequest.parseQueryParams(from: "/path?key=value")
        #expect(params == ["key": "value"])
    }

    @Test func multipleParams() {
        let params = MuwaHTTPRequest.parseQueryParams(from: "/path?a=1&b=2&c=3")
        #expect(params["a"] == "1")
        #expect(params["b"] == "2")
        #expect(params["c"] == "3")
    }
    ...
    @Test func percentEncoded() {
        let params = MuwaHTTPRequest.parseQueryParams(from: "/path?name=hello%20world&code=abc%3D123")
        #expect(params["name"] == "hello world")
        #expect(params["code"] == "abc=123")
    }

    @Test func paramWithoutValue() {
        let params = MuwaHTTPRequest.parseQueryParams(from: "/path?flag")
        #expect(params["flag"] == "")
    }

    @Test func paramWithEmptyValue() {
        let params = MuwaHTTPRequest.parseQueryParams(from: "/path?key=")
        #expect(params["key"] == "")
    }

    @Test func valueWithEquals() {
        let params = MuwaHTTPRequest.parseQueryParams(from: "/path?data=a=b=c")
