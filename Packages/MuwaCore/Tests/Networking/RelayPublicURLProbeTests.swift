...
struct RelayPublicURLProbeTests {
    @Test func healthRequestTargetsPublicHealthEndpoint() throws {
        let request = try #require(
            RelayPublicURLProbe.makeHealthRequest(baseURL: "https://0xabc.agent.muwa.ai")
        )

        #expect(request.url?.absoluteString == "https://0xabc.agent.muwa.ai/health")
        #expect(request.httpMethod == "GET")
        #expect(request.value(forHTTPHeaderField: "Cache-Control") == "no-cache")
        #expect(request.value(forHTTPHeaderField: "User-Agent") == "MuwaRelayHealthCheck/1")
    }
    ...
    @Test func checkTreatsHTTP200HealthAsReachable() async throws {
        ...
        let result = await probe.check(
            baseURL: "https://0xabc.agent.muwa.ai",
            attempts: 1,
    ...
    private static func response(for request: URLRequest, statusCode: Int) -> HTTPURLResponse {
        HTTPURLResponse(
            url: request.url ?? URL(string: "https://0xabc.agent.muwa.ai/health")!,
