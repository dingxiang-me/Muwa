...
struct HTTPHandlerChatStreamingTests {
    ...
    @Test func chatCompletions_agentHeaderDoesNotInjectAgentContext() async throws {
        ...
        try await SandboxTestLock.runWithStoragePaths {
            let root = FileManager.default.temporaryDirectory.appendingPathComponent(
                "muwa-http-strict-context-\(UUID().uuidString)",
                isDirectory: true
            )
            ...
            request.setValue(agent.id.uuidString, forHTTPHeaderField: "X-Muwa-Agent-Id")
    ...
    @Test func agent_run_executes_tool_without_streaming_internal_sentinels() async throws {
        ...
        // never lands in the live `~/.muwa/agents` store and is torn down
        // with the directory (mirrors the strict-context test above).
        try await SandboxTestLock.runWithStoragePaths {
            let root = FileManager.default.temporaryDirectory.appendingPathComponent(
                "muwa-http-agent-run-sentinel-\(UUID().uuidString)",
                isDirectory: true
            )
            try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
            let previousRoot = MuwaPaths.overrideRoot
    ...
    /// agent via `/agents/{id}/run` so the App Intents "Ask Muwa" surface
