...
struct AgentReorderPersistenceTests {
    ...
    private func writeLegacyAgent(id: UUID, name: String) throws {
        let directory = MuwaPaths.agents()
        ...
        try data.write(to: MuwaPaths.agentFile(for: id), options: [.atomic])
    }
    ...
    func legacyAgentsWithoutOrderCanBeReordered() async throws {
        try await ChatHistoryTestStorage.run {
            ...
            let legacyJSON = try String(contentsOf: MuwaPaths.agentFile(for: alphaId), encoding: .utf8)
