...
struct CoreModelServiceFallbackTests {
    ...
    ///   3. Redirects MuwaPaths to a per-test temporary directory so
    ...
    private func withChatConfig<T: Sendable>(
        ...
        try await StoragePathsTestLock.shared.run {
            let root = FileManager.default.temporaryDirectory
                .appendingPathComponent("muwa-coremodel-\(UUID().uuidString)", isDirectory: true)
            try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)

            // Snapshot the singleton's current config BEFORE we redirect
            // MuwaPaths so the in-memory snapshot is authoritative —
            ...
            await MainActor.run {
                ...
                MuwaPaths.ensureExistsSilent(MuwaPaths.config())
