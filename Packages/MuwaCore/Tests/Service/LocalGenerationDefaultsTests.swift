...
struct LocalGenerationDefaultsTests {
    ...
    func qwen35() {
        // Qwen 3.5 specifies LOWER temperature than the 0.7 Muwa used to
    ...
    func nemotronUltraJANGTQ1L() {
        ...
        // The missing top_k is intentional: Muwa must not invent one for
        // this model family.
        let d = Self.defaults(
            fromJSON: #"""
                {
    ...
    func filesystemRoundTrip() throws {
        let tmp = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("muwa-gencfg-test-\(UUID().uuidString)", isDirectory: true)
    ...
    /// `jang_config.json > chat > sampling_defaults`. Muwa must prefer it.
    ...
    func bothFilesMerge() throws {
        let tmp = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent(
                "muwa-gencfg-both-\(UUID().uuidString)",
                isDirectory: true
            )
    ...
    func jangOnlyLoad() throws {
        let tmp = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent(
                "muwa-gencfg-jang-\(UUID().uuidString)",
                isDirectory: true
            )
        try FileManager.default.createDirectory(
            at: tmp,
    ...
    func jangConfigNoSamplingFallsThroughToHF() throws {
        ...
        let tmp = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent(
                "muwa-gencfg-jang-noop-\(UUID().uuidString)",
                isDirectory: true
            )
        try FileManager.default.createDirectory(
            at: tmp,
