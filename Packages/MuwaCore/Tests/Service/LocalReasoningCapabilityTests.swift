...
struct LocalReasoningCapabilityTests {
    ...
    func jangConfigDSV4Reasoning() {
        ...
        // mode. From Muwa's perspective there is no on-disk Jinja to
    ...
    func filesystemDSV4Shape() throws {
        let tmp = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent(
                "muwa-reasoning-dsv4-\(UUID().uuidString)",
                isDirectory: true
            )
    ...
    func filesystemNoJangConfig() throws {
        let tmp = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent(
                "muwa-reasoning-empty-\(UUID().uuidString)",
                isDirectory: true
            )
        try FileManager.default.createDirectory(
            at: tmp,
    ...
    func filesystemVisionSidecarWinsOverTextOnlyTokenizerTemplate() throws {
        let tmp = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent(
                "muwa-reasoning-vlm-sidecar-\(UUID().uuidString)",
                isDirectory: true
            )
        try FileManager.default.createDirectory(
            at: tmp,
