...
struct SwiftTransformersTokenizerLoaderTests {
    @Test func qwen35LocalTokenizerExposesNoGenerationPromptPrefixForCacheBoundary() async throws {
        ...
        let modelPath = ProcessInfo.processInfo.environment["MUWA_QWEN35_TEST_MODEL"] ?? defaultPath
    ...
    @Test func zayaVLLocalTokenizerRendersImagePlaceholderFromMuwaFallback() async throws {
        let defaultPath = "/Users/eric/models/Muwa/ZAYA1-VL-8B-MXFP4"
        let modelPath = ProcessInfo.processInfo.environment["MUWA_ZAYA_VL_TEST_MODEL"] ?? defaultPath
        let modelURL = URL(fileURLWithPath: modelPath)
        guard
            FileManager.default.fileExists(
                atPath: modelURL.appendingPathComponent("tokenizer.json").path
    ...
    @Test func zayaVLLocalTokenizerRendersTextOnlyToolsFromMuwaFallback() async throws {
        let defaultPath = "/Users/eric/models/Muwa/ZAYA1-VL-8B-MXFP4"
        let modelPath = ProcessInfo.processInfo.environment["MUWA_ZAYA_VL_TEST_MODEL"] ?? defaultPath
        let modelURL = URL(fileURLWithPath: modelPath)
        guard
    ...
    @Test func zayaTextLocalTokenizerRendersZyphraToolsNotGemmaFallback() async throws {
        ...
        let modelPath = ProcessInfo.processInfo.environment["MUWA_ZAYA_TEXT_TEST_MODEL"] ?? defaultPath
        let modelURL = URL(fileURLWithPath: modelPath)
        guard
            FileManager.default.fileExists(
                atPath: modelURL.appendingPathComponent("tokenizer.json").path
    ...
    @Test func gemma4LocalTokenizerRendersUnionToolSchemaTypeNatively() async throws {
        ...
        let modelPath = ProcessInfo.processInfo.environment["MUWA_GEMMA4_TEST_MODEL"] ?? defaultPath
        let modelURL = URL(fileURLWithPath: modelPath)
        guard
            FileManager.default.fileExists(
                atPath: modelURL.appendingPathComponent("tokenizer.json").path
    ...
    @Test func nemotronRequiredToolChoiceUsesNemotronFallbackNotGemmaFallback() async throws {
        ...
        let modelPath = ProcessInfo.processInfo.environment["MUWA_NEMOTRON_TEST_MODEL"] ?? defaultPath
        let modelURL = URL(fileURLWithPath: modelPath)
        guard
            FileManager.default.fileExists(
                atPath: modelURL.appendingPathComponent("tokenizer.json").path
    ...
    @Test func gemma3nLocalTokenizerDoesNotInventRequiredToolContractFromFallback() async throws {
        ...
        let modelPath = ProcessInfo.processInfo.environment["MUWA_GEMMA3N_TEST_MODEL"] ?? defaultPath
        let modelURL = URL(fileURLWithPath: modelPath)
        guard
            FileManager.default.fileExists(
                atPath: modelURL.appendingPathComponent("tokenizer.json").path
    ...
    @Test func gemma4LocalTokenizerRendersFirstTurnChatUIToolSurface() async throws {
        ...
        let tokenIds = try tokenizer.applyChatTemplate(
            messages: [
                [
                    "role": "user",
                    "content": "Create a file named muwa_live_probe.txt containing ok.",
                ]
            ],
        ...
        #expect(decoded.contains("Create a file named muwa_live_probe.txt"), "Decoded: \(decoded)")
    ...
    @Test func dsv4LocalTokenizerUsesCanonicalNoChatTemplatePath() async throws {
        ...
        let modelPath = ProcessInfo.processInfo.environment["MUWA_DSV4_TEST_MODEL"] ?? defaultPath
        let modelURL = URL(fileURLWithPath: modelPath)
        guard
            FileManager.default.fileExists(
                atPath: modelURL.appendingPathComponent("tokenizer.json").path
        ...
        #expect(
            ...
            "DSV4 bundles have no tokenizer chat_template; Muwa must route through vmlx's canonical DSV4 encoder path. Decoded: \(decoded)"
        )
        #expect(
    ...
    @Test func dsv4LocalTokenizerRendersDSMLToolsFromMuwaToolSpec() async throws {
        let defaultPath = "/Users/eric/models/JANGQ/DeepSeek-V4-Flash-JANGTQ-K"
        let modelPath = ProcessInfo.processInfo.environment["MUWA_DSV4_TEST_MODEL"] ?? defaultPath
        let modelURL = URL(fileURLWithPath: modelPath)
        guard
        ...
        #expect(
            ...
            "DSV4 canonical template path must include the Muwa-provided tool schema. Decoded: \(decoded)"
        )
        #expect(
    ...
    @Test func downloadedFamilyTokenizersRenderCapabilitiesDiscoverToolSurface() async throws {
        let rows = [
            ...
            LocalTokenizerRow(
                ...
                path: "/Users/eric/muwa_models/finished/gemma-4-26b-a4b-it-4bit"
            ),
            LocalTokenizerRow(
                family: "gemma4",
            ...
            LocalTokenizerRow(
                ...
                path: "/Users/eric/muwa_models/finished/gemma-4-31b-a4b-it-4bit"
            ),
            LocalTokenizerRow(
                family: "gemma4",
                label: "Gemma 4 E2B finished 4bit",
                path: "/Users/eric/muwa_models/finished/gemma-4-e2b-it-4bit"
            ),
            LocalTokenizerRow(
                family: "gemma4",
                label: "Gemma 4 E4B finished 4bit",
                path: "/Users/eric/muwa_models/finished/gemma-4-e4b-it-4bit"
            ),
            LocalTokenizerRow(
                family: "qwen36-27b",
        ...
        for row in availableRows {
            ...
            #expect(
                ...
                "\(row.label) must render the Muwa tool surface. Decoded: \(decoded)"
            )
            #expect(
    ...
    @Test func lfm2LocalTokenizerUsesStrictRequiredToolFallback() async throws {
        ...
        let modelPath = ProcessInfo.processInfo.environment["MUWA_LFM2_TEST_MODEL"] ?? defaultPath
        let modelURL = URL(fileURLWithPath: modelPath)
        guard
            FileManager.default.fileExists(
                atPath: modelURL.appendingPathComponent("tokenizer.json").path
    ...
    @Test func step37LocalTokenizerUsesRequiredToolFallbackAndClosesThinkingRail() async throws {
        ...
        let modelPath = ProcessInfo.processInfo.environment["MUWA_STEP37_TEST_MODEL"] ?? defaultPath
        let modelURL = URL(fileURLWithPath: modelPath)
        guard
            FileManager.default.fileExists(
                atPath: modelURL.appendingPathComponent("tokenizer.json").path
