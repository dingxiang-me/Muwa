...
struct RuntimePolicySourceTests {
    private static func packageRoot() -> URL {
        ...
        return cursor.deletingLastPathComponent()  // MuwaCore/
    }
    ...
    func makefileUsesWorkspaceResolver() throws {
        ...
        #expect(source.contains("WORKSPACE := Muwa.xcworkspace"))
    ...
    func appDelegateDoesNotForceDSV4DiagnosticCacheMode() throws {
        ...
        #expect(
            ...
            "Muwa must not force DSV4_KV_MODE; unset keeps vmlx's SWA+CSA+HSA default"
        )
        #expect(
    ...
    func storageKeyFailsClosedWhenEncryptedDataExists() throws {
        ...
        #expect(source.contains("MuwaPaths.chatHistoryDatabaseFile()"))
        #expect(source.contains("MuwaPaths.memoryDatabaseFile()"))
    }
    ...
    func startupAvoidsStorageKeyReadsAndBackgroundKeychainsSkipAuthenticationUI() throws {
        ...
        let server = try Self.source("Networking/MuwaServer.swift")
        ...
        #expect(!serverView.contains("if MuwaIdentity.exists()"))
    ...
    func keychainWrappersUseSharedHelper() throws {
        ...
        let entitlements = try Self.source("../../App/Muwa/muwa.entitlements")
    ...
    func osaurusServerDoesNotInstallNIOIdleTimeoutForInferenceChannels() throws {
        let source = try Self.source("Networking/MuwaServer.swift")
    ...
    func vmlxPinIncludesRuntimeHardening() throws {
        ...
        let workspaceResolved = try Self.source(
            "../../Muwa.xcworkspace/xcshareddata/swiftpm/Package.resolved"
        )
        let appResolved = try Self.source(
            "../../App/Muwa.xcodeproj/project.xcworkspace/xcshareddata/swiftpm/Package.resolved"
        )

        // The synchronized revision keeps the consolidated vmlx-swift pin for Muwa
        ...
        // role-token/DSML fragments in Muwa tool turns, plus the Gemma4
        ...
        // mamba_projection role alias so Muwa auto-settings consume the
        ...
        // as one shared C dependency. Muwa must not carry SwiftPM
    ...
    func dsv4RendererChecklistTracksInvalidGenericFlags() throws {
        ...
        let liveMatrix = try Self.source("../../docs/VMLX_SWIFT_MUWA_LIVE_MATRIX_2026_05_18.md")
    ...
    func swiftPMGraphUsesConsolidatedVMLXRuntime() throws {
        ...
        let workspaceMirrors = try Self.source(
            "../../Muwa.xcworkspace/xcshareddata/swiftpm/configuration/mirrors.json"
        )
        let appProjectMirrors = try Self.source(
            "../../App/Muwa.xcodeproj/project.xcworkspace/xcshareddata/swiftpm/configuration/mirrors.json"
        )
        ...
        let acknowledgements = try Self.source("../../App/Muwa/Acknowledgements.json")
    ...
    @Test("Muwa source does not import unvendored tokenizer or template modules")
    ...
    func cacheConfigEnablesSSMReDerive() throws {
        ...
        // migrated default seeds `enableSSMReDerive: true` so Muwa does
    ...
    /// TokenIterator-backed fast path. Muwa must not let a second solo
    ...
    /// cache store finishes, so Muwa also must not release the solo lease
    ...
    /// `.info` / stats sentinels at any Muwa stream boundary.
    ...
    func openAIChatCompletionsEndpointDoesNotInjectAgentContext() throws {
        ...
        #expect(chatCompletions.contains("X-Muwa-Agent-Id"))
    ...
    func nioServerStopReportsCompletion() throws {
        ...
        #expect(
            ...
            "MuwaServer.stop must return whether shutdown completed"
        )
    ...
    func liveProofKeychainDisabledModeKeepsStartupOffUserKeychain() throws {
        let paths = try Self.source("Utils/MuwaPaths.swift")
        ...
        #expect(paths.contains("MUWA_TEST_ROOT"))
        #expect(paths.contains("MUWA_TEST_ROOT"))
        #expect(storage.contains("MUWA_DISABLE_KEYCHAIN_FOR_TESTS"))
        ...
        #expect(appDelegate.contains("MUWA_KEYCHAIN_FREE_SHOW_UI"))
        #expect(appDelegate.contains("Keychain disabled by MUWA_DISABLE_KEYCHAIN_FOR_TESTS=1"))
    ...
    func modelRuntimeUsesTypedVMLXLoadConfiguration() throws {
        ...
        #expect(runtime.contains("base: .muwaProduction"))
        #expect(runtime.contains("baseLoadConfiguration: .muwaProduction"))
        ...
        #expect(
            ...
            "ModelRuntime must not use the plain local-directory load overload; it bypasses vmlx LoadConfiguration.muwaProduction, including load-time memory caps, mmap safetensors, and routed MLXPress auto policy"
        )
    }
    ...
    func runtimeConfigSnapshotAvoidsMainActorPreLoadHop() throws {
        ...
        #expect(config.contains("MuwaPaths.serverConfigFile()"))
    }
    ...
    func localDecodeLoopKeepsToolSchemasForParserValidation() throws {
        ...
        #expect(
            protocolErrors.contains(#"(error as NSError).domain == "MuwaToolChoice""#)
    ...
    func localStreamWithToolsTerminatesOnParsedToolInvocationBeforePostToolProseLeak() throws {
        ...
        #expect(
            ...
            "The Chat UI must stop the local stream as soon as vMLX emits a parsed tool invocation; otherwise DSV4 can leak pseudo-tool prose after the tool event before Muwa executes it."
        )
        #expect(
    ...
    func mlxBatchAdapterDoesNotForceHiddenReasoningDefaults() throws {
        ...
        #expect(
            ...
            "Local chat sampler fallback must use vmlx GenerateParameters defaults, not Muwa-specific invented temperature defaults."
        )
        #expect(
            ...
            "Local chat sampler fallback must use vmlx GenerateParameters defaults for topP/topK/minP instead of hardcoded Muwa literals."
        )
        #expect(
        ...
        #expect(
            ...
            "Gemma4 required/named tool turns must not bypass the model-bundled native template with an Muwa-forced Gemma4WithTools fallback."
        )
        #expect(
