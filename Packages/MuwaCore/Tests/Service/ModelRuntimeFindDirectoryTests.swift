...
struct ModelRuntimeFindDirectoryTests {
    ...
    func jangtq_formatStampWithSidecar_passes() throws {
        ...
        // sidecar/codebook; Muwa preflight must not block that valid
        // bundle shape.
    ...
    func nonexistentReturnsNil() throws {
        let tmp = URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
            .appendingPathComponent("muwa-test-\(UUID().uuidString)", isDirectory: true)
        let resolved = ModelRuntime.resolveLocalModelDirectory(
    ...
    private func makeRoot(layout: Layout = .plain) throws -> (root: URL, realModel: URL) {
        ...
        let root = URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
            .appendingPathComponent("muwa-modelruntime-test-\(UUID().uuidString)", isDirectory: true)
        ...
        case .symlinked:
            ...
            let externalBase = URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
                .appendingPathComponent("muwa-modelruntime-external-\(UUID().uuidString)", isDirectory: true)
    ...
    private func makeIsolatedDir() throws -> URL {
        let url = URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
            .appendingPathComponent("muwa-jangtq-test-\(UUID().uuidString)", isDirectory: true)
