...
struct HFRepoCandidatesTests {
    ...
    /// LOWERCASED org id (Muwa's chat router lowercases ids) must still
...
@Suite(.serialized)
struct MuwaOrgAutoFetchTests {

    private func makeMxtqBundle() throws -> URL {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("osu-muwa-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
    ...
    @Test func flatBundleResolvesViaOsaurusAIFallback() async throws {
        ...
        let fetcher: @Sendable (URL, URL) async throws -> Void = { url, _ in
            ...
            if url.absoluteString.contains("/OsaurusAI/") {
                try Data("real-muwa-ai-sidecar".utf8).write(to: dest)
                return
            }
            throw NSError(
                domain: "ModelRuntime",
    ...
    /// through Muwa): fetcher hits exactly that URL once.
