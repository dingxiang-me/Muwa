...
struct MLXBatchAdapterTests {
    ...
    @Test func maxBatchSize_respectsUserDefaults() {
        let key = "ai.muwa.scheduler.mlxBatchEngineMaxBatchSize"
        let defaults = isolatedDefaults()
    ...
    @Test func generateParameters_enableCompiledBatchDecodeForSoloDefault() {
        ...
        #expect(
            ...
            "Muwa default maxBatchSize=1 path must opt into vmlx BatchEngine compiled decode; leaving this false is the observed half-speed path"
        )
    }
    ...
    @Test func cacheDiskDirectoryOverrideHonorsBlockDiskDirectory() {
        ...
        settings.cache.blockDisk.directory = "~/Library/Caches/muwa-custom-kv"
        ...
        #expect(
            resolved?.standardizedFileURL.path
                ...
                .appendingPathComponent("Library/Caches/muwa-custom-kv")
                .standardizedFileURL.path
        )
    }

    @Test func cacheDiskDirectoryOverrideFallsBackToMuwaPathForPagedDiskDefault() {
        var settings = VMLXServerRuntimeSettings()
        settings.cache.prefix.enabled = true
        settings.cache.pagedKV.enabled = true
        settings.cache.blockDisk.enabled = true
        ...
        #expect(ModelRuntime.cacheDiskDirectoryOverride(for: settings.cache) == MuwaPaths.diskKVCache())
    }
    ...
    @Test func cacheDiskDirectoryOverrideHonorsLegacyDiskDirectoryWhenPagedKVIsOff() {
        ...
        settings.cache.legacyDisk.directory = "/tmp/muwa-legacy-kv"

        #expect(
            ModelRuntime.cacheDiskDirectoryOverride(for: settings.cache)
                == URL(fileURLWithPath: "/tmp/muwa-legacy-kv", isDirectory: true)
        )
    }
    ...
    @Test func compiledBatchDecodeDisabledForKnownUnsafeSoloModels() {
        #expect(
            ...
            "Hy3 is coherent on the uncompiled path but diverges on the B=1 compiled trace; Muwa must not request that path"
        )
        #expect(
            !MLXBatchAdapter.shouldEnableCompiledBatchDecode(
        ...
        #expect(
            ...
            "Step 3.7 is proven on vmlx's uncompiled BatchEngine path; Muwa must not route it through the compiled B=1 trace until that path is separately proven"
        )
        #expect(
            !MLXBatchAdapter.shouldEnableCompiledBatchDecode(
                modelName: "JANGQ-AI/Step-3.7-Flash-JANGTQ_K",
        ...
        #expect(
            ...
            "Hybrid SSM families need exact cache/topology proof on the uncompiled path; Muwa must not opt them into the B=1 compiled trace"
        )
        #expect(
            !MLXBatchAdapter.shouldEnableCompiledBatchDecode(
    ...
    @Test func additionalContext_mapsDSV4ReasoningModesToEncoderKwargs() {
        ...
        #expect(
            ...
            "DSV4 Max must reach vmlx-swift unchanged; Muwa must not hide runtime issues behind an effort downgrade"
        )
