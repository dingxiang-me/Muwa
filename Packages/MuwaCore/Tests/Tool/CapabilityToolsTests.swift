        ...
        try await StoragePathsTestLock.shared.run {
            try await DynamicCatalogTestLock.shared.run {
                let root = FileManager.default.temporaryDirectory.appendingPathComponent(
                    "muwa-capability-search-root-\(UUID().uuidString)",
                    isDirectory: true
                )
                ...
                let tempDir = FileManager.default.temporaryDirectory.appendingPathComponent(
                    "muwa-capability-search-grant-\(UUID().uuidString)",
                    isDirectory: true
                )
...
struct CapabilitiesLoadToolTests {
    ...
    func toolLoadRejectsDynamicToolOutsideAgentGrant() async throws {
        try await StoragePathsTestLock.shared.run {
            try await DynamicCatalogTestLock.shared.run {
                let root = FileManager.default.temporaryDirectory.appendingPathComponent(
                    "muwa-capability-load-root-\(UUID().uuidString)",
                    isDirectory: true
                )
                try? FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
                let previousRoot = MuwaPaths.overrideRoot
                ...
                let tempDir = FileManager.default.temporaryDirectory.appendingPathComponent(
                    "muwa-capability-load-grant-\(UUID().uuidString)",
                    isDirectory: true
                )
                let previousOverride = ToolConfigurationStore.overrideDirectory
                ToolConfigurationStore.overrideDirectory = tempDir
    ...
    func skillLoadAutoLoadsPluginToolGroup() async throws {
        try await StoragePathsTestLock.shared.run {
            try await DynamicCatalogTestLock.shared.run {
                let root = FileManager.default.temporaryDirectory.appendingPathComponent(
                    "muwa-skill-autoload-root-\(UUID().uuidString)",
                    isDirectory: true
                )
                try? FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
                let previousRoot = MuwaPaths.overrideRoot
    ...
    func toolLoadReportsDisabledAvailability() async throws {
        try await DynamicCatalogTestLock.shared.run {
            let tempDir = FileManager.default.temporaryDirectory.appendingPathComponent(
                "muwa-capability-load-disabled-\(UUID().uuidString)",
                isDirectory: true
            )
            let previousOverride = ToolConfigurationStore.overrideDirectory
            ToolConfigurationStore.overrideDirectory = tempDir
...
private struct CapabilityPolicyFixtureTool: MuwaTool {
