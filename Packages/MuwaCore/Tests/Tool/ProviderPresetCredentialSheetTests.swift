...
//  chat-driven `muwa_provider_add` tool relies on:
//
...
struct ProviderPresetCredentialSheetTests {
    ...
    @Test
    func legacyInit_muwaAgent_carriesNilPreset() {
        let request = ProviderCredentialRequest(
            providerType: .muwa,
            providerName: "Peer",
            mode: .addNew
        ...
        #expect(request.providerType == .muwa)
    ...
    @Test
    func resolver_muwaAgentAliasIsSpecialCase() {
        if case .muwaAgent = ProviderToolShared.resolve("muwa_agent") {
            return
        }
        Issue.record("muwa_agent must resolve to the muwaAgent special case")
    }
    ...
    func parseManualModelIds_singleEntryYieldsOneId() {
        #expect(MuwaProviderAddTool.parseManualModelIds("gpt-4o") == ["gpt-4o"])
    }

    @Test
    func parseManualModelIds_commaAndNewlineSeparated() {
        let parsed = MuwaProviderAddTool.parseManualModelIds(
            "gpt-4o, gpt-4o-mini\nprod-chat"
        )
    ...
    func parseManualModelIds_emptyAndWhitespaceCollapseToEmptyArray() {
        #expect(MuwaProviderAddTool.parseManualModelIds("") == [])
        #expect(MuwaProviderAddTool.parseManualModelIds(" , \n , ") == [])
    }
    ...
    func reservedExtraKeys_blockHostAndDeploymentFromCustomHeaders() {
        ...
        #expect(MuwaProviderAddTool.reservedExtraKeys.contains("host"))
        #expect(MuwaProviderAddTool.reservedExtraKeys.contains("deployment"))
    }
}
