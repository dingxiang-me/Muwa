...
//  modify the running user's `com.muwa.account` Master Key. We isolate
...
// `com.muwa.account` Master Key slot in Keychain. Without it Swift Testing
...
struct MasterKeyExistsGuardTests {
    ...
    func generateAllowReplaceOverwrites() throws {
        try withEphemeralMaster {
            ...
            #expect(first.muwaId != second.muwaId)
        }
    }
...
private func canProbeMasterKeyWritePath() -> Bool {
    let probeService = "com.muwa.tests.keychain-probe"
