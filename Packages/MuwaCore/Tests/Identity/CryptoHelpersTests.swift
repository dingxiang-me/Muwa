...
struct CryptoHelpersTests {
    ...
    @Test func keccak256_deterministic() {
        let input = Data("muwa identity".utf8)
    ...
    @Test func signPayload_recoverAddress_roundtrip() throws {
        ...
        let recovered = try recoverAddress(
            payload: payload,
            signature: signature,
            domainPrefix: "Muwa Signed Message"
        )
    ...
    @Test func signAccessPayload_recoverAddress_roundtrip() throws {
        ...
        let recovered = try recoverAddress(
            payload: payload,
            signature: signature,
            domainPrefix: "Muwa Signed Access"
        )
        #expect(recovered.lowercased() == expectedAddress.lowercased())
    }
    ...
    @Test func recoverAddress_invalidSignatureLength_throws() {
        ...
        #expect(throws: MuwaIdentityError.self) {
            _ = try recoverAddress(payload: payload, signature: shortSig, domainPrefix: "Muwa Signed Access")
        }
    }
    ...
    @Test func deriveMuwaId_deterministic() throws {
        let addr1 = try deriveMuwaId(from: TestKeys.alicePrivateKey)
        let addr2 = try deriveMuwaId(from: TestKeys.alicePrivateKey)
        #expect(addr1 == addr2)
    }

    @Test func deriveMuwaId_differentKeys_differentAddresses() throws {
        let alice = try deriveMuwaId(from: TestKeys.alicePrivateKey)
        let bob = try deriveMuwaId(from: TestKeys.bobPrivateKey)
        let carol = try deriveMuwaId(from: TestKeys.carolPrivateKey)
        #expect(alice != bob)
        #expect(bob != carol)
        #expect(alice != carol)
    }

    @Test func deriveMuwaId_checksumFormat() throws {
        let addr = try deriveMuwaId(from: TestKeys.alicePrivateKey)
    ...
    @Test func base64url_roundtrip() {
        let original = Data("Hello, Muwa!".utf8)
