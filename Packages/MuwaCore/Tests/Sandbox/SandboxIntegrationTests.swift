...
/// `MUWA_RUN_SANDBOX_INTEGRATION_TESTS=1` and reported as `Disabled`
/// in CI rather than silently passing.
private let isSandboxIntegrationEnabled =
    ProcessInfo.processInfo.environment["MUWA_RUN_SANDBOX_INTEGRATION_TESTS"] == "1"

@Suite(
    .serialized,
    .disabled(
        if: !isSandboxIntegrationEnabled,
        "Set MUWA_RUN_SANDBOX_INTEGRATION_TESTS=1 to run; requires Apple Containerization."
    )
)
