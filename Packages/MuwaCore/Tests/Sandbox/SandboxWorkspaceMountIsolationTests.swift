...
//  the sandbox mounts ONLY its own Muwa-owned workspace, never the
...
//
    ...
    struct SandboxWorkspaceMountIsolationTests {
        ...
        /// Muwa-owned container workspace, not a host folder.
        @Test func returnsContainerWorkspaceUnchanged() {
            let workspace = MuwaPaths.containerWorkspace().path
        ...
        @Test func containerWorkspaceDiffersFromHostFolderRoots() {
            let workspace = MuwaPaths.containerWorkspace().standardizedFileURL.path
