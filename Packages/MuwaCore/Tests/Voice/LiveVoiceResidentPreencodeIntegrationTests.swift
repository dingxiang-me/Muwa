...
private let isResidentOmniPreencodeEnabled =
    ProcessInfo.processInfo.environment["MUWA_RUN_REAL_OMNI_PREENCODE"] == "1"
...
@Suite(
    ...
    .disabled(
        ...
        "Set MUWA_RUN_REAL_OMNI_PREENCODE=1, OSU_MODELS_DIR, MUWA_OMNI_MODEL, and MUWA_OMNI_AUDIO to run."
    )
)
struct LiveVoiceResidentPreencodeIntegrationTests {
    ...
    func residentModelStoresFreshPreencodedLiveAudio() async throws {
        ...
        let modelName = env["MUWA_OMNI_MODEL"] ?? "nemotron-omni-nano-jangtq-crack"
        let audioPath = try #require(
            env["MUWA_OMNI_AUDIO"],
            "MUWA_OMNI_AUDIO must point to a real audio file."
        )
        ...
        print(
            "[Muwa][LiveVoiceIntegration] model=\(resident.name) warm_ms=\(warmMs) samples=\(samples.count) encode_ms=\(result.encodeMs) embedding_shape=\(embedding.shape) composer_submit=preencoded"
        )
    }
            ...
            throw NSError(
                ...
                userInfo: [
                    NSLocalizedDescriptionKey:
                        "Unable to find MLX default.metallib. Build the Muwa app first or set MUWA_MLX_METALLIB to the metallib path."
                ]
            )
        }
    ...
    private static func findMLXMetallib(fileManager: FileManager) throws -> URL? {
        ...
        if let envPath = env["MUWA_MLX_METALLIB"], !envPath.isEmpty {
        ...
        candidates.append(
            root.appendingPathComponent(
                "build/XcodeDerivedData-livevoice/Build/Products/Debug/muwa.app/Contents/Resources/default.metallib"
            )
        )
        candidates.append(
            root.appendingPathComponent(
    ...
    private static func metallibDestinationDirectories(fileManager: FileManager) -> [URL] {
        ...
        candidates.append(
            packageRoot.appendingPathComponent(
                ".build/arm64-apple-macosx/debug/MuwaCorePackageTests.xctest/Contents/MacOS"
            )
        )
        candidates.append(
            packageRoot.appendingPathComponent(
                ".build/arm64-apple-macosx/debug/MuwaCorePackageTests.xctest/Contents/Resources"
            )
        )
        candidates.append(packageRoot.appendingPathComponent(".build/debug"))
        candidates.append(
            packageRoot.appendingPathComponent(
                ".build/debug/MuwaCorePackageTests.xctest/Contents/MacOS"
            )
        )
        candidates.append(
            packageRoot.appendingPathComponent(
                ".build/debug/MuwaCorePackageTests.xctest/Contents/Resources"
            )
        )
