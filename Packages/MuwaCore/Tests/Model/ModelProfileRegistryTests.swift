...
/// Guards the profile-matching behavior behind Muwa's "reasoning
...
struct ModelProfileRegistryTests {
    ...
    func qwen3_6_jangtq_notAutoProfile() {
        ...
        // "mxtq" in jang_config.json) — muwa-side the *profile* is still
    ...
    /// `laguna_glm_thinking_v5/chat_template.jinja`. Muwa exposes the
    ...
    /// `detailed thinking on/off` directive. Muwa exposes the option without
