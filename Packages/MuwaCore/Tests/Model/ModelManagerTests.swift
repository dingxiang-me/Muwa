...
struct ModelManagerTests {
    ...
    @Test func loadAvailableModels_initializesStates() async throws {
        ...
        // download state probing is intentionally off-main and applies shortly
        // after init returns.
        ...
        if models.count > 0 {
            let states = try await waitForDownloadStates(manager: manager, modelIds: models.map(\.id))
            for model in models {
    ...
    @Test func scanLocalModels_detectsExternalGemma4QATRootWhenConfigured() async throws {
        guard let rawRoot = ProcessInfo.processInfo.environment["MUWA_TEST_GEMMA4_QAT_ROOT"],
            !rawRoot.isEmpty
        else {
            return
        }
    ...
    @Test func discoverLocalModels_timeoutDoesNotCacheEmptyResult() async throws {
        try await StoragePathsTestLock.shared.run {
            ...
            let previousRoot = MuwaPaths.overrideRoot
            ...
            MuwaPaths.overrideRoot = manifestRoot
            ...
            defer {
                ...
                MuwaPaths.overrideRoot = previousRoot
                ExternalModelLocator.invalidateInMemory()
                ModelManager.invalidateLocalModelsCache()
            ...
            } while second.isEmpty && Date() < deadline
...
private func waitForDownloadStates(
    manager: ModelManager,
    modelIds: [String],
    timeout: Duration = .seconds(2)
) async throws -> [String: DownloadState] {
    let deadline = ContinuousClock.now + timeout
    while ContinuousClock.now < deadline {
        let states = await MainActor.run { manager.downloadStates }
        if modelIds.allSatisfy({ states[$0] != nil }) {
            return states
        }
        try await Task.sleep(for: .milliseconds(20))
    }
    return await MainActor.run { manager.downloadStates }
}
