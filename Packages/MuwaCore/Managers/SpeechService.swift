...
public enum SpeechError: Error, LocalizedError {
    ...
    case cloudProviderNotConfigured

    public var errorDescription: String? {
        switch self {
        case .noModelSelected:
            return "未选择语音模型。请先下载并选择一个模型。"
        case .cloudProviderNotConfigured:
            return "云端语音识别服务未配置。请在语音设置中填写凭据。"
        case .modelNotLoaded:
            return "语音模型尚未加载。请先加载模型。"
        case .modelNotReady:
            return "语音模型尚未就绪。"
...
public final class SpeechService: ObservableObject {
    ...
    private var voiceConfigObserver: NSObjectProtocol?
    ...
    private init() {
        ...
        // Switching the STT provider changes what "model loaded" means;
        // re-evaluate readiness whenever voice settings change.
        voiceConfigObserver = NotificationCenter.default.addObserver(
            forName: .voiceConfigurationChanged,
            object: nil,
            queue: .main
        ) { [weak self] _ in
            Task { @MainActor in
    ...
    deinit {
        ...
        if let observer = voiceConfigObserver {
            NotificationCenter.default.removeObserver(observer)
        }
    }
    ...
    /// Sentinel `loadedModelId` for the cloud-backed transcription path.
    public static let cloudModelId = "volcengine"

    /// True when the user selected a cloud STT provider in settings.
    public var usesCloudTranscription: Bool {
        SpeechConfigurationStore.load().transcriptionProvider == .volcengine
    }

    /// True when the cloud provider is selected AND has credentials, i.e.
    /// transcription works without any downloaded local model.
    public var cloudTranscriptionReady: Bool {
        usesCloudTranscription && VolcengineASRSettings.current().isConfigured
    }

    /// Reconcile `isModelLoaded` with the configured provider after a
    /// settings change (provider switch, credential update).
    private func refreshProviderState() {
        if usesCloudTranscription {
            if cloudTranscriptionReady {
                isModelLoaded = true
                loadedModelId = Self.cloudModelId
                lastError = nil
            } else if loadedModelId == Self.cloudModelId {
                isModelLoaded = false
                loadedModelId = nil
            }
        } else if loadedModelId == Self.cloudModelId {
            // Back to local: drop the cloud sentinel and lazy-load on demand.
            isModelLoaded = false
            loadedModelId = nil
            Task { await autoLoadIfNeeded() }
        }
    }
    ...
    public func ensureModelLoaded() async throws {
        if usesCloudTranscription {
            guard cloudTranscriptionReady else {
                throw SpeechError.cloudProviderNotConfigured
            }
            isModelLoaded = true
            loadedModelId = Self.cloudModelId
            return
    ...
    public func transcribe(audioURL: URL) async throws -> TranscriptionResult {
        ...
        if usesCloudTranscription {
            return try await transcribeViaCloud(audioURL: audioURL)
        }
    ...
    /// One-shot cloud transcription of an audio file.
    private func transcribeViaCloud(audioURL: URL) async throws -> TranscriptionResult {
        let settings = VolcengineASRSettings.current()
        ...
        do {
            let samples = try await Task.detached(priority: .userInitiated) {
                try Self.loadAudioFileAs16kMono(audioURL)
            }.value
            let text = try await VolcengineASRSession.transcribe(samples: samples, settings: settings)
            return TranscriptionResult(
                text: text,
    ...
    /// Decode any AVFoundation-readable audio file into 16 kHz mono Float32.
    private nonisolated static func loadAudioFileAs16kMono(_ url: URL) throws -> [Float] {
        ...
        guard
            ...
            throw SpeechError.transcriptionFailed("Unsupported audio format")
        }
        ...
        while !reachedEnd {
            ...
            let outCapacity = AVAudioFrameCount(Double(readCapacity) * ratio) + 100
            guard
            ...
            let inputBlock: AVAudioConverterInputBlock = { _, outStatus in
                guard
                    let inBuffer = AVAudioPCMBuffer(
                        pcmFormat: file.processingFormat, frameCapacity: readCapacity)
                else {
            ...
            if let readError {
                throw SpeechError.transcriptionFailed(readError.localizedDescription)
            }
            if let conversionError {
                throw SpeechError.transcriptionFailed(conversionError.localizedDescription)
            }
            if let floatData = outBuffer.floatChannelData?[0], outBuffer.frameLength > 0 {
                output.append(
                    contentsOf: UnsafeBufferPointer(start: floatData, count: Int(outBuffer.frameLength)))
            }
    ...
    private var transcriptionWorker: (any TranscriptionWorking)?
    ...
    public func startStreamingTranscription() async throws {
        ...
        if inputSource == .microphone {
            ...
            do {
                ...
                transcriptionWorker = try makeTranscriptionWorker(inputFormat: tapFormat, config: config)
            ...
            do {
                ...
                transcriptionWorker = try makeTranscriptionWorker(inputFormat: systemAudioFormat, config: config)
    ...
    private func makeTranscriptionWorker(
        inputFormat: AVAudioFormat,
        config: SpeechConfiguration
    ) throws -> any TranscriptionWorking {
        if config.transcriptionProvider == .volcengine {
            let settings = VolcengineASRSettings.current()
            guard settings.isConfigured else {
                throw SpeechError.cloudProviderNotConfigured
            }
            return CloudTranscriptionWorker(
                settings: settings,
    ...
    public func stopStreamingTranscription(force: Bool = false) async -> String {
        ...
        let trailingCloudText = await transcriptionWorker?.finalize()
        ...
        if let trailingCloudText,
            !trailingCloudText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
        {
            let finalText = trailingCloudText.trimmingCharacters(in: .whitespacesAndNewlines)
            appendConfirmedTranscription(finalText)
            currentTranscription = ""
            return finalText
        }
    ...
    private func appendConfirmedTranscription(_ text: String) {
    ...
    private func startWorkerProcessing() {
        ...
        Task {
            ...
            for await update in await worker.start() {
                ...
                case .final(let text):
                    self.appendConfirmedTranscription(text)
...
/// Common surface for the streaming transcription workers. The local
/// FluidAudio worker transcribes on-device; `CloudTranscriptionWorker`
/// forwards audio to a remote ASR service.
protocol TranscriptionWorking: Actor {
    func start() -> AsyncStream<TranscriptionUpdate>
    func stop()
    /// Flush remaining audio and return any trailing text that was not yet
    /// delivered through the update stream. The local worker returns `nil`
    /// (its final pass runs in `stopStreamingTranscription` instead).
    func finalize() async -> String?
}

private actor TranscriptionWorker: TranscriptionWorking {
    ...
    func finalize() async -> String? { nil }
