...
public final class ScheduleManager {
    ...
    private init() {
        ...
        print("[Muwa] ScheduleManager initialized with \(schedules.count) schedules")
    }
    ...
    public func create(
        ...
        print("[Muwa] Created schedule: \(schedule.name)")
    ...
    public func update(_ schedule: Schedule) {
        ...
        NotificationCenter.default.post(name: .schedulesChanged, object: nil)
        print("[Muwa] Updated schedule: \(schedule.name)")
    }
    ...
    public func delete(id: UUID) -> Bool {
        ...
        NotificationCenter.default.post(name: .schedulesChanged, object: nil)
        print("[Muwa] Deleted schedule: \(id)")
    ...
    private func scheduleNextTimer() {
        ...
        guard !enabledSchedules.isEmpty else {
            print("[Muwa] No enabled schedules, timer cancelled")
            return
        }
        ...
        guard let fireDate = soonestDate else {
            print("[Muwa] No upcoming schedule runs")
            return
        }

        let delay = max(0, fireDate.timeIntervalSince(now))
        print(
            "[Muwa] Next schedule timer in \(String(format: "%.1f", delay)) seconds (\(schedulesToRun.count) schedule(s))"
        )
        ...
        for schedule in schedules where schedule.isEnabled {
            ...
            if case .once(let date) = schedule.frequency {
                ...
                if date <= now && schedule.executionAnchor == nil {
                    print("[Muwa] Found missed once schedule: \(schedule.name)")
                    executeSchedule(schedule)
                }
            } else {
                ...
                if let anchor = schedule.executionAnchor {
                    if let nextAfterAnchor = schedule.frequency.nextRunDate(after: anchor),
                        nextAfterAnchor <= now
                    {
                        print("[Muwa] Found missed recurring schedule: \(schedule.name)")
                        executeSchedule(schedule)
                    }
                }
            }
    ...
    private func executeSchedule(_ schedule: Schedule) {
        ...
        if let rejection = Agent.rejectBuiltInForExternalSurface(
            ...
            print("[Muwa] Skipping schedule '\(schedule.name)': \(rejection.message)")
            return
        }
        ...
        print("[Muwa] Executing schedule: \(triggeredSchedule.name)")

        let task = Task { @MainActor in
            guard let handle = await TaskDispatcher.shared.dispatch(request) else {
                print("[Muwa] Failed to dispatch schedule: \(triggeredSchedule.name)")
    ...
    private func handleResult(_ result: DispatchResult, schedule: Schedule, request: DispatchRequest) {
        ...
        case .completed(let sessionId):
            ...
            print("[Muwa] Schedule completed: \(schedule.name)")

        case .cancelled:
            print("[Muwa] Schedule cancelled: \(schedule.name)")

        case .failed(let error):
            print("[Muwa] Schedule failed: \(schedule.name) - \(error)")
        }
    }
}
