...
enum LaunchGuard {
    ...
    static func checkOnLaunch() -> Bool {
        ...
        if defaults.bool(forKey: startupInProgressKey) {
            ...
            NSLog("[Muwa] Previous launch did not complete (consecutive crashes: %d)", count)
            disabledFeatures = features(forCrashCount: count)
            if !disabledFeatures.isEmpty {
                NSLog(
                    "[Muwa] Safe mode active — disabled features bitmask: %d",
    ...
    static func noteHealthyHealthCheck() {
        ...
        disabledFeatures = []
        NSLog(
            "[Muwa] Clean /health observed — clearing safe mode (was bitmask %d) and requesting subsystem recovery",
            recovered.rawValue
        )
