...
public final class WatcherManager {
    ...
    private init() {
        ...
        print("[Muwa] WatcherManager initialized with \(watchers.count) watchers")
    }
    ...
    public func create(
        ...
        print("[Muwa] Created watcher: \(watcher.name)")
    ...
    public func update(_ watcher: Watcher) {
        ...
        NotificationCenter.default.post(name: .watchersChanged, object: nil)
        print("[Muwa] Updated watcher: \(watcher.name)")
    }
    ...
    public func runNow(_ watcherId: UUID) {
        ...
        guard currentPhase == .idle else {
            print("[Muwa] runNow skipped for \(watcher.name): phase is \(currentPhase.rawValue)")
            return
        }
    ...
    private func rebuildEventStream() {
        ...
        guard !enabledWatchers.isEmpty else {
            print("[Muwa] No enabled watchers, FSEvent stream stopped")
            return
        }
        ...
        guard !paths.isEmpty else {
            print("[Muwa] No valid watch paths found")
            return
        }
        ...
        guard
            ...
            print("[Muwa] Failed to create FSEvent stream")
            return
        }
        ...
        print("[Muwa] FSEvent stream started for \(paths.count) path(s)")
    }
    ...
    private func resolveWatchPath(for watcher: Watcher) -> String? {
        ...
        do {
            ...
            guard !isStale else {
                print("[Muwa] Watch bookmark is stale for: \(watcher.name)")
                return watcher.watchPath
            }
            _ = url.startAccessingSecurityScopedResource()
            return url.path
        } catch {
            print("[Muwa] Failed to resolve watch bookmark for \(watcher.name): \(error)")
            return watcher.watchPath
        }
    }
    ...
    private func processCurrentState(for watcher: Watcher) {
        ...
        if let rejection = Agent.rejectBuiltInForExternalSurface(
            ...
            print("[Muwa] [\(watcher.name)] watcher skipped: \(rejection.message)")
        ...
        guard currentPhase == .debouncing || currentPhase == .idle else {
            print("[Muwa] [\(watcher.name)] dispatch skipped: phase is \(currentPhase.rawValue)")
            return
        }

        // Belt-and-suspenders: reject if an execution task already exists
        if executionTasks[watcher.id] != nil {
            print("[Muwa] [\(watcher.name)] dispatch skipped: execution task exists")
            phases[watcher.id] = .idle
            return
        }
        ...
        let task = Task { @MainActor [weak self] in
            ...
            defer {
                ...
                print("[Muwa] [\(watcher.name)] phase → idle")
            }
            ...
            while !Task.isCancelled {
                ...
                if iteration > maxIterations {
                    print("[Muwa] [\(watcher.name)] hit max iterations (\(maxIterations)), forcing idle")
                ...
                guard let fingerprint = await Self.captureFingerprint(at: watchURL, excluding: excluded) else {
                    print("[Muwa] [\(watcher.name)] fingerprint capture failed (iteration \(iteration))")
                    break
                }
                ...
                if let known = self.lastKnownFingerprints[watcherId], !fingerprint.changed(from: known) {
                    if iteration > 1 {
                        print("[Muwa] [\(watcher.name)] converged after \(iteration - 1) iteration(s)")
                    } else {
                        // Phantom event slipped through (race between initial check and task start)
                        print("[Muwa] [\(watcher.name)] phantom event, skipping")
                    }
                    break
                }
                ...
                print("[Muwa] [\(watcher.name)] phase → processing (iteration \(iteration), \(changeCount) changes)")
                ...
                guard let handle = await TaskDispatcher.shared.dispatch(request) else {
                    print("[Muwa] [\(watcher.name)] dispatch failed (iteration \(iteration))")
                    break
                }
                ...
                print("[Muwa] [\(watcher.name)] phase → settling (\(watcher.settleSeconds)s)")
    ...
    private func handleResult(_ result: DispatchResult, watcher: Watcher) {
        ...
        case .completed(let sessionId):
            ...
            print("[Muwa] Watcher completed: \(watcher.name)")

        case .cancelled:
            print("[Muwa] Watcher cancelled: \(watcher.name)")

        case .failed(let error):
            print("[Muwa] Watcher failed: \(watcher.name) - \(error)")
        }
    }
