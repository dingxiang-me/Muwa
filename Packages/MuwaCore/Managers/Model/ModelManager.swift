...
final class ModelManager: NSObject, ObservableObject {
    ...
    /// tests that exercise `applyMuwaOrgFetch(...)` flip it on so the
...
extension ModelManager {
    ...
    nonisolated fileprivate static let curatedSuggestedModels: [MLXModel] = [
        ...
        // (`"mxtq"`) — no muwa-side branching required.
        ...
        // jang_config.json (`weight_format: mxtq`) at load time — no muwa-side
        // branching required.
        ...
        // via `BatchEngine.admitPendingRequests`; Muwa *also* calls
        ...
        // factory based on config / jang_config metadata; Muwa only needs
...
extension ModelManager {
    ...
    ///
    /// "Download size" here is the sum of just the files Muwa actually
...
extension ModelManager {
    ...
    fileprivate static let muwaOrgAuthor = "OsaurusAI"

    /// True if `id` is `"<muwaOrgAuthor>/<repo>"` (case-insensitive).
    fileprivate static func isMuwaOrgRepo(_ id: String) -> Bool {
        guard let org = id.split(separator: "/").first.map(String.init) else { return false }
        return org.caseInsensitiveCompare(muwaOrgAuthor) == .orderedSame
    }
    ...
    func loadOsaurusAIOrgModels() async {
        ...
        applyMuwaOrgFetch(autoFetched: autoFetched, statsById: statsById, sizesById: sizesById)
    }
    ...
    func applyMuwaOrgFetch(
        ...
        let preserved = suggestedModels.filter { model in
            ...
            return !Self.isMuwaOrgRepo(model.id)
        }
