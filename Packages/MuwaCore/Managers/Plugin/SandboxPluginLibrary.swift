...
//  installation. Plugin recipes live in ~/.muwa/sandbox-plugins/ as JSON
...
public final class SandboxPluginLibrary: ObservableObject {
    ...
    public func save(_ plugin: SandboxPlugin) {
        ...
        let dir = MuwaPaths.sandboxPluginLibrary()
        ...
        MuwaPaths.ensureExistsSilent(versionsDir)
    ...
    public func update(oldId: String, plugin: SandboxPlugin) {
        if oldId != plugin.id {
            ...
            if fm.fileExists(atPath: oldVersionsDir.path) {
                MuwaPaths.ensureExistsSilent(newVersionsDir.deletingLastPathComponent())
