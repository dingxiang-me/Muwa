...
final class PluginManager {
    ...
    /// `muwa_plugin.h`), but most plugins call `host->get_active_agent_id`
    ...
    /// `key == "__muwa_abi_probe__"` and early-return; the value is
    ...
    nonisolated static let abiProbeKey = "__muwa_abi_probe__"
    ...
    /// the launch sequence.
    ///
    /// The probe is fast (a single `(__muwa_abi_probe__, <UUID>)`
    ...
    /// Reads the host's `CFBundleShortVersionString` for `min_muwa`
    ...
    /// rejecting every plugin that declares `min_muwa`. Erroring
    ...
    /// `min_muwa` or `min_macos` constraint that the running host
    ...
    nonisolated static func compatibilityFailure(
        ...
        if let minHost = manifest.min_muwa, !minHost.isEmpty {
            if let required = SemanticVersion.parse(minHost) {
                if let current = parseHostVersion(hostVersion) {
                    if current < required {
                        return PluginLoadError(
                            message:
                                "Plugin \(manifest.plugin_id) requires Muwa \(required) or later; "
                                + "this host is \(current). Update Muwa to load this plugin."
                        )
                    }
                } else {
                    ...
                    NSLog(
                        "[Muwa] Cannot enforce min_muwa='%@' for %@ — host version "
                ...
                NSLog(
                    "[Muwa] Plugin %@ has unparseable min_muwa '%@' — ignoring constraint.",
                    manifest.plugin_id,
                    minHost
                )
            }
        ...
        if let minOS = manifest.min_macos, !minOS.isEmpty {
            if let required = parseOSVersion(minOS) {
                ...
                NSLog(
                    "[Muwa] Plugin %@ has unparseable min_macos '%@' — ignoring constraint.",
                    manifest.plugin_id,
                    minOS
                )
            }
    ...
    nonisolated private static func loadPluginWithError(at url: URL) -> Result<LoadedPlugin, PluginLoadError> {
        ...
        guard let handle = dlopen(url.path, Int32(flags)) else {
            ...
            print("[Muwa] dlopen failed for \(url.path): \(errorMsg)")
        ...
        if let v2sym = dlsym(handle, "muwa_plugin_entry_v2") {
            ...
            do {
                ...
                print("[Muwa] \(errorMsg) for \(url.lastPathComponent)")
                dlclose(handle)
                return .failure(PluginLoadError(message: errorMsg))
            }
            ...
            guard let apiRawPtr else {
                ...
                print("[Muwa] \(errorMsg) in \(url.lastPathComponent)")
                ctx.teardown()
                dlclose(handle)
                return .failure(PluginLoadError(message: errorMsg))
            }
            ...
            print("[Muwa] Loaded plugin from \(url.lastPathComponent) (entry=v2, abi=v\(abiVersion))")
        } else if let v1sym = dlsym(handle, "muwa_plugin_entry") {
            ...
            abiVersion = 1
            print(
                "[Muwa] Loaded plugin from \(url.lastPathComponent) (entry=v1 legacy). "
                    + "v1 plugins cannot call host APIs. Consider rebuilding against the v3 surface "
                    + "(export muwa_plugin_entry_v2 with api.version >= 2) for richer functionality."
            )
        } else {
            let errorMsg = "Missing plugin entry point (muwa_plugin_entry_v2 or muwa_plugin_entry)"
            print("[Muwa] \(errorMsg) in \(url.lastPathComponent)")
            dlclose(handle)
            return .failure(PluginLoadError(message: errorMsg))
        }
        ...
        // declare `min_muwa` and `min_macos` so they can opt into ABI
        ...
        if let compatError = compatibilityFailure(
            ...
            print("[Muwa] \(compatError.message)")
            api.destroy?(ctx)
            hostContext?.teardown()
            dlclose(handle)
            return .failure(
    ...
    nonisolated private static func loadPluginSkills(from dylibURL: URL, pluginId: String) -> [Skill] {
        ...
        for file in files {
            ...
            do {
                ...
                NSLog("[Muwa] Loaded skill '\(skill.name)' from plugin \(pluginId)")
            } catch {
                NSLog("[Muwa] Failed to parse SKILL.md from plugin \(pluginId): \(error)")
            }
        }
    ...
    private nonisolated static func addToQuarantine(_ pluginId: String) {
        ...
        NSLog("[Muwa] Quarantined plugin '%@' after crash during load", pluginId)
    }
    ...
    nonisolated static func toolsDirectoryURLsWithFailures() -> (urls: [URL], failures: [String: String]) {
        ...
        for pluginDir in pluginDirs where pluginDir.hasDirectoryPath {
            ...
            if quarantined.contains(pluginId) {
                ...
                // `muwa-cli tools reset` CLI (which unquarantines all
