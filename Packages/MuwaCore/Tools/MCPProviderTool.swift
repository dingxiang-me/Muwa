...
final class MCPProviderTool: MuwaTool, PermissionedTool, @unchecked Sendable {
    let name: String
    ...
    /// Convert MCP Value schema to Muwa JSONValue
...
extension MCPProviderTool {
    ...
    private static func convertToMCPValue(_ value: Any) throws -> MCP.Value {
        ...
        default:
            ...
            if let jsonData = try? JSONSerialization.data(withJSONObject: value, options: .muwaCanonical),
...
extension MCPProviderTool {
    ...
    static func convertMCPContent(_ content: [MCP.Tool.Content]) -> String {
        ...
        if let jsonData = try? JSONSerialization.data(withJSONObject: results, options: .muwaCanonical),
            let jsonString = String(data: jsonData, encoding: .utf8)
        {
            return jsonString
        }
