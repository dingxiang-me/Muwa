...
//  inspect Muwa's current configuration:
//
//   - muwa_status   — one-shot snapshot + suggestions
//   - muwa_list     — list items in a scope
//   - muwa_describe — full detail for one item
//
...
public final class MuwaStatusTool: MuwaTool, @unchecked Sendable {
    public let name = "muwa_status"
    public let description =
        "One-shot snapshot of Muwa configuration: default agent, hardware, models, providers, "
        + "plugins, schedules. Returns `suggestions` derived from the snapshot — call this first when "
        + "the user says 'help me set up Muwa' or asks what's configured."
    ...
    public func execute(argumentsJSON: String) async throws -> String {
        ...
        let envelope: String = await MainActor.run {
            ...
            if providerCount == 0 && installedModels.isEmpty {
                suggestions.append(
                    "No providers or local models configured — call muwa_list({scope: 'models', filter: 'recommended'}) for top picks."
                )
            } else {
                if providerCount == 0 {
                    suggestions.append(
            ...
            if !downloadingModels.isEmpty {
                suggestions.append("\(downloadingModels.count) model(s) downloading — poll muwa_status again.")
            }
...
public final class MuwaListTool: MuwaTool, @unchecked Sendable {
    public let name = "muwa_list"
    public let description =
...
public final class MuwaDescribeTool: MuwaTool, @unchecked Sendable {
    public let name = "muwa_describe"
    public let description =
        "Full detail for one item in a configuration scope. Same scopes as muwa_list. "
