...
final class CapabilitiesDiscoverTool: MuwaTool, @unchecked Sendable {
    ...
    func execute(argumentsJSON: String) async throws -> String {
        ...
        //     so search returns ONLY `muwa_*_<verb>` candidates. The
...
final class CapabilitiesLoadTool: MuwaTool, @unchecked Sendable {
            ...
            return .failure(
                LoadFailure(
                    ...
                    message:
                        ...
                        + "Use `capabilities_discover` to find a configuration tool (muwa_*_<verb>) "
                ...
                return .failure(
                    LoadFailure(
                        ...
                        message:
                            ...
                            + "(`muwa_*_<verb>`). Use `muwa_status`, `muwa_list`, or "
                            + "`muwa_describe` for reads; nothing else needs `capabilities_load`."
                    )
                )
            }
        }
