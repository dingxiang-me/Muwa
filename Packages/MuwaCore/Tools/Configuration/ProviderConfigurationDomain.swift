...
//   - muwa_provider_add            (opens credential sheet)
//   - muwa_provider_update         (non-secret fields only)
//   - muwa_provider_remove
//   - muwa_provider_set_credentials (key rotation)
//
...
enum ProviderConfigurationDomain {
    static let domain = ConfigurationDomain(
        ...
        tools: [
            MuwaProviderAddTool(),
            MuwaProviderUpdateTool(),
            MuwaProviderRemoveTool(),
            MuwaProviderSetCredentialsTool(),
        ],
        writeToolNames: [
            "muwa_provider_add",
            "muwa_provider_update",
            "muwa_provider_remove",
            "muwa_provider_set_credentials",
        ]
    )
}
...
/// `.muwaAgent` is a peer agent rather than a third-party vendor).
...
public final class MuwaProviderAddTool: MuwaTool, PermissionedTool, @unchecked Sendable {
    public let name = "muwa_provider_add"
    public let description =
        "Add a cloud LLM provider. Pass `name` and `provider` "
        + "(anthropic, openai, codex_oauth, azure_openai, google, xai, deepseek, "
        + "venice, openrouter, ollama, custom, muwa_agent). "
            ...
            "provider": .object([
                ...
                "description": .string(
                    "One of: anthropic, openai, codex_oauth, azure_openai, google, xai, "
                        + "deepseek, venice, openrouter, ollama, custom, muwa_agent. "
        ...
        case .apiKey(let key, let headers):
            ...
            return ToolEnvelope.success(
                ...
                result: [
                    ...
                    "connecting": true,
                    "next_steps": [
                        "Use muwa_describe({scope: 'providers', id: '\(providerId.uuidString)'}) "
    ...
    /// and never reaches this helper; `muwaAgent` defaults match the
    /// pairing port).
...
// MARK: - muwa_provider_update

public final class MuwaProviderUpdateTool: MuwaTool, PermissionedTool, @unchecked Sendable {
    public let name = "muwa_provider_update"
    public let description =
        ...
        + "or OAuth tokens, call muwa_provider_set_credentials instead."
    public let parameters: JSONValue? = .object([
        "type": .string("object"),
        "additionalProperties": .bool(false),
        "properties": .object([
    ...
    public func execute(argumentsJSON: String) async throws -> String {
        ...
        let outcome: String = await MainActor.run {
            ...
            if let raw = args["manual_model_ids"] as? String {
                provider.manualModelIds = MuwaProviderAddTool.parseManualModelIds(raw)
            }
...
// MARK: - muwa_provider_remove

public final class MuwaProviderRemoveTool: MuwaTool, PermissionedTool, @unchecked Sendable {
    public let name = "muwa_provider_remove"
    public let description =
...
// MARK: - muwa_provider_set_credentials

public final class MuwaProviderSetCredentialsTool: MuwaTool, PermissionedTool, @unchecked Sendable {
    public let name = "muwa_provider_set_credentials"
    public let description =
        "Re-enter credentials for an existing provider (key rotation / fix wrong key). "
        + "Opens the same secure sheet as muwa_provider_add."
    public let parameters: JSONValue? = .object([
        "type": .string("object"),
        "additionalProperties": .bool(false),
        "properties": .object(["id": .object(["type": .string("string")])]),
