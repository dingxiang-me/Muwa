// muwa_plugin.h
//
// Muwa Plugin ABI — current documented surface is v5.
//
// COMPATIBILITY
// =============
// Both legacy entry points continue to load:
//
//   - muwa_plugin_entry      (v1 — never received the host API)
//   - muwa_plugin_entry_v2   (current — receives `osr_host_api*`)
//
// New plugins should target v5 by exporting `muwa_plugin_entry_v2`
...
#ifndef MUWA_PLUGIN_H
#define MUWA_PLUGIN_H
...
// Inference — routes through the Muwa unified inference layer.
...
// paths (`~/.muwa/artifacts/`) for security; max 50 MB.
...
typedef struct {
    ...
    // `request_json`: JSON-encoded MuwaHTTPRequest.
    // Returns: JSON-encoded MuwaHTTPResponse. Host must call `free_string`.
...
const osr_plugin_api* muwa_plugin_entry(void);

// CURRENT (v2 entry, v3 surface): Receives host-provided callbacks. Muwa
// tries this symbol first. If the plugin was compiled against v1, this
// symbol won't exist and Muwa falls back to `muwa_plugin_entry`.
// New plugins should:
//   - Export this symbol
