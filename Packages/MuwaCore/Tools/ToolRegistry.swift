...
private let toolBodyTimeoutQueue = DispatchQueue(label: "ai.muwa.tool-registry.timeout")
...
final class ToolRegistry: ObservableObject {
    ...
    @Published private var toolsByName: [String: MuwaTool] = [:]
    ...
    private static func estimateTokenCount(_ tool: MuwaTool) -> Int {
    ...
    nonisolated static func externalSurfaceDenialEnvelope(tool: String) -> String {
        ToolEnvelope.failure(
            kind: .rejected,
            message:
                "'\(tool)' is not available to external callers. Folder write and shell tools can only run from the Muwa app.",
            tool: tool
        )
    }
            ...
            throw NSError(
                ...
                userInfo: [
                    NSLocalizedDescriptionKey:
                        "'\(name)' is not available to external callers. Folder write and shell tools can only run from the Muwa app."
                ]
            )
        }
    ...
    private func runPermissionGate(tool: MuwaTool, name: String, argumentsJSON: String) async throws {
    ...
    private var combinedSandboxReadBridge: SandboxReadBridge? {
        ...
        return SandboxReadBridge(
            agentName: agentName,
            home: MuwaPaths.inContainerAgentHome(agentName)
        )
    }
    ...
    func registerSandboxTool(_ tool: MuwaTool, runtimeManaged: Bool = false) {
        let firstTime =
    ...
    func registerMCPTool(_ tool: MuwaTool) {
        let firstTime =
            toolsByName[tool.name] == nil
            && !configuration.enabled.keys.contains(tool.name)
        toolsByName[tool.name] = tool
    ...
    func registerPluginTool(_ tool: MuwaTool) {
        let firstTime =
            toolsByName[tool.name] == nil
            && !configuration.enabled.keys.contains(tool.name)
        toolsByName[tool.name] = tool
...
// its turn-1 schema and which `muwa_*_<verb>` writes are loaded on
...
extension ToolRegistry {
    ...
    /// generic reads (`muwa_status`, `muwa_list`,
    /// `muwa_describe`) plus every write across every domain. Used
