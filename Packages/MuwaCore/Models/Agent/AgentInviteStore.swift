...
//
//  One JSON file per agent at `~/.muwa/agent-invites/<id>.json`. The
...
public enum AgentInviteStore {
    ...
    private static func ledgerURL(for agentId: UUID) -> URL {
        MuwaPaths.agentInvites().appendingPathComponent("\(agentId.uuidString).json")
    }
    ...
    private static func load(for agentId: UUID) -> [IssuedInviteRecord] {
        ...
        do {
            ...
            print("[Muwa] Failed to load invite ledger for \(agentId): \(error)")
            return []
        }
    }
    ...
    private static func save(_ records: [IssuedInviteRecord], for agentId: UUID) {
        ...
        do {
            ...
            print("[Muwa] Failed to save invite ledger for \(agentId): \(error)")
        }
    }
}
