...
//  `~/.muwa/config/default-agent.json`.
//
...
public enum DefaultAgentConfigurationStore {
    ...
    /// `~/.muwa/config/` set this to a sandboxed `tmp` URL before
    /// calling `load()`.
    ...
    private static func configFileURL() -> URL {
        ...
        return MuwaPaths.config().appendingPathComponent("default-agent.json")
    }
    ...
    private static func loadFromDisk() -> DefaultAgentConfiguration {
        ...
        if FileManager.default.fileExists(atPath: url.path) {
            ...
            print("[Muwa] Failed to decode default-agent.json — using defaults (file preserved)")
    ...
    private static func saveToDisk(_ configuration: DefaultAgentConfiguration) {
        ...
        do {
            ...
            print("[Muwa] Failed to save default-agent.json: \(error)")
