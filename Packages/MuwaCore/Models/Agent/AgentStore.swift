...
public enum AgentStore {
    ...
    public static func loadAll() -> [Agent] {
        ...
        for file in files where file.pathExtension == "json" {
            do {
                ...
                print("[Muwa] Failed to load agent from \(file.lastPathComponent): \(error)")
            }
        }
    ...
    public static func load(id: UUID) -> Agent? {
        ...
        do {
            ...
            print("[Muwa] Failed to load agent \(id): \(error)")
            return nil
        }
    }
    ...
    public static func save(_ agent: Agent) {
        guard !agent.isBuiltIn else {
            print("[Muwa] Cannot save built-in agent: \(agent.name)")
            return
        }
        ...
        do {
            ...
            print("[Muwa] Failed to save agent \(agent.id): \(error)")
        }
    }
    ...
    public static func delete(id: UUID) -> Bool {
        if Agent.builtInAgents.contains(where: { $0.id == id }) {
            print("[Muwa] Cannot delete built-in agent")
            return false
        }
        ...
        do {
            ...
            print("[Muwa] Failed to delete agent \(id): \(error)")
            return false
        }
    }
    ...
    public static func writeCustomAvatar(_ data: Data, ext: String, for agentId: UUID) -> String? {
        ...
        MuwaPaths.ensureExistsSilent(dir)
        ...
        do {
            ...
            print("[Muwa] Failed to write custom avatar for \(agentId): \(error)")
            return nil
        }
    }
    ...
    private static func agentsDirectory() -> URL {
        MuwaPaths.resolvePath(new: MuwaPaths.agents(), legacy: "Personas")
    }
    ...
    private static func avatarsDirectory() -> URL {
        MuwaPaths.agents().appendingPathComponent("avatars", isDirectory: true)
    }
}
