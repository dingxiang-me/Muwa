...
public struct AgentQuickAction: Codable, Identifiable, Sendable, Equatable {
    ...
    /// Setup-oriented quick actions for the built-in Muwa configuration
    ...
    public static var defaultConfigurationQuickActions: [AgentQuickAction] {
        [
            AgentQuickAction(
                ...
                prompt: L("What's currently configured in Muwa?")
            ),
            AgentQuickAction(
...
public struct Agent: Codable, Identifiable, Sendable, Equatable {
    ...
    /// `MuwaPaths.agents()/avatars/`. When set, takes precedence over
    ...
    /// Well-known UUID for the default Muwa agent
    ...
    /// The default agent — front door to configuring Muwa.
    /// Renders as "Muwa" in chat and the picker; subtitle nudges
    /// users toward the configure flow that's unique to this agent.
    /// `avatar: "green"` resolves the bundled `muwa-avatar-green`
