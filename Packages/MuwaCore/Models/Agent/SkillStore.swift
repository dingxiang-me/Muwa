...
public enum SkillStore {
    ...
    public static func save(_ skill: Skill) async {
        ...
        do {
            ...
            print("[Muwa] Failed to save skill \(skill.id): \(error)")
        }
    }
    ...
    private static func loadFromDirectory(_ directoryURL: URL) -> Skill? {
        ...
        do {
            ...
            print("[Muwa] Failed to load skill from \(directoryURL.lastPathComponent): \(error)")
            return nil
        }
    }
    ...
    private static func saveBuiltInState(_ skill: Skill) {
        ...
        do {
            ...
            print("[Muwa] Failed to save built-in skill state: \(error)")
        }
    }
    ...
    private static func migrateOldFormat() {
        ...
        for file in files where file.pathExtension == "md" {
            do {
                ...
                try FileManager.default.removeItem(at: file)
                print("[Muwa] Migrated skill: \(skill.name)")
            } catch {
                print("[Muwa] Failed to migrate \(file.lastPathComponent): \(error)")
            }
        }
    }
}
