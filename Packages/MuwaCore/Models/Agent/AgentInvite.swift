...
public enum AgentInviteError: Error, LocalizedError {
    ...
    public var errorDescription: String? {
        switch self {
        case .identityMissing:
            return "No Muwa identity is configured on this device."
        case .agentMissingIdentity:
...
///   "name": "Muwa",
///   "desc": "optional brief",
///   "url":  "https://0xabcd....agent.muwa.ai",
...
public struct AgentInvite: Codable, Sendable, Equatable {
    ...
    /// `Muwa Signed Invite` domain prefix over the canonical signing string.
    public let sig: String

    public static let currentVersion: Int = 1
    public static let signingDomain: String = "Muwa Signed Invite"
    ...
    public static func signingPayload(addr: String, nonce: String, exp: Int64) -> Data {
        Data("muwa-agent-invite-v1:\(addr):\(nonce):\(exp)".utf8)
    }
    ...
    public func deeplinkURL() throws -> URL {
        ...
        components.scheme = ProductBrand.urlScheme
    ...
    /// Decode an invite from an `muwa://` deeplink.
    public static func decode(from url: URL) throws -> AgentInvite {
        guard url.scheme?.lowercased() == ProductBrand.urlScheme,
