...
/// - max: raw DSV4 max reasoning effort; Muwa passes it through to vmlx
...
/// kwarg. Muwa exposes the toggle but does not synthesize a reasoning mode:
...
/// reads an `enable_thinking` Jinja kwarg. Muwa exposes the native switch
...
/// directive. Muwa only forwards explicit user/API choices; this is a
