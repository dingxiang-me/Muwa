...
public enum RemoteProviderType: String, Codable, Sendable, CaseIterable {
    ...
    case muwa = "muwa"  // Native Muwa agent — full server-side execution via /agents/{id}/run
    ...
    public var chatEndpoint: String {
        ...
        case .muwa: return "/run"  // Unused — full URL built by RemoteProviderService.buildURLRequest
        }
    }
...
public struct RemoteProvider: Codable, Identifiable, Sendable, Equatable {
    ...
    /// The UUID of the agent on the remote Muwa server. Only used when providerType == .muwa.
    public var remoteAgentId: UUID?

    /// The crypto address (e.g. "0x...") of the remote agent, used to build relay tunnel URLs.
    /// Only used when providerType == .muwa.
    ...
    public func resolvedHeaders() -> [String: String] {
        ...
        // OpenRouter app attribution: surfaces Muwa on openrouter.ai/rankings.
...
public enum RemoteProviderConfigurationStore {
    public static func load() -> RemoteProviderConfiguration {
        ...
        do {
            ...
            print("[Muwa] Failed to load RemoteProviderConfiguration: \(error)")
            return RemoteProviderConfiguration()
        }
    }
    ...
    public static func save(_ configuration: RemoteProviderConfiguration) {
        ...
        do {
            ...
            print("[Muwa] Failed to save RemoteProviderConfiguration: \(error)")
        }
    }

    private static func configurationFileURL() -> URL {
        MuwaPaths.resolvePath(
            new: MuwaPaths.remoteProviderConfigFile(),
