...
public final class AppConfiguration: ObservableObject {
    ...
    private static func loadFromDisk() -> ChatConfiguration {
        ...
        do {
            ...
            print("[Muwa] Failed to load ChatConfiguration: \(error)")
            return ChatConfiguration.default
        }
    ...
    static func clearUnavailableFoundationCoreModel(
        ...
        print(
            "[Muwa] Cleared persisted core model 'foundation' "
    ...
    private static func saveToDisk(_ config: ChatConfiguration) {
        ...
        do {
            ...
            print("[Muwa] Failed to save ChatConfiguration: \(error)")
        }
    }

    private static func configFileURL() -> URL {
        MuwaPaths.resolvePath(new: MuwaPaths.chatConfigFile(), legacy: "ChatConfiguration.json")
    }
    ...
    static func migrateCoreModelFromMemoryConfig(into config: ChatConfiguration) -> ChatConfiguration {
        guard let json = loadJSONObject(from: MuwaPaths.memoryConfigFile()) else { return config }
        ...
        print(
            "[Muwa] Migrated core model from memory.json: "
    ...
    /// from `~/.muwa/config/memory.json`. They were moved to
    ...
    static func stripLegacyCoreModelKeysFromMemoryConfig() {
        let memoryURL = MuwaPaths.memoryConfigFile()
        ...
        do {
            ...
            print("[Muwa] Removed legacy core-model keys from memory.json")
        } catch {
            print("[Muwa] Failed to scrub memory.json legacy keys: \(error)")
        }
    }
