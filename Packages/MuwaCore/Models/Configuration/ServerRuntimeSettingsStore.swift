...
//
//  Storage layout: `~/.muwa/config/server-runtime.json`
//
...
//  so the existing `MuwaServer` start path keeps working unchanged.
//
...
public enum ServerRuntimeSettingsStore {
    ...
    public nonisolated static func load() -> VMLXServerRuntimeSettings? {
        ...
        do {
            ...
            print("[Muwa] Failed to load ServerRuntimeSettings: \(error)")
            return nil
        }
    }
    ...
    public nonisolated static func save(_ settings: VMLXServerRuntimeSettings) {
        ...
        do {
            ...
            print("[Muwa] Failed to save ServerRuntimeSettings: \(error)")
        }
    }
    ...
    private nonisolated static func normalizeLoadedSettings(
        ...
        // "auto". Existing Muwa installs persisted the old default exactly,
    ...
    public nonisolated static func migratedFromLegacy(
        ...
        let rawMaxBatch = userDefaults.integer(
            forKey: "ai.muwa.scheduler.mlxBatchEngineMaxBatchSize"
        )
