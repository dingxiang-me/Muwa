...
//
//  Adding a new provider (for example a future hosted "Muwa API") is a
...
/// Muwa API provider is a single line here once it has a `ProviderPreset`
/// case whose `configuration.providerType` is `RemoteProviderType.muwa`
/// (already defined) and, if it uses browser sign-in, a new
/// `ProviderOAuthKind.muwa` carrying its CTA/subtitle copy:
/// ```swift
/// // OAuth-first, also accepts a pasted key:
/// ProviderCatalogEntry(.muwa, authMethods: [.oauth(.muwa), .apiKey], placement: .oauthTopLevel)
/// // or API-key only:
/// ProviderCatalogEntry(.muwa, authMethods: [.apiKey], placement: .apiKey)
/// ```
