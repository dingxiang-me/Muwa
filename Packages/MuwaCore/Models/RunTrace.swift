...
//  `~/.muwa/agents/<id>/runs/<run_id>.json` when a DB-enabled
...
public enum RunTraceWriter {
    ...
    /// `MuwaPaths.agentRunTraceFile(agentId:runId:)`. Creates the
    ...
    public static func write(_ trace: RunTrace) -> Bool {
        let url = MuwaPaths.agentRunTraceFile(agentId: trace.agentId, runId: trace.runId)
