...
// MARK: - C ABI Mirror
//
// Swift mirror of the C struct in `Tools/PluginABI/muwa_plugin.h`.
...
/// Frozen layout — field order and padding must match `muwa_plugin.h`
...
public struct PluginManifest: Decodable, Sendable {
    ...
    public enum ConfigDefault: Decodable, Sendable {
        ...
        public var stringValue: String {
            ...
            case .stringArray(let a):
                let data = (try? JSONSerialization.data(withJSONObject: a, options: .muwaCanonical)) ?? Data()
    ...
    public struct WebSpec: Decodable, Sendable {
        ...
        public let auth: RouteAuth
        /// Optional custom API mount injected into `window.__muwa.apiUrl`.
...
final class ExternalPlugin: @unchecked Sendable {
    ...
    /// `on_config_changed` in `muwa_plugin.h`.
    ...
    private func eventQueue(for taskId: String) -> DispatchQueue {
        taskEventQueuesLock.withLock {
            ...
            let q = DispatchQueue(
                label: "com.muwa.plugin.events.\(id).\(taskId)",
                qos: .utility
            )
    ...
    init(
        ...
        self.invokeQueue = DispatchQueue(
            label: "com.muwa.plugin.invoke.\(manifest.plugin_id)",
            qos: .userInitiated,
            attributes: .concurrent
        )
        self.configEventQueue = DispatchQueue(
            label: "com.muwa.plugin.config.\(manifest.plugin_id)",
            qos: .userInitiated
        )
    }
