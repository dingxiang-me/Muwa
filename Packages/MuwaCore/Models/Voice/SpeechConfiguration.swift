...
/// Where speech-to-text runs: on-device FluidAudio models or a cloud service.
public enum SpeechTranscriptionProvider: String, Codable, Equatable, CaseIterable, Sendable {
    /// On-device Parakeet models via FluidAudio
    case local
    /// Volcengine (Doubao) BigModel streaming ASR
    case volcengine

    public var displayName: String {
        switch self {
        case .local: return L("本地模型")
        case .volcengine: return L("火山引擎（豆包）")
        }
    }

    public var description: String {
        switch self {
        case .local: return L("使用已下载模型进行完全离线转写")
        case .volcengine: return L("在线流式识别，中文效果更好")
        }
    }
}
...
public struct SpeechConfiguration: Codable, Equatable, Sendable {
    ...
    public var transcriptionProvider: SpeechTranscriptionProvider

    /// Volcengine resource ID, e.g. "volc.seedasr.sauc.duration"
    public var volcengineResourceId: String

    /// Optional language hint for Volcengine ASR (empty = auto detect)
    public var volcengineLanguage: String
    ...
    public init(from decoder: Decoder) throws {
        ...
        self.transcriptionProvider =
            try container.decodeIfPresent(SpeechTranscriptionProvider.self, forKey: .transcriptionProvider)
            ?? defaults.transcriptionProvider
        self.volcengineResourceId =
            try container.decodeIfPresent(String.self, forKey: .volcengineResourceId)
            ?? defaults.volcengineResourceId
        self.volcengineLanguage =
            try container.decodeIfPresent(String.self, forKey: .volcengineLanguage)
            ?? defaults.volcengineLanguage
    }
    ...
    public init(
        ...
        transcriptionProvider: SpeechTranscriptionProvider = .local,
        volcengineResourceId: String = VolcengineASRSettings.defaultResourceId,
        ...
        self.volcengineLanguage = volcengineLanguage
    }
...
public enum SpeechConfigurationStore {
    ...
    private static func loadFromDisk() -> SpeechConfiguration {
        ...
        do {
            ...
            print("[Muwa] Failed to load SpeechConfiguration: \(error)")
            return SpeechConfiguration.default
        }
    }
    ...
    private static func saveToDisk(_ configuration: SpeechConfiguration) {
        ...
        do {
            ...
            print("[Muwa] Failed to save SpeechConfiguration: \(error)")
        }
    }

    private static func configurationFileURL() -> URL {
        MuwaPaths.speechConfigFile()
    }
}
