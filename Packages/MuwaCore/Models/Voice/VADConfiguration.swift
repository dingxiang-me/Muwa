...
public struct VADConfiguration: Codable, Equatable, Sendable {
    ...
    /// Custom wake phrase (e.g., "Hey Muwa"). Empty = use agent names only
...
public enum VADConfigurationStore {
    public static func load() -> VADConfiguration {
        ...
        do {
            ...
            print("[Muwa] Failed to load VADConfiguration: \(error)")
            return VADConfiguration.default
        }
    }
    ...
    public static func save(_ configuration: VADConfiguration) {
        ...
        do {
            ...
            print("[Muwa] Failed to save VADConfiguration: \(error)")
        }
    }

    private static func configurationFileURL() -> URL {
        MuwaPaths.resolvePath(new: MuwaPaths.vadConfigFile(), legacy: "VADConfiguration.json")
    }
}
