...
public struct SharedArtifact: Identifiable, Codable, Sendable, Equatable {
    ...
    /// Absolute path on the host filesystem (~/.muwa/artifacts/{contextId}/{filename})
...
extension SharedArtifact {
    ...
    /// `~/.muwa/artifacts/{contextId}/` and are referenced by the
    ...
    static func processToolResultDetailed(
        ...
        let contextDir = MuwaPaths.contextArtifactsDir(contextId: contextId)
        MuwaPaths.ensureExistsSilent(contextDir)
        ...
        if hasContent {
            ...
            case .candidate(let url, let attempted):
                ...
                let fileSize =
                    isDirectory
                    ? MuwaPaths.directorySize(at: destPath)
    ...
    fileprivate static func resolveSourcePathDetailed(
        ...
        case .sandbox:
            ...
            let agentDir = MuwaPaths.containerAgentDir(agent)
            let containerHome = MuwaPaths.inContainerAgentHome(agent)
            ...
            if relativePath.hasPrefix(containerHome + "/") {
                ...
                if let resolved = resolveContainedPath(stripped, within: MuwaPaths.containerWorkspace()) {
    ...
    private static func rebuildToolResult(
        ...
        var inner = ""
        if let jsonData = try? JSONSerialization.data(withJSONObject: parsed.metadata, options: .muwaCanonical),
