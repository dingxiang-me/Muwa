...
struct OpenAIModel: Codable, Sendable {
    ...
    init(from decoder: Decoder) throws {
        ...
        // Muwa model discovery, so preserve the model row instead of
...
extension Tool {
    ...
    /// prompt is enforced at *encode time* by `JSONEncoder.muwaCanonical()`
    /// / `.muwaCanonical` writing options (see
    ...
    func canonicalHashPayload() -> Data {
        ...
        if JSONSerialization.isValidJSONObject(spec),
            let data = try? JSONSerialization.data(withJSONObject: spec, options: .muwaCanonical)
        {
            return data
        }
    ...
    fileprivate static func canonicalize(_ value: [String: any Sendable]) -> [String: any Sendable]? {
        if JSONSerialization.isValidJSONObject(value),
            let data = try? JSONSerialization.data(withJSONObject: value, options: .muwaCanonical),
