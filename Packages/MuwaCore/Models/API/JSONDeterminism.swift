...
//  Muwa
//
//  Centralised canonical-encoding contract for any JSON Muwa emits where
...
extension JSONEncoder {
    ...
    /// path. Grep for `muwaCanonical` to find existing call sites.
    static func muwaCanonical(prettyPrinted: Bool = false) -> JSONEncoder {
...
extension JSONSerialization.WritingOptions {
    ...
    /// Same determinism contract as `JSONEncoder.muwaCanonical`.
    static let muwaCanonical: JSONSerialization.WritingOptions = [.sortedKeys, .withoutEscapingSlashes]
}
...
/// encode time by `.muwaCanonical`; the walker's job is to ensure every
