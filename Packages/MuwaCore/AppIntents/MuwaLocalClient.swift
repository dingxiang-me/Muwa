...
public enum MuwaLocalClientError: LocalizedError {
    ...
    public var errorDescription: String? {
        switch self {
        case .serverUnreachable:
            return "Muwa isn't reachable. Open Muwa and make sure its local server is running."
        case .badResponse(let status, let body):
            let trimmed = body.trimmingCharacters(in: .whitespacesAndNewlines)
            if trimmed.isEmpty {
                return "Muwa returned an unexpected response (HTTP \(status))."
            }
            return "Muwa returned an error (HTTP \(status)): \(trimmed)"
        case .emptyReply:
            return "Muwa didn't return a response."
        }
    }
}
...
public final class MuwaLocalClient: Sendable {
    public static let shared = MuwaLocalClient()
    ...
    /// The currently active agent's id. "Ask Muwa" targets whatever agent
    ...
    /// "Muwa" agent when that is the active one — which is why the run/dispatch
    ...
    public func runAgent(id: String, prompt: String?) async throws -> String {
        ...
        if let http = response as? HTTPURLResponse, http.statusCode != 200 {
            ...
            throw MuwaLocalClientError.badResponse(status: http.statusCode, body: collected)
        }
        ...
        for try await line in bytes.lines {
            ...
            {
                throw MuwaLocalClientError.badResponse(status: 200, body: message)
            }
        ...
        guard !trimmed.isEmpty else { throw MuwaLocalClientError.emptyReply }
        return trimmed
    }
    ...
    public func startAgent(id: String, input: String?) async throws {
        ...
        guard let http = response as? HTTPURLResponse else {
            throw MuwaLocalClientError.serverUnreachable
        }
        // Dispatch returns 202 Accepted on success.
        guard http.statusCode == 202 else {
            throw MuwaLocalClientError.badResponse(
