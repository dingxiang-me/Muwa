...
//  Muwa
//
//  Enumerates every at-rest SQLCipher database under `~/.muwa/`:
...
public enum StorageDatabaseCatalog {
    ...
    public static func databaseTargets() -> [DatabaseTarget] {
        var targets: [DatabaseTarget] = [
            .init(label: "chat history", path: MuwaPaths.chatHistoryDatabaseFile().path),
            .init(label: "memory", path: MuwaPaths.memoryDatabaseFile().path),
            .init(label: "methods", path: MuwaPaths.methodsDatabaseFile().path),
            .init(label: "tool index", path: MuwaPaths.toolIndexDatabaseFile().path),
        ...
        if let plugins = try? FileManager.default.contentsOfDirectory(at: toolsDir, includingPropertiesForKeys: nil) {
            for plugin in plugins {
                ...
                // MuwaCore test suite without isolating
                // `MuwaPaths.overrideRoot`. End users will never
                // have one, so filter them out.
                if Self.isLeakedTestPluginId(pluginId) { continue }
                let dbPath = MuwaPaths.pluginDatabaseFile(for: pluginId).path
        ...
        let agentsDir = MuwaPaths.agents()
