...
//  Muwa
//
//  Periodic SQLite maintenance for the five Muwa databases. Runs:
//
...
//  in `~/.muwa/.storage-maintenance.json` so a long-lived process
...
public actor StorageMaintenance {
    ...
    private let log = Logger(subsystem: "ai.muwa", category: "storage.maintenance")
    ...
    private func runOptimize(force: Bool) async {
        ...
        for db in MuwaDatabaseHandle.allOpenHandles {
    ...
    private func stateURL() -> URL {
        MuwaPaths.root().appendingPathComponent(".storage-maintenance.json")
    }
...
public final class MuwaDatabaseHandle: @unchecked Sendable {
    ...
    nonisolated(unsafe) private static var handles: [String: MuwaDatabaseHandle] = [:]

    /// Snapshot of every currently registered handle.
    public static var allOpenHandles: [MuwaDatabaseHandle] {
        lock.lock()
        defer { lock.unlock() }
        return Array(handles.values)
    }

    public static func register(_ handle: MuwaDatabaseHandle) {
        lock.lock()
        defer { lock.unlock() }
