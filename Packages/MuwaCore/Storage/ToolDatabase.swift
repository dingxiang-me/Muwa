...
public final class ToolDatabase: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "ai.muwa.tools.database")
    ...
    public func open() throws {
        ...
        try queue.sync {
            ...
            MuwaPaths.ensureExistsSilent(MuwaPaths.toolIndex())
            try openConnection()
            try runMigrations()
        }
    ...
    public func close() {
        MuwaDatabaseHandle.deregister(name: "tool-index")
        queue.sync {
            stmtCache.clear()
    ...
    private func openConnection() throws {
        let path = MuwaPaths.toolIndexDatabaseFile().path
