...
final class PluginDatabase: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "ai.muwa.plugin.db")
    ...
    /// from a runaway plugin filling up `~/.muwa/Tools/<id>/data/`.
    /// Documented under `db_exec` in `muwa_plugin.h`.
    ...
    func open() throws {
        ...
        // DBs with `MuwaDatabaseHandle.register(...)`. Users may
        ...
        // independently walks `~/.muwa/Tools/<plugin>/data/data.db`
        // from disk.
        ...
        try queue.sync {
            ...
            MuwaPaths.ensureExistsSilent(MuwaPaths.pluginDataDirectory(for: pluginId))
            let path = MuwaPaths.pluginDatabaseFile(for: pluginId).path
    ...
    private func bindParams(stmt: OpaquePointer, paramsJSON: String) {
        ...
        for (i, param) in arr.enumerated() {
            ...
            default:
                if let jsonData = try? JSONSerialization.data(withJSONObject: param, options: .muwaCanonical),
