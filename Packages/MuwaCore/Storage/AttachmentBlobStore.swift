...
//  `Self.spillThreshold` to `~/.muwa/chat-history/blobs/<sha256>.osec`,
...
public enum AttachmentBlobStore {
    ...
    private static let log = Logger(subsystem: "ai.muwa", category: "storage.blobs")

    // MARK: - Disk layout

    /// `~/.muwa/chat-history/blobs/`
    public static func blobsDir() -> URL {
        MuwaPaths.chatHistory().appendingPathComponent("blobs", isDirectory: true)
    }

    /// `~/.muwa/chat-history/blobs/<sha256>.osec`
