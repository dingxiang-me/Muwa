...
//  (`MuwaSQLCipher`). The data-encryption key comes from
...
public final class ChatHistoryDatabase: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "ai.muwa.chatHistory.database")
    ...
    public func open() throws {
        ...
        try queue.sync {
            ...
            MuwaPaths.ensureExistsSilent(MuwaPaths.chatHistory())
            try openConnection()
            try runMigrations()
        }
    ...
    public func close() {
        MuwaDatabaseHandle.deregister(name: "chat-history")
        queue.sync {
            stmtCache.clear()
    ...
    private func openConnection() throws {
        let path = MuwaPaths.chatHistoryDatabaseFile().path
