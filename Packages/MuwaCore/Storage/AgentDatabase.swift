...
//  `~/.muwa/agents/<id>/db.sqlite`. Encrypted via the same SQLCipher
...
public final class AgentDatabase: @unchecked Sendable {
    ...
    /// Path on disk. Stored eagerly so tests with `MuwaPaths.overrideRoot`
    ...
    public init(agentId: UUID, path: String? = nil) {
        ...
        self.path = path ?? MuwaPaths.agentDatabaseFile(for: agentId).path
        // Label distinguishes per-agent queues in profiler traces.
        self.queue = DispatchQueue(label: "ai.muwa.agentdb.\(agentId.uuidString)")
    }
    ...
    public func open() throws {
        ...
        try queue.sync {
            ...
            MuwaPaths.ensureExistsSilent(MuwaPaths.agentDirectory(for: agentId))
            try openConnection()
            try runMigrations()
        }
    }
