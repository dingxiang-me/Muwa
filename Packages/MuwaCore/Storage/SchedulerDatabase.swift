...
//  spec §4.2. One file at `~/.muwa/scheduler.sqlite`, not per-agent —
...
//  Muwa stack uses, so prompts / instructions / error messages in
...
public final class SchedulerDatabase: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "ai.muwa.scheduler.database")
    ...
    public func open() throws {
        ...
        MuwaDatabaseHandle.register(maintenanceHandle)
    }

    private lazy var maintenanceHandle = MuwaDatabaseHandle(
        name: "scheduler",
    ...
    public func close() {
        MuwaDatabaseHandle.deregister(name: "scheduler")
        queue.sync {
            stmtCache.clear()
    ...
    private func openConnection() throws {
        let path = MuwaPaths.schedulerDatabaseFile().path
