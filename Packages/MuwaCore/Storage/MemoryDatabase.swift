...
public final class MemoryDatabase: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "ai.muwa.memory.database")
    ...
    public func open() throws {
        ...
        try queue.sync {
            ...
            MuwaPaths.ensureExistsSilent(MuwaPaths.memory())
            do {
                try openConnection()
            } catch {
    ...
    public func close() {
        MuwaDatabaseHandle.deregister(name: "memory")
        queue.sync {
            stmtCache.clear()
    ...
    private func openConnection() throws {
        let path = MuwaPaths.memoryDatabaseFile().path
