...
public final class MethodDatabase: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "ai.muwa.methods.database")
    ...
    public func open() throws {
        ...
        try queue.sync {
            ...
            MuwaPaths.ensureExistsSilent(MuwaPaths.methods())
            try openConnection()
            try runMigrations()
        }
    ...
    public func close() {
        MuwaDatabaseHandle.deregister(name: "methods")
        queue.sync {
            stmtCache.clear()
    ...
    private func openConnection() throws {
        let path = MuwaPaths.methodsDatabaseFile().path
