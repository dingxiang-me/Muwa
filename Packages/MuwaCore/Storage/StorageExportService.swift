...
//      under `~/.muwa/`, decrypt, and write a self-contained
...
public actor StorageExportService {
    ...
    private let log = Logger(subsystem: "ai.muwa", category: "storage.export")
    ...
    /// Walk every encrypted artifact under `~/.muwa/`, decrypt
    ...
    public func exportPlaintextBackup(to destination: URL) async throws -> ExportSummary {
        ...
        let osecRoots = [
            ...
            MuwaPaths.providers(),
            MuwaPaths.config(),
            MuwaPaths.sessionsArchive(),
            MuwaPaths.sandboxPluginLibrary(),
            MuwaPaths.toolSpecs(),
        ]
    ...
    /// `.osec` file under `~/.muwa/`. This is destructive in the
    ...
    private func performRotation() async throws -> SymmetricKey {
        ...
        // the reopen).
        try MuwaDatabaseHandle.withAllHandlesQuiesced {
            for target in StorageDatabaseCatalog.databaseTargets() {
                do {
            ...
            rewrapOSecTree(under: MuwaPaths.root(), oldKey: oldKey, newKey: newKey)
    ...
    private func decryptOSecTree(under url: URL, key: SymmetricKey, into destination: URL) -> Int {
        ...
        for case let f as URL in enumerator {
            ...
            let relPath = relativePath(from: MuwaPaths.root(), to: plain) ?? plain.lastPathComponent
