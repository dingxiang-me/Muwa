...
exposed to Muwa Swift code as the `MuwaSQLCipher` SwiftPM C
target.
...
make sqlite3.c
cp sqlite3.c     <muwa>/Packages/MuwaCore/SQLCipher/sqlite3.c
cp sqlite3.h     <muwa>/Packages/MuwaCore/SQLCipher/include/sqlite3.h
cp sqlite3ext.h  <muwa>/Packages/MuwaCore/SQLCipher/include/sqlite3ext.h
```
...
`include/sqlite3.h` has an MUWA LOCAL MODIFICATION that wraps the
entire `_FTS5_H` block in `#ifndef MUWA_OMIT_FTS5_HEADERS`. Without
...
2. Find the matching `#endif /* _FTS5_H */` line near the end. Insert
   ...
   ```c
   #endif /* MUWA_OMIT_FTS5_HEADERS — END MUWA LOCAL MODIFICATION */
   ```

3. Confirm `MuwaSQLCipher.h` still defines
   `MUWA_OMIT_FTS5_HEADERS` before it includes `sqlite3.h`, and
   `Package.swift` still defines the same flag in the
   `MuwaSQLCipher` target `cSettings`.
...
`include/sqlite3ext.h` has an MUWA LOCAL MODIFICATION that hides the
loadable-extension API behind `MUWA_OMIT_SQLITE_EXTENSION_API`.
`include/MuwaSQLCipher.h` defines that macro before including
`sqlite3ext.h`.

This exists because Muwa imports SQLCipher from Swift for the core
...
1. Find `#include "sqlite3.h"` near the top of the file. Insert the
   `#ifndef MUWA_OMIT_SQLITE_EXTENSION_API` block immediately after
   it.
2. Insert the matching close immediately before the final
   `#endif /* SQLITE3EXT_H */`.
3. Confirm `MuwaSQLCipher.h` still defines
   `MUWA_OMIT_SQLITE_EXTENSION_API` immediately before it includes
   `sqlite3ext.h`.
...
- AES-256 + HMAC-SHA512 + PBKDF2 are implemented by Apple-maintained
  primitives.
- Linker pulls `Security.framework` (already required by Muwa).
...
ours resolve to the statically-linked SQLCipher inside MuwaCore.
