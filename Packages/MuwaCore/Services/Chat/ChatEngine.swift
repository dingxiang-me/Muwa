...
actor ChatEngine: Sendable, ChatEngineProtocol {
    ...
    private static func canonicalToolArgumentsJSON(
        ...
        guard JSONSerialization.isValidJSONObject(coerced),
            let data = try? JSONSerialization.data(withJSONObject: coerced, options: .muwaCanonical),
    ...
    private func wrapStreamWithLogging(
        ...
        let producerTask = Task.detached(priority: .userInitiated) {
            ...
            print("[Muwa][Stream] Starting stream wrapper for model: \(model)")
            ...
            do {
                for try await delta in inner {
                    ...
                    if Task.isCancelled {
                        print("[Muwa][Stream] Task cancelled after \(deltaCount) deltas")
                        continuation.finish()
                        return
                    }
                    ...
                    if deltaCount % 50 == 1 || timeSinceLastDelta > 2.0 {
                        print(
                            "[Muwa][Stream] Delta #\(deltaCount): +\(String(format: "%.2f", timeSinceStart))s total, gap=\(String(format: "%.3f", timeSinceLastDelta))s, len=\(delta.count)"
                        )
                    }
                ...
                print(
                    "[Muwa][Stream] Stream completed: \(deltaCount) deltas in \(String(format: "%.2f", totalTime))s"
                )
                ...
                if deltaCount == 0, let mlxErr = MLXErrorRecovery.errorSince(mlxErrorEpoch) {
                    print("[Muwa][Stream] Empty stream after MLX error: \(mlxErr)")
                    finishReason = .error
                    errorMsg = mlxErr
                ...
                print("[Muwa][Stream] Tool invocations (batch): count=\(invs.invocations.count)")
                ...
                print("[Muwa][Stream] Tool invocation: \(inv.toolName)")
                ...
                if Task.isCancelled || error is CancellationError {
                    print("[Muwa][Stream] Stream cancelled after \(deltaCount) deltas")
                    continuation.finish()
                    return
                }
                print("[Muwa][Stream] Stream error after \(deltaCount) deltas: \(error.localizedDescription)")
                finishReason = .error
        ...
        continuation.onTermination = { @Sendable termination in
            switch termination {
            case .cancelled:
                print("[Muwa][Stream] Consumer cancelled - stopping producer task")
                producerTask.cancel()
            case .finished:
