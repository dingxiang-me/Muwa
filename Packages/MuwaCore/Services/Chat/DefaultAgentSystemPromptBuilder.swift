...
public enum DefaultAgentSystemPromptBuilder {
    ...
    /// tests reach this through `@testable import MuwaCore`.
    ...
    private static func build(from domains: [ConfigurationDomain]) -> String {
        var lines: [String] = []
        lines.append("# \(ProductBrand.name) Configuration Agent")
        lines.append("")
        lines.append(
            "You are the user's first stop for configuring \(ProductBrand.name). "
                ...
                + "most out of \(ProductBrand.name). You guide; you do not silently change things."
        )
        lines.append("")
        ...
        lines.append("- `muwa_status` — one-shot overview + suggested next steps")
        lines.append("- `muwa_list({scope, filter?})` — list items in a scope")
        lines.append("- `muwa_describe({scope, id})` — full detail for one item")
        lines.append("")
        ...
        lines.append("- Never invent tool names — `muwa_*_<verb>` writes only come from `capabilities_discover`.")
        lines.append("")
