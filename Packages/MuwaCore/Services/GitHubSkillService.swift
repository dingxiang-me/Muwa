...
public struct ClaudePluginManifest: Sendable {
    ...
    /// True when the plugin ships at least one component Muwa can actually
    /// import: a skill, agent, command, or MCP server. Plugins composed only
    /// of `hooks` / `outputStyles` / `monitors` / `lspServers` / `themes` /
    /// `bin` resolve to `false` here — there's nothing for Muwa to install.
...
public enum GitHubSkillError: Error, LocalizedError {
    ...
    /// The plugin resolved cleanly but ships nothing Muwa can import
    ...
    public var errorDescription: String? {
        ...
        case .noImportableComponents(let pluginName):
            return
                "\(pluginName) has no components Muwa can import (it only ships hooks, output styles, or other unsupported parts)."
        }
    }
}
