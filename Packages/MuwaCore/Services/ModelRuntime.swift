...
private let genLog = Logger(subsystem: "com.dinoki.muwa", category: "Generation")
...
public actor ModelRuntime {
    ...
    private func finishLoadedContainer(
        ...
        // Cache tier config is entirely muwa-internal — not user-visible.
    ...
    private func loadContainer(id: String, name: String) async throws -> SessionHolder {
        ...
        // the whole process - taking Muwa with it. Caught here so the user
    ...
    //
    // Per OSAURUS-INTEGRATION.md §"Coordinator-owned KV sizing", Muwa
    ...
    // `enableDiskCache` (muwa-managed disk path, sandbox-aware).
    ...
    /// sizing) plus Muwa's per-environment disk-path config. See the
    ...
    private nonisolated static func buildCacheCoordinatorConfig(
        ...
        if let diskCacheDir {
            MuwaPaths.ensureExistsSilent(diskCacheDir)
        }
        ...
        // user-configured disk directory or the writable Muwa default;
    ...
    nonisolated static func cacheDiskDirectoryOverride(
        ...
        return resolvedServerRuntimeDirectory(directory) ?? MuwaPaths.diskKVCache()
    }
    ...
    nonisolated static func shouldUseTurboQuantByDefault(
        ...
        // Current no-sign Muwa warm rows prove disk L2 restore on that
    ...
    private nonisolated static func isDirectoryWritable(_ url: URL) -> Bool {
        let probe = url.appendingPathComponent(".muwa_write_probe_\(UUID().uuidString)")
        do {
    ...
    /// for model access, so Muwa only needs `ModelLease` (held for the
    ...
    /// `CacheCoordinator` - Muwa does not need to plumb anything cache-
    ...
    private func generateEventStream(
        ...
        // continuous-batching path that Muwa advertises as a feature.
    ...
    static func effectiveLoadFootprintBytes(
        ...
        // vMLX `.muwaProduction` loads routed JANG/JANGTQ through
    ...
    /// `MUWA_ALLOW_EXPERIMENTAL_DSV4_AFFINE_JANG=1` or
        ...
        throw MLXService.RuntimePolicyError(
            ...
            issues: [
                "Model '\(name)' is a plain affine DeepSeek V4 Flash JANG bundle. "
                    + "That path is not production-supported in this Muwa build because "
                    ...
                    + "MUWA_ALLOW_EXPERIMENTAL_DSV4_AFFINE_JANG=1."
            ]
        )
    }
    ...
    private static var experimentalDSV4AffineJANGAllowed: Bool {
        ...
        for key in [
            "MUWA_ALLOW_EXPERIMENTAL_DSV4_AFFINE_JANG",
    ...
    /// (Muwa's chat router lowercases names internally) still resolves
