...
final class SharedConfigurationService {
    ...
    private func baseDirectoryURL() -> URL {
        MuwaPaths.resolvePath(new: MuwaPaths.runtime(), legacy: "SharedConfiguration")
    }
    ...
    private func ensureDirectories() -> URL? {
        ...
        do {
            try MuwaPaths.ensureExists(baseDirectoryURL())
            try MuwaPaths.ensureExists(instance)
            return instance
        } catch {
            print("[Muwa] SharedConfigurationService: failed to create directories: \(error)")
            return nil
        }
    }
    ...
    func update(health: ServerHealth, configuration: ServerConfiguration, localAddress: String) {
        ...
        case .running:
            ...
            do {
                ...
                print("[Muwa] SharedConfigurationService: failed to write configuration: \(error)")
            }
        case .starting:
            ...
            do {
                ...
                print(
                    "[Muwa] SharedConfigurationService: failed to write starting configuration: \(error)"
                )
            }
        case .restarting:
            ...
            do {
                ...
                print(
                    "[Muwa] SharedConfigurationService: failed to write restarting configuration: \(error)"
                )
            }
    ...
    func remove() {
        ...
        do {
            if FileManager.default.fileExists(atPath: instance.path) {
                ...
                print(
                    "[Muwa] SharedConfigurationService: removed instance directory at \(instance.path)"
                )
            }
        } catch {
            print("[Muwa] SharedConfigurationService: failed to remove instance directory: \(error)")
        }
    }
}
