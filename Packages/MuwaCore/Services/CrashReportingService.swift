...
public final class CrashReportingService {
    ...
    /// `init` is `internal`, so the app (which links MuwaCore as a product)
    ...
    ///      `App/Muwa/Secrets.xcconfig`; in Release it's injected by CI.
    ...
    private static func resolveDSNFromConfig() -> String? {
            ...
            !plist.isEmpty
        {
                ...
                if !plist.contains("://") {
                    NSLog(
                        "[Muwa] SENTRY_DSN looks truncated (\"%@\"). An xcconfig treats "
