...
//  themes.muwa.ai. Combines a canonical encoder, the master-key
...
public enum ThemeShareError: LocalizedError, Sendable {
    ...
    case identity(MuwaIdentityError)
    case api(ThemesAPIError)

    public var errorDescription: String? {
        switch self {
        case .noMasterKey:
            return "No Muwa identity is set up yet. Create one in Identity settings before sharing themes."
        ...
        case .invalidIdentifier:
            return "Not a valid theme ID. Paste a 64-character hex hash or an muwa://themes-install link."
...
public final class ThemeShareService {
    ...
    /// Deep link constants. Mirrors `muwa://plugins-install?tool=…`.
    public static let deepLinkScheme = "muwa"
    ...
    public func share(_ theme: CustomTheme) async throws -> ThemeShareOutcome {
        ...
        } catch let error as MuwaIdentityError {
        ...
        let address: String
        do {
            address = try deriveMuwaId(from: privateKey).lowercased()
        } catch let error as MuwaIdentityError {
            throw ThemeShareError.identity(error)
        } catch {
        ...
        let message = "muwa-theme:\(address):\(bodyHash):\(nonce):\(timestamp)"
    ...
    public func install(hashOrLink rawInput: String) async throws -> CustomTheme {
        ...
        let tempURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("muwa-share-\(UUID().uuidString).json")
        do {
    ...
    /// Build the Muwa deep link for a content hash.
    ...
    /// Accepts a 64-char hex hash, an `muwa://themes-install?hash=…` deep
    /// link, or the public web URL `https://themes.muwa.ai/themes/<hash>`.
