...
final class ModelDownloadService: ObservableObject {
    ...
    private func startOrchestration(model: MLXModel, resuming: PausedSnapshot?) {
        ...
        if let needed = model.totalSizeEstimateBytes,
            ...
            let freeBytes = MuwaPaths.volumeFreeBytes(forPath: probePath.path),
    ...
    func delete(_ model: MLXModel) async {
        ...
        // references — Muwa never owns those files. "Deleting" one only
    ...
    /// OsaurusAI repos folded in by `applyMuwaOrgFetch` — running that on the
    ...
    /// hosts `url`. Delegates to `MuwaPaths.volumeFreeBytes(forPath:)`
    ...
    static func freeBytesOnVolume(containing url: URL) -> Int64? {
        MuwaPaths.volumeFreeBytes(forPath: url.path)
    }
