...
class SystemMonitorService: ObservableObject {
    ...
    ///
    /// `MuwaPaths.volumeFreeBytes` prefers the cheap filesystem free-space
    ...
    private func refreshStorageUsage() {
        ...
        Task.detached(priority: .utility) {
            ...
            let freeBytes = MuwaPaths.volumeFreeBytes(forPath: path) ?? 0
            let totalBytes = MuwaPaths.volumeTotalBytes(forPath: path) ?? 0
