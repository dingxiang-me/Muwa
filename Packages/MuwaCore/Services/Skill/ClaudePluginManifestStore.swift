...
public struct ClaudePluginManifestSnapshot: Codable, Sendable, Hashable {
    ...
    /// written by older Muwa builds still decode.
...
/// per `pluginId` under `~/.muwa/claude-plugins/manifests/`. Reads
...
public enum ClaudePluginManifestStore {
    ...
    public static func save(_ snapshot: ClaudePluginManifestSnapshot) -> Bool {
        let url = MuwaPaths.claudePluginManifestFile(for: snapshot.pluginId)
    ...
    public static func load(pluginId: String) -> ClaudePluginManifestSnapshot? {
        let url = MuwaPaths.claudePluginManifestFile(for: pluginId)
    ...
    public static func delete(pluginId: String) {
        let fm = FileManager.default
        try? fm.removeItem(at: MuwaPaths.claudePluginManifestFile(for: pluginId))
        try? fm.removeItem(at: MuwaPaths.claudePluginUserConfigFile(for: pluginId))
        try? fm.removeItem(at: MuwaPaths.claudePluginDataDir(for: pluginId))
        try? fm.removeItem(at: MuwaPaths.claudePluginCacheDir(for: pluginId))
    }
    ...
    public static func saveUserConfig(
        ...
        let url = MuwaPaths.claudePluginUserConfigFile(for: pluginId)
        MuwaPaths.ensureExistsSilent(url.deletingLastPathComponent())
    ...
    /// Return `~/.muwa/claude-plugins/data/<safeId>/`, creating it on
    ...
    public static func ensureDataDir(for pluginId: String) -> URL {
        let url = MuwaPaths.claudePluginDataDir(for: pluginId)
        MuwaPaths.ensureExistsSilent(url)
        return url
    }
    ...
    public static func dataDirSize(for pluginId: String) -> Int {
        ...
        return MuwaPaths.directorySize(at: url)
    }
}
