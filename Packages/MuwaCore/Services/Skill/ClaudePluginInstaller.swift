...
//  Muwa equivalent and tags everything with a stable plugin id so the
...
public struct ClaudePluginInstallReport: Sendable {
    ...
    public struct PluginSummary: Sendable {
        ...
        /// Stdio MCP servers we couldn't import because the Muwa sandbox
...
public final class ClaudePluginInstaller {
    ...
    /// keychain-disabled gate (`MUWA_DISABLE_KEYCHAIN_FOR_TESTS=1`)
    ...
    /// either `name:` Agent-Skills format or `id:` Muwa format). When that
    ...
    /// resolve once the skill lives in `~/.muwa/skills/<name>/`:
