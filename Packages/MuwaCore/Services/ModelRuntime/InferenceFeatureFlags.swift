...
//  prior muwa-side scheduler / cooperative-yield / multi-stream gates have
...
public enum InferenceFeatureFlags {
    private enum Keys {
        static let mlxBatchEngineMaxSize = "ai.muwa.scheduler.mlxBatchEngineMaxBatchSize"
    }
    ...
    /// → 2.7s).
    ///
    /// Muwa's primary use case is single-user chat through the macOS app,
