...
//  Muwa
//
//  Bridge from vmlx-swift `Generation` events to Muwa's typed
...
//  stream with `.info(stopReason: .stop)`. Muwa never inspects chunk
...
private let mapperSignposter = OSSignposter(subsystem: "ai.muwa", category: "Generation")
private let mapperLog = Logger(subsystem: "ai.muwa", category: "Generation")
...
enum GenerationEventMapper {
    ...
    ///   emits `.reasoning`, Muwa keeps that signal visible for root-cause
    ...
    private static func serializeArguments(
        ...
        do {
            ...
            let data = try JSONSerialization.data(withJSONObject: anyDict, options: .muwaCanonical)
