...
//  library - Muwa passes `stopSequences` as `GenerateParameters.extraStopStrings`
//  and forwards every event through `GenerationEventMapper`.
//
//  Muwa no longer parses tool calls, reasoning, or stop sequences at the
...
private let batchAdapterLog = Logger(subsystem: "ai.muwa", category: "BatchAdapter")
...
struct MLXBatchAdapter {
    ...
    static func effectiveGenerationSettings(
        ...
        // defaults. Muwa must not invent sampler defaults.
    ...
    static func generate(
        ...
        // local app path instead of inventing muwa-specific defaults.
        let modelDefaults = LocalGenerationDefaults.defaults(forModelId: modelName)
