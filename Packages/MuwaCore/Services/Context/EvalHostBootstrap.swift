...
//  Muwa
//
//  Public, off-process bootstrap helpers for the MuwaEvals package and
...
public enum EvalHostBootstrap {
    ...
    /// MuwaEvals runner so it can `skip + warn` cases whose
    /// `requirePlugins` aren't installed locally instead of failing
    /// them. Includes native dylib plugins (muwa.browser, etc.) —
