...
//
    ...
    public actor SandboxManager {
        ...
        private static let containerID = "muwa-sandbox"
        ...
        private static var bridgeSocketPath: String {
            MuwaPaths.container().appendingPathComponent("bridge.sock").path
        }
        /// Where the bridge socket appears inside the guest container
        private static let guestBridgeSocketPath = "/tmp/muwa-bridge.sock"
        ...
        fileprivate static let bridgeTokenDir = "/run/muwa"
        ...
        var staleContainerDir: URL {
            MuwaPaths.container().appendingPathComponent("containers/\(Self.containerID)")
        }
        ...
        private var hasRequiredAssets: Bool {
            ...
            return fm.fileExists(atPath: MuwaPaths.containerKernelFile().path)
                && fm.fileExists(atPath: MuwaPaths.containerInitFSFile().path)
        }
        ...
        public func removeContainer() async throws {
            ...
            var warnings: [String] = []
            let containersRoot = MuwaPaths.container().appendingPathComponent("containers")
            do {
                try await Self.forciblyRemoveAsync(at: containersRoot)
            } catch {
                warnings.append("containers/: \(error.localizedDescription)")
            }
            try? FileManager.default.removeItem(at: MuwaPaths.containerKernelFile())
            try? FileManager.default.removeItem(at: MuwaPaths.containerInitFSFile())
        ...
        public func execAsAgent(
            ...
            let cwd = pluginName.map { MuwaPaths.inContainerPluginDir(agentName, $0) }
        ...
        public func ensureAgentUser(_ agentName: String) async throws {
            ...
            let homeDir = MuwaPaths.inContainerAgentHome(agentName)
        ...
        /// to `/run/muwa/<linuxName>.token` inside the guest with mode 0600
        ...
        /// chown target exists.
        ///
        /// `agentId` ties the token to a specific Muwa agent so the bridge
        ...
        public func runDiagnostics() async -> [DiagnosticResult] {
            ...
            async let vsockD = diagnose("vsock-bridge") {
                let r = try await self.exec(
                    command: "curl -sf --unix-socket /tmp/muwa-bridge.sock http://localhost/api/log "
        ...
        private func ensureKernel() async throws -> Kernel {
            let kernelPath = MuwaPaths.containerKernelFile()
            ...
            let kernelDir = MuwaPaths.containerKernelDir()
            try MuwaPaths.ensureExists(kernelDir)
        ...
        private func ensureHostDirectories() throws {
            try MuwaPaths.ensureExists(MuwaPaths.container())
            try MuwaPaths.ensureExists(MuwaPaths.containerWorkspace())
            try MuwaPaths.ensureExists(MuwaPaths.containerAgentsDir())
            try MuwaPaths.ensureExists(MuwaPaths.containerSharedDir())
        }
        ...
        private func configureSandbox() async throws {
            ...
            // The shim at `/usr/local/bin/muwa-host` lives in the
            ...
            let shimScript = Self.muwaHostShimScript
            let shimDigest = SHA256.hash(data: Data(shimScript.utf8))
                .map { String(format: "%02x", $0) }
                .joined()
            let check = try? await exec(
                command: "sha256sum /usr/local/bin/muwa-host 2>/dev/null | awk '{print $1}'"
            )
            let existingDigest = check?.stdout
                .trimmingCharacters(in: .whitespacesAndNewlines)
            if existingDigest != shimDigest {
                let shimStagingPath = MuwaPaths.containerWorkspace()
                    .appendingPathComponent(".muwa-host-shim")
                try shimScript.write(to: shimStagingPath, atomically: true, encoding: .utf8)
                _ = try await execAsRoot(
                    command:
                        "cp /workspace/.muwa-host-shim /usr/local/bin/muwa-host && chmod 555 /usr/local/bin/muwa-host && rm /workspace/.muwa-host-shim"
                )
            }
        ...
        // MARK: - muwa-host Shell Shim

        private static let muwaHostShimScript = """
            #!/bin/sh
            # muwa-host — Host API bridge shim for sandbox plugins.
            ...
            # token at /run/muwa/$USER.token (mode 0600, owned by that user).
            # The host bridge derives the agent identity from the token alone —
            # caller-supplied X-Muwa-User headers are no longer trusted.
            SOCK="/tmp/muwa-bridge.sock"
            API="http://localhost/api"
            USER=$(id -un)
            PLUGIN="${MUWA_PLUGIN:-${MUWA_PLUGIN:-$(basename "$(pwd)")}}"
            TOKEN_FILE="/run/muwa/$USER.token"
            if [ ! -r "$TOKEN_FILE" ]; then
              echo "muwa-host: bridge token for $USER missing (host has not provisioned this agent yet)" >&2
              exit 1
            fi
            TOKEN=$(cat "$TOKEN_FILE")
            if [ -z "$TOKEN" ]; then
              echo "muwa-host: bridge token for $USER is empty" >&2
              exit 1
            fi
            ...
            case "$1" in
              secrets)
                case "$2" in
                  ...
                  *) echo "Usage: muwa-host secrets get <name>" >&2; exit 1 ;;
                esac ;;
              config)
                case "$2" in
                  get) _call "$API/config/$3" | jq -r '.value // empty' ;;
                  set) _call -X POST "$API/config/$3" -d "{\\"value\\":\\"$4\\"}" > /dev/null ;;
                  *) echo "Usage: muwa-host config get|set <key> [value]" >&2; exit 1 ;;
                esac ;;
              inference)
                case "$2" in
                  chat)
                  ...
                  *) echo "Usage: muwa-host inference chat -m <message>" >&2; exit 1 ;;
                esac ;;
              agent)
                case "$2" in
                  dispatch)
                  ...
                  memory)
                    case "$3" in
                      ...
                      *) echo "Usage: muwa-host agent memory query|store <text>" >&2; exit 1 ;;
                    esac ;;
                  *) echo "Usage: muwa-host agent dispatch|memory ..." >&2; exit 1 ;;
                esac ;;
              events)
                case "$2" in
                  emit) _call -X POST "$API/events/emit" -d "{\\"type\\":\\"$3\\",\\"payload\\":${4:-{}}}" > /dev/null ;;
                  *) echo "Usage: muwa-host events emit <type> [payload]" >&2; exit 1 ;;
                esac ;;
              plugin)
                case "$2" in
                  create) cat | _call -X POST "$API/plugin/create" -d @- ;;
                  *) echo "Usage: muwa-host plugin create < plugin.json" >&2; exit 1 ;;
                esac ;;
              log)
                _call -X POST "$API/log" \
                  -d "{\\"level\\":\\"$2\\",\\"message\\":\\"$3\\"}" > /dev/null ;;
              *) echo "Usage: muwa-host <secrets|config|inference|agent|events|plugin|log> ..." >&2; exit 1 ;;
            esac
            """
            ...
            (
                ...
                cocoa: [
                    NSFileWriteFileExistsError:
                        "Stale sandbox state on disk. Run `rm -rf ~/.muwa/container/containers/muwa-sandbox` and try again.",
                ...
                posix: [
                    ...
                    EACCES:
                        "Sandbox start denied by macOS — check that Muwa has the required entitlements.",
                    EPERM:
                        "Sandbox start denied by macOS — check that Muwa has the required entitlements.",
                ],
                substrings: [
