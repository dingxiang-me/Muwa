            ...
            if startupFailureCount >= Self.maxStartupFailures {
                if unavailability[agent.id] == nil {
                    recordUnavailability(
                        ...
                        kind: preStartKind,
                        message:
                            "Sandbox start has failed \(startupFailureCount) times this session — automatic retries disabled. Open the Sandbox settings panel to start it manually or check ~/.muwa/container/containers/muwa-sandbox for stale state."
                    )
                }
                return
            }
