...
public enum ToolIndexLogger {
    static let search = Logger(subsystem: "ai.muwa", category: "toolindex.search")
    static let service = Logger(subsystem: "ai.muwa", category: "toolindex.service")
}
...
/// `MUWA_DEBUG_CAPABILITY_SEARCH=1` log path in `CapabilitySearch.search`
...
public actor ToolSearchService {
    ...
    public func initialize() async {
        ...
        let storageDir = MuwaPaths.toolIndex().appendingPathComponent("vectura", isDirectory: true)

        for attempt in 1 ... 2 {
            do {
                MuwaPaths.ensureExistsSilent(storageDir)
