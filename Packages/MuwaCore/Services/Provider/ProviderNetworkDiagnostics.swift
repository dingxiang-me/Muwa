    ...
    private static func remoteStateRow(
        ...
        guard provider.enabled else {
            return ProviderDiagnosticRow(
                ...
                value: L("Disabled"),
                severity: .warning,
                detail: L("Muwa will not auto-connect this provider while the row toggle is off."),
    ...
    private static func remoteAuthRow(
        ...
        case .none:
            return ProviderDiagnosticRow(
                ...
                value: L("None"),
                severity: .info,
                detail: L("No Authorization header is added by Muwa.")
            )
        case .apiKey:
            if apiKeyPresent {
                return ProviderDiagnosticRow(
    ...
    private static func mcpStateRow(provider: MCPProvider, state: MCPProviderState?) -> ProviderDiagnosticRow {
        guard provider.enabled else {
            return ProviderDiagnosticRow(
                ...
                value: L("Disabled"),
                severity: .warning,
                detail: L("Muwa will not auto-connect this MCP provider while the row toggle is off.")
            )
        }
        if state?.isConnecting == true {
            return ProviderDiagnosticRow(
        ...
        case .stdio:
            ...
            return ProviderDiagnosticRow(
                ...
                detail: provider.executionHost == .host
                    ...
                    : L("Runs inside the Muwa sandbox and is torn down on disconnect.")
            )
        }
    }
