...
/// dispatch. `nil` means "no preset" (the special Muwa-Agent peer path).
public struct ProviderCredentialRequest: Sendable {
    ...
    /// `.custom` so the sheet still renders fields. `.muwa` keeps `preset = nil`
    ...
    public init(
        ...
        if providerType == .muwa {
            self.preset = nil
            self.providerType = .muwa
            self.providerName = providerName
            self.mode = mode
            self.instructions = ProviderCredentialInstructionsCatalog.muwaAgentEntry()
            return
        }
