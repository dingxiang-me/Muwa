...
/// `log stream --debug --predicate 'subsystem == "ai.muwa" AND category == "reasoning"'`.
private let reasoningLogger = Logger(subsystem: "ai.muwa", category: "reasoning")
...
public actor RemoteProviderService: ToolCapableService {
    ...
    func generateOneShot(
        ...
        // Native Muwa agents only support the streaming /agents/{id}/run endpoint.
        // Consume the SSE stream and concatenate all text deltas into a single string.
        if provider.providerType == .muwa {
    ...
    func respondWithTools(
        ...
        // Native Muwa agents run tools server-side and only expose a streaming endpoint.
        // Route through generateOneShot, which consumes the SSE stream for .muwa.
        if provider.providerType == .muwa {
    ...
    func streamWithTools(
        ...
        // Native Muwa agents run tools server-side — the /agents/{id}/run endpoint handles
    ...
    static func handleStreamEvent(
        ...
        do {
            ...
            case .openaiLegacy, .azureOpenAI, .muwa:
            ...
            print("[Muwa] Warning: Failed to parse SSE chunk: \(error.localizedDescription)")
            return .continue
        }
    }
        ...
        if let parts = chunk.candidates?.first?.content?.parts {
            for part in parts {
                ...
                case .functionCall(let funcCall):
                    ...
                    print("[Muwa] Gemini tool call detected: index=\(idx), name=\(funcCall.name)")
    ...
    private static func handleAnthropicEvent(
        ...
        case "content_block_start":
            ...
            if case .toolUse(let toolBlock) = startEvent.content_block {
                ...
                print("[Muwa] Anthropic tool call detected: index=\(idx), name=\(toolBlock.name)")
    ...
    private static func handleOpenResponsesEvent(
        ...
        case "response.output_item.added":
                ...
                case .functionCall(let funcCall) = addedEvent.item
            {
                ...
                print("[Muwa] Open Responses tool call detected: index=\(idx), name=\(funcCall.name)")
                yield(StreamingToolHint.encode(funcCall.name))
            }
        ...
        if let toolCalls = chunk.choices.first?.delta.tool_calls {
            for toolCall in toolCalls {
                ...
                if let name = toolCall.function?.name, current.name == nil {
                    current.name = name
                    print("[Muwa] OpenAI tool call detected: index=\(idx), name=\(name)")
    ...
    static func dispatchFinal(
        ...
        case .ready(let invocation):
            print(
                "[Muwa] Stream ended: emitting tool call '\(invocation.toolName)' "
        ...
        case .none:
            ...
            {
                print("[Muwa] Fallback: detected inline tool call '\(name)' in text")
                continuation.finish(
    ...
    private func _streamRemote(
        ...
        // Native Muwa peers: the plaintext request (Bearer included) is
        ...
        let secureProvider = providerType == .muwa ? self.provider : nil
    ...
    private static func geminiArgsJSON(from args: [String: AnyCodableValue]?) -> String {
        ...
        if let data = try? JSONSerialization.data(withJSONObject: dict, options: .muwaCanonical),
    ...
    private static func truncatedToolCallError(
        ...
        print(
            "[Muwa] Discarding truncated tool call '\(toolName)' — "
    ...
    private static func validateToolCallJSON(_ json: String) -> ValidatedToolCallJSON {
        ...
        if let data = repaired.data(using: .utf8),
            (try? JSONSerialization.jsonObject(with: data)) != nil
        {
            print("[Muwa] Repaired incomplete tool call JSON (\(json.count) -> \(repaired.count) chars)")
        ...
        print("[Muwa] Warning: Tool call JSON is malformed and could not be repaired: \(json.prefix(200))")
    ...
    private func buildURLRequest(for request: RemoteChatRequest) throws -> URLRequest {
        ...
        } else if requestProviderType == .muwa {
            // Native Muwa agent: POST /agents/{remoteAgentId}/run
        ...
        let encoder = JSONEncoder.muwaCanonical(prettyPrinted: true)
    ...
    private func parseResponse(
        ...
        case .muwa:
            // Native Muwa agent returns OpenAI-compatible responses
            let response = try JSONDecoder().decode(ChatCompletionResponse.self, from: data)
            let content = response.choices.first?.message.content
            return (content, nil)
        }
...
extension OpenResponsesRequest {
    func toCodexOAuthPayloadData() throws -> Data {
        let encoded = try JSONEncoder.muwaCanonical().encode(self)
        ...
        return try JSONSerialization.data(withJSONObject: object, options: .muwaCanonical)
    }
}
...
extension RemoteProviderService {
    ...
    /// Muwa's own preflight still validates tool arguments against the
...
extension RemoteProviderService {
    ...
    public static func fetchModels(from provider: RemoteProvider) async throws -> [String] {
        ...
        // Native Muwa agent — fetch all models from the server's /models endpoint
        if provider.providerType == .muwa {
            return try await fetchMuwaModels(from: provider)
        }
    ...
    private static func isOpenAICompatibleModelDiscoveryProvider(_ providerType: RemoteProviderType) -> Bool {
        ...
        case .openaiLegacy, .openResponses, .azureOpenAI:
            return true
        case .anthropic, .openAICodex, .gemini, .muwa:
            return false
        }
    }

    /// Fetch models for a native Muwa agent.
    ...
    private static func fetchMuwaModels(from provider: RemoteProvider) async throws -> [String] {
        ...
        if let url = provider.url(for: "/models") {
            ...
            if let (data, status) = await muwaGET(req, provider: provider),
                status < 400,
        ...
        req.timeoutInterval = min(provider.timeout, 10)
        for (key, value) in headers { req.setValue(value, forHTTPHeaderField: key) }
        guard let (data, status) = await muwaGET(req, provider: provider), status < 400 else {
            return ["default"]
        }
    ...
    /// Metadata GET against an Muwa peer. Prefers the Secure Channel (so
