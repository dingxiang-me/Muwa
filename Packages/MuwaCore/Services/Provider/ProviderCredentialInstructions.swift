...
public enum ProviderCredentialInstructionsCatalog {
    ...
    /// Special entry for the Muwa-Agent peer path. `.muwa` has no
    /// `ProviderPreset` because it isn't a third-party vendor — it's
    /// another Muwa instance reachable over the local network. The
    /// sheet still needs an entry so the user can paste a pairing key.
    public static func muwaAgentEntry() -> ProviderCredentialInstructions {
        ProviderCredentialInstructions(
            providerType: .muwa,
            displayName: L("Muwa Agent"),
            authMethod: .apiKey,
            getKeyURL: nil,
            keyFormatHint: L("Paste the pairing API key from the remote Muwa."),
            storageAuthType: .apiKey,
            presetId: ""
        )
    }
