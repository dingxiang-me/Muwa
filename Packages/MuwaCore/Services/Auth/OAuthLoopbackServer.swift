...
public final class OAuthLoopbackServer: @unchecked Sendable {
    private static let queue = DispatchQueue(label: "ai.muwa.oauth-loopback")
    ...
    private func sendResponse(
        ...
        let message =
            success
            ? "You can return to \(ProductBrand.name)."
            : "\(ProductBrand.name) could not complete the sign-in. Please try again."
        let body = """
