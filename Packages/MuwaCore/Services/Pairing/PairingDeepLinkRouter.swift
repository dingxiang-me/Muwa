...
//  Muwa
//
//  Parses `muwa://<addr>?pair=<base64url(invite)>` deeplinks and stages
...
public enum PairingDeepLinkRouter {
    ...
    /// URL not in the `muwa://` scheme so other handlers (e.g. Hugging Face)
    /// can take over.
    @discardableResult
    public static func handle(_ url: URL) -> Bool {
        guard url.scheme?.lowercased() == ProductBrand.urlScheme else { return false }
