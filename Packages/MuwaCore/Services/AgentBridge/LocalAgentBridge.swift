...
public final class LocalAgentBridge: @unchecked Sendable, AgentRuntimeBridge {
    ...
    public func notify(
        ...
        // `muwa-evals` CLI. Log instead of crashing the headless run;
        // the bundled app path is unaffected.
        guard Bundle.main.bundleIdentifier != nil else {
            print("[Muwa] notify (headless, suppressed): \(agentName) · \(title) — \(body)")
            return
        }
    ...
    private func serialQueue(for agentId: UUID) -> DispatchQueue {
        ...
        let q = DispatchQueue(label: "ai.muwa.bridge.\(agentId.uuidString)")
