...
//  bundle is a tar archive (`.muwa-agent`) containing:
//
...
//  `~/.muwa/agents/<id>/` and rekey `db.sqlite` to the local storage
//  key.
//
...
public actor AgentBundleService {
    ...
    /// Build a `.muwa-agent` bundle for `agentId`, sealed with
    /// `passphrase`. `destinationDirectory` must exist and be writable;
    /// the file is named `<agent-slug>.muwa-agent`. Returns the URL
    ...
    public func exportBundle(
        ...
        //    can't ship the live `~/.muwa/agents/<id>/db.sqlite`
        ...
        let scratch = try makeScratchDirectory(prefix: "muwa-agent-export-")
        ...
        let agentDir = MuwaPaths.agentDirectory(for: agentId)
        try copyIfExists(
            from: agentDir.appendingPathComponent("schema.sql"),
            to: scratch.appendingPathComponent("schema.sql")
        )
        try copyDirIfExists(
            from: MuwaPaths.agentViewsDirectory(for: agentId),
            to: scratch.appendingPathComponent("views")
        )
        try copyDirIfExists(
            from: MuwaPaths.agentMigrationsDirectory(for: agentId),
            to: scratch.appendingPathComponent("migrations")
        )
        try copyDirIfExists(
            from: MuwaPaths.agentRunsDirectory(for: agentId),
        ...
        let bundleURL =
            ...
            .appendingPathComponent("\(slug).muwa-agent")
    ...
    /// Unpack and verify a bundle without touching `~/.muwa/`. The
    ...
    public func openBundleForReview(
        ...
        let staging = try makeScratchDirectory(prefix: "muwa-agent-import-")
        do {
    ...
    /// into `~/.muwa/agents/<id>/`, rekey `db.sqlite` from the
    ...
    public func activate(preview: ImportPreview) async throws -> Agent {
        ...
        let agentDir = MuwaPaths.agentDirectory(for: agent.id)
        MuwaPaths.ensureExistsSilent(agentDir)
        ...
        try moveDirOverwritingIfExists(
            ...
            to: MuwaPaths.agentViewsDirectory(for: agent.id).path
        )
        try moveDirOverwritingIfExists(
            from: staging.appendingPathComponent("migrations").path,
            to: MuwaPaths.agentMigrationsDirectory(for: agent.id).path
        )
        try moveDirOverwritingIfExists(
            from: staging.appendingPathComponent("runs").path,
            to: MuwaPaths.agentRunsDirectory(for: agent.id).path
        )
    ...
    private func copyAndRekeyAgentDB(
        ...
        let fm = FileManager.default
        let src = MuwaPaths.agentDatabaseFile(for: agentId).path
    ...
    /// though the extension is `.muwa-agent`; future versions can
