...
final class NotificationService: NSObject, UNUserNotificationCenterDelegate {
    ...
    /// exception in processes without an app bundle (the `muwa-evals`
    ...
    func postSafeModeActive() {
        guard let center else { return }
        let content = UNMutableNotificationContent()
        content.title = L("Muwa started in safe mode")
        content.body = L("Plugins disabled after repeated crashes. Run \"muwa tools reset\" in Terminal to recover.")
