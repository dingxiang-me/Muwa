...
final class PluginHostContext: @unchecked Sendable {
    ...
    /// the plugin's `init` / `muwa_plugin_entry_v2` call and cleared
    ...
    static func parseRawRequest(_ requestJSON: String) -> (json: [String: Any], sanitized: Data)? {
        ...
        guard let cleanData = try? JSONSerialization.data(withJSONObject: clean, options: .muwaCanonical)
        else { return nil }
    ...
    func complete(requestJSON: String) -> String {
        ...
        return Self.blockingAsync(fallback: Self.pluginHostTimeoutJSON(kind: "complete")) {
            ...
            if exit == .endedBySurface, let intercept = interceptedExit {
                ...
                guard let encoded = try? JSONEncoder.muwaCanonical().encode(synthesized),
            ...
            guard let encoded = try? JSONEncoder.muwaCanonical().encode(response),
                var json = (try? JSONSerialization.jsonObject(with: encoded)) as? [String: Any]
            else {
                return Self.jsonString([
                    "error": "serialization_error", "message": "Failed to serialize response",
    ...
    private static func emitChunk(
        ...
        guard let callback,
            let data = try? JSONSerialization.data(withJSONObject: payload, options: .muwaCanonical),
    ...
    func httpRequest(requestJSON: String) -> String {
        ...
        let suffix = "Muwa/\(appVersion) Plugin/\(pluginId)"
    ...
    func fileRead(requestJSON: String) -> String {
        ...
        let allowedPrefix = MuwaPaths.artifactsDir().standardizedFileURL.path + "/"
...
extension PluginHostContext {
    ...
    private static let blockingBridgeQueue = DispatchQueue(
        label: "com.muwa.pluginHost.blockingBridge",
        qos: .userInitiated,
    ...
    static func jsonString(_ dict: [String: Any]) -> String {
        guard let data = try? JSONSerialization.data(withJSONObject: dict, options: .muwaCanonical)
        else { return "{}" }
...
extension PluginHostContext {
    ...
    private static let tlsKey: String = "ai.muwa.plugin.active"
    ...
    private static let agentTlsKey: String = "ai.muwa.plugin.agent"
    ...
    /// the documented contract in `muwa_plugin.h`:
    ...
    /// `muwa_plugin.h` for the contract rationale (plugins should
