...
//  repetition_penalty / do_sample) so Muwa can honor them when the
...
//       JANG, JANGTQ, FP16, ...). Muwa mirrors vmlx's
...
enum LocalGenerationDefaults {
    ...
    /// loader and tool-parser / reasoning-parser resolvers, not Muwa's
    /// sampling overlay.
