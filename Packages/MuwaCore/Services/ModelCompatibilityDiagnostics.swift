...
//  This is intentionally host-side only: it explains what Muwa can prove
    ...
    private static func sourceStatus(isLocal: Bool, externalSource: String?) -> SourceStatus {
        if let externalSource {
            return SourceStatus(
                ...
                detail: L("Referenced in place; Muwa does not copy or mutate this bundle.")
            )
        }
        if isLocal {
            return SourceStatus(
                kind: .muwaLocal,
                title: L("Muwa local models"),
                detail: L("Stored under the configured Muwa model directory.")
            )
        }
        return SourceStatus(
            kind: .catalog,
    ...
    private static func runtimeStatus(
        ...
        case .notDownloaded:
            return RuntimeStatus(
                ...
                detail: L("Muwa cannot prove runtime behavior until the model bundle is local.")
            )
        case .incomplete:
            return RuntimeStatus(
                kind: .blocked,
    ...
    private static func unsupportedFamilyStatus(
        ...
        if isHunyuanDense {
            return RuntimeStatus(
                ...
                detail:
                    L(
                        "Unsupported local model type: hunyuan_v1_dense. Muwa needs vmlx Hunyuan Dense support before this model can run locally."
                    )
            )
        }
        ...
        if isLongCat {
            return RuntimeStatus(
                ...
                detail:
                    L(
                        "LongCat local bundles require native vmlx architecture, processor, cache, and media-path support before Muwa should offer them as runnable."
                    )
            )
        }
