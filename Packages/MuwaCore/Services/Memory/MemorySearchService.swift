...
//  VecturaKit instance under `~/.muwa/memory/vectura/<agentId>/`,
...
public actor MemorySearchService {
    ...
    private static func storageDir(for agentId: String?) -> URL {
        ...
        let base = MuwaPaths.memory().appendingPathComponent("vectura", isDirectory: true)
        ...
        for attempt in 1 ... 2 {
            do {
                ...
                let config = try VecturaConfig(
                    name: "muwa-memory-\(bucket.isEmpty ? "shared" : bucket)",
