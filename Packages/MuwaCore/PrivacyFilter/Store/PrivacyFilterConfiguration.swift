...
//  JSON under `~/.muwa/config/privacy-filter.json`. Per-provider
...
public struct PrivacyFilterConfiguration: Codable, Equatable, Sendable {
    ...
    /// block in `init(from:)`. Files written by older Muwa
    ...
    /// positives after an Muwa update that adds a preset.
