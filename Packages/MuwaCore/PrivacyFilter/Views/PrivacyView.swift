...
private struct PrivacyRulesTab: View {
    ...
    private var detectionPatternsSection: some View {
        SettingsSection(title: L("Detection Patterns"), icon: "ruler") {
            VStack(alignment: .leading, spacing: 10) {
                Text(
                    "Built-in deterministic detectors run alongside the on-device model. Turning a category off stops Muwa from flagging it AND from blocking sends when it leaks past redaction.",
                    bundle: .module
                )
                .font(.system(size: 11))
                .foregroundColor(theme.tertiaryText)
        ...
        Button {
            ...
            HStack(alignment: .top, spacing: 8) {
                VStack(alignment: .leading, spacing: 3) {
                    ...
                    Text(
                        "All disabled by default. Enable individually — Muwa will redact matches and block sends that leak them.",
                        bundle: .module
                    )
                    .font(.system(size: 11))
                    .foregroundColor(theme.tertiaryText)
    ...
    private var customRulesSection: some View {
        SettingsSection(title: L("Custom Rules"), icon: "wand.and.rays") {
            VStack(alignment: .leading, spacing: 10) {
                Text(
                    "Define your own regex patterns — internal codenames, customer IDs, anything Muwa's built-ins don't cover. Bad patterns are validated before save.",
                    bundle: .module
                )
                .font(.system(size: 11))
                .foregroundColor(theme.tertiaryText)
