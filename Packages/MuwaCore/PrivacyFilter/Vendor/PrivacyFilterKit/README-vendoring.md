...
## Rewires applied (search "Muwa-local rewires" in the source)
...
| `Decoder/Label.swift` + transitive | `Label` / `LabelTable` / `LabelTableError` | `BIOESLabel` / `BIOESLabelTable` / `BIOESLabelTableError` — the original names collide with SwiftUI's generic `Label<Title, Icon>` once they share the MuwaCore module namespace |
...
cp /tmp/pfk/Sources/PrivacyFilterKit/PrivacyFilterKit.swift \
   Packages/MuwaCore/PrivacyFilter/Vendor/PrivacyFilterKit/
cp /tmp/pfk/Sources/PrivacyFilterKit/Decoder/*.swift \
   Packages/MuwaCore/PrivacyFilter/Vendor/PrivacyFilterKit/Decoder/
cp /tmp/pfk/Sources/PrivacyFilterKit/Model/*.swift \
   Packages/MuwaCore/PrivacyFilter/Vendor/PrivacyFilterKit/Model/
cp /tmp/pfk/Sources/PrivacyFilterKit/Tokenizer/*.swift \
   Packages/MuwaCore/PrivacyFilter/Vendor/PrivacyFilterKit/Tokenizer/
```
