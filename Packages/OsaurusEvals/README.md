...
- `expect.agentLoop.workflowSaved` — `{ nameContains?, minSteps?, runnable? }` outcome assertion: at least one workflow matching every present sub-field must have been **created in `WorkflowDatabase`** during the run (scored by diffing workflow ids before/after the loop). `runnable: true` additionally requires that workflow to pass the same static contract validation `workflow_run` preflights with (`WorkflowContract.validate`: templates parse, placeholders resolve against declared parameters, rendered args satisfy each tool's schema) — a save→run round trip, not just a persisted row. The runner deletes and unindexes every workflow created during the run regardless of pass/fail, so `workflow_save` cases can't pollute the developer's live DB.
...
- `save-after-task` — the model performs a small file task, then saves it via `workflow_save` (confirmation pre-granted in the query, matching the nudge's user-confirm contract). Scored with `workflowSaved` against the database diff — including `runnable: true`, so the saved workflow must pass the run-time contract preflight, not just persist a row.
