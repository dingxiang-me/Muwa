...
@main
struct OsaurusCLI: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        commandName: "osaurus-cli",
        abstract: "Headless osaurus inference server (OpenAI-compatible).",
...
struct Serve: AsyncParsableCommand {
    ...
    var modelDir: String = NSString(string: "~/.osaurus/models").expandingTildeInPath
    ...
    mutating func run() async throws {
        ...
        FileHandle.standardError.write(
            Data("[osaurus-cli] serving http://\(host):\(port) (models: \(resolved.path))\n".utf8)
        )
...
struct Version: ParsableCommand {
    static let configuration = CommandConfiguration(
        abstract: "Print osaurus-cli version."
    )

    func run() {
        print("osaurus-cli 0.1.0")
    }
}
