...
/// acceptance bit. **Workflows + skills lanes are pure embedding** —
...
public struct CapabilitySearchEvaluation: Sendable, Codable {
    ...
    /// Embed-cosine acceptance floor applied to the **workflows** lane.
    /// Echoes `CapabilitySearch.minimumRelevanceScoreWorkflows`. The
    ...
    public let appliedWorkflowsThreshold: Float
    /// Embed-cosine acceptance floor applied to the **skills** lane.
    /// Held independent of the workflows cutoff so a future eval pass
...
public enum CapabilitySearchEvaluator {
    ...
    /// hybrid (tools) + pure-embedding (workflows/skills) hits with
    ...
    /// is used. The workflows + skills lanes always use their own
    /// production constants (`minimumRelevanceScoreWorkflows` and
    ...
    /// workflows/skills quality gate when sweeping low fused values
    ...
    public static func evaluate(
        ...
        let appliedWorkflowsThreshold = CapabilitySearch.minimumRelevanceScoreWorkflows
        ...
        async let workflowPair = WorkflowSearchService.shared.searchWithDiagnostic(
            query: query,
            topK: topK,
            threshold: appliedWorkflowsThreshold
        )
        ...
        let (_, workflowDiag) = await workflowPair
        ...
        return CapabilitySearchEvaluation(
            ...
            workflowHits: makeEmbeddingOnlyHits(
                raw: workflowDiag.rawHits,
                accepted: workflowDiag.acceptedHits
            ),
            ...
            appliedWorkflowsThreshold: appliedWorkflowsThreshold,
    ...
    /// Build `[Hit]` from a pure-embedding diagnostic (workflows,
...
/// so `makeEmbeddingOnlyHits` can be written once across the workflows
...
extension WorkflowSearchDiagnostic.Hit: SearchDiagnosticHit {}
