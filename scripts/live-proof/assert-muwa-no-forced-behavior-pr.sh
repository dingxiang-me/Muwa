...
SOURCE_ROOT="$ROOT/Packages/MuwaCore"
...
active_forbidden="$({ ps -axo pid,ppid,rss,etime,command || true; } \
  ...
  | rg -v 'rg -i|assert-muwa-no-forced-behavior-pr' || true)"
...
if [[ "$fail" -ne 0 ]]; then
  echo "Muwa no-forced-behavior PR guard failed." >&2
  exit 1
fi

echo "Muwa no-forced-behavior PR guard passed."
