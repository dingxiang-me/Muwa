...
fail=0
VMLX_PIN="$(sed -nE 's/.*revision: "([0-9a-f]{40})".*/\1/p' "$ROOT/Packages/MuwaCore/Package.swift" | head -1 || true)"
...
VMLX_READY="$ROOT/scripts/live-proof/assert-muwa-vmlx-pr-readiness.sh"
...
NO_FORCED_GUARD="$ROOT/scripts/live-proof/assert-muwa-no-forced-behavior-pr.sh"
...
require_text "$ROOT/scripts/live-proof/build-keychain-free-muwa.sh" 'CODE_SIGNING_ALLOWED=NO' \
  "keychain-free build disables Xcode signing"
require_text "$ROOT/scripts/live-proof/build-keychain-free-muwa.sh" 'timestamp=none' \
  "keychain-free build uses timestamp-free ad-hoc seal"
require_text "$ROOT/scripts/live-proof/open-keychain-free-muwa.sh" 'MUWA_DISABLE_KEYCHAIN_FOR_TESTS' \
...
if [[ -n "$VMLX_PIN" ]]; then
  require_text "$ROOT/Packages/MuwaCore/Package.swift" \
    "revision: \"$VMLX_PIN\"" \
    "Package.swift pinned to current vMLX revision $VMLX_PIN"
  require_text "$ROOT/Packages/MuwaCore/Tests/Service/RuntimePolicySourceTests.swift" \
    "$VMLX_PIN" \
    "RuntimePolicySourceTests guard pinned current vMLX revision"
else
  fail_msg "Package.swift does not expose a 40-hex vMLX revision"
fi
require_text "$ROOT/Packages/MuwaCore/Tests/Service/ModelRuntimeIsHybridTests.swift" \
...
require_text "$ROOT/Packages/MuwaCore/Tests/Service/IsKnownHybridModelMCDCTests.swift" \
  'dealignai/Qwen3\.6-35B-A3B-MXFP4-CRACK-MTP' \
...
require_text "$ROOT/Packages/MuwaCore/Tests/Service/DSV4ParserPipelineTests.swift" \
...
require_text "$ROOT/Packages/MuwaCore/Tests/Service/SwiftTransformersTokenizerLoaderTests.swift" \
...
require_text "$ROOT/Packages/MuwaCore/Tests/Tool/ToolSearchServiceTests.swift" \
...
require_text "$ROOT/Packages/MuwaCore/Tests/Tool/CapabilityToolsTests.swift" \
  'capabilitiesSearchSchemaIsGemmaRenderable' \
  "capabilities_discover schema remains Gemma-renderable"
require_text "$ROOT/Packages/MuwaCore/Models/Configuration/ModelMediaCapabilities.swift" \
  '"qwen3_6", "qwen3_6_moe"' \
  "Qwen3.6 config-based media detection preserves video-capable model types"
require_text "$ROOT/Packages/MuwaCore/Tests/Service/MultiTurnFamilyMatrixTests.swift" \
  '"qwen3_6", "qwen3_6_moe"' \
  "multi-turn family matrix covers Qwen3.6 config-based media detection"
require_text "$ROOT/Packages/MuwaCore/Services/ModelRuntime/MLXBatchAdapter.swift" \
  'let engineDefaults = MLXLMCommon\.GenerateParameters\(\)' \
  "MLXBatchAdapter uses vMLX engine defaults"
require_text "$ROOT/Packages/MuwaCore/Networking/HTTPHandler.swift" \
  'ChannelEvent\.inputClosed|requestTasks\.cancelAll|Task\.checkCancellation' \
  "HTTP cancellation source path present"
require_text "$ROOT/Packages/MuwaCore/Views/Settings/ServerSettings/CacheSection.swift" \
...
require_text "$ROOT/Packages/MuwaCore/Views/Settings/ServerSettings/ConcurrencySection.swift" \
...
active_forbidden="$({ ps -axo pid,ppid,rss,etime,command || true; } \
  ...
  | rg -v 'rg -i|assert-muwa-pr-hygiene|assert-keychain-free-proof-path|assert-muwa-vmlx-pr-readiness|assert-vmlx-gemma4-parser-fix-wired|assert-no-hidden-local-sampler-defaults|assert-openresponses-cache-proof-wiring|assert-muwa-no-forced-behavior-pr|assert-server-settings-runtime-wiring|assert-chat-reasoning-delta-routing|assert-chat-ui-reasoning-routing|assert-http-channel-load-cancellation' || true)"
...
echo "Muwa PR hygiene guard passed."
