...
CORE_RESOLVED="$ROOT/Packages/MuwaCore/Package.resolved"
WORKSPACE_RESOLVED="$ROOT/Muwa.xcworkspace/xcshareddata/swiftpm/Package.resolved"
APP_RESOLVED="$ROOT/App/Muwa.xcodeproj/project.xcworkspace/xcshareddata/swiftpm/Package.resolved"
...
if [[ -n "$core_pin" ]]; then
  pass "MuwaCore Package.resolved vMLX pin: $core_pin"
else
  fail_msg "MuwaCore Package.resolved vMLX pin not found"
fi
...
if [[ -n "$pkg_pin" && -n "$core_pin" && -n "$workspace_pin" && -n "$app_pin" ]]; then
  if [[ "$pkg_pin" == "$core_pin" && "$pkg_pin" == "$workspace_pin" && "$pkg_pin" == "$app_pin" ]]; then
    pass "all Muwa vMLX pin surfaces agree"
  else
    fail_msg "Muwa vMLX pin surfaces disagree: package=$pkg_pin core=$core_pin workspace=$workspace_pin app=$app_pin"
  fi
fi
...
active_forbidden="$({ ps -axo pid,ppid,rss,etime,command || true; } \
  ...
  | rg -v 'rg -i|assert-muwa-vmlx-pr-readiness|assert-keychain-free-proof-path|assert-vmlx-gemma4-parser-fix-wired|assert-no-hidden-local-sampler-defaults|assert-openresponses-cache-proof-wiring|assert-server-settings-runtime-wiring' || true)"
...
if [[ "$fail" -ne 0 ]]; then
  cat >&2 <<'EOF'
Muwa vMLX PR readiness is BLOCKED.
...
echo "Muwa vMLX PR readiness source guard passed."
