...
| HY3/Hunyuan local rows | live blocked | `/v1/models` on the current `775e785e` no-sign app did not list `hy3`/`hunyuan` | live Osaurus proof is blocked by missing imported model id. Do not infer HY3/CAA/CCA behavior from ZAYA, MiniMax, or MiMo source guards |
